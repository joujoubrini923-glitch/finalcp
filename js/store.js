...
(function () {
  ...
  Store.dailyBonusTiers = () => (db.settings.dailyBonusTiers || []).filter((t) => Number(t.count) > 0 && Number(t.points) > 0).sort((a, b) => Number(a.count) - Number(b.count));
  ...
  Store.bonusPoints = function (sid) {
    ...
    const start = Store.newScoringStart ? Store.newScoringStart() : '2026-09-06';
    const bonusSolves = Store.solvesOf(sid).filter((x) => x.date >= start);
    const tiers = Store.dailyBonusTiers();
    if (tiers.length) {
      const perDay = {};
      bonusSolves.forEach((x) => { perDay[x.date] = (perDay[x.date] || 0) + 1; });
      Object.values(perDay).forEach((n) => { tiers.forEach((t) => { if (n >= t.count) bonus += t.points; }); });
    }
  ...
  Store.scoreEventsOf = function (sid) {
    ...
    const dailyTiers = Store.dailyBonusTiers();
    if (dailyTiers.length) {
      const perDay = {};
      bonusSolves.forEach((x) => { (perDay[x.date] ||= []).push(x); });
      Object.entries(perDay).forEach(([date, list]) => {
        dailyTiers.forEach((t) => {
          if (list.length >= t.count) add(date, t.points, 'habit', `${t.count}-solve daily bonus`, `${list.length} problems solved that day`, 'zap');
        });
      });
    }
