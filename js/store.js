...
(function () {
  ...
  Store.streakBonusCount = function (sid) {
    const weeks = Array.from(new Set(Store.solvesOf(sid).map((x) => mondayOf(x.date)))).sort();
  ...
  Store.bonusPoints = function (sid) {
    ...
    if (tiers.length) {
      const perDay = {};
      Store.solvesOf(sid).forEach((x) => { perDay[x.date] = (perDay[x.date] || 0) + 1; });
      Object.values(perDay).forEach((n) => { tiers.forEach((t) => { if (n >= t.count) bonus += t.points; }); });
    }
    if ((S.bonusTopicMastery | 0) > 0) bonus += Store.mastery(sid).filter((m) => m.done).length * S.bonusTopicMastery;
    return bonus;
  };
  ...
  Store.score = (sid) => Math.max(0, Store.solvePoints(sid) + Store.contestPointsSum(sid) + Store.bonusPoints(sid) + Store.adjustOf(sid).score);
  ...
  Store.scoreEventsOf = function (sid) {
    ...
    if ((S.bonusWeeklyStreak | 0) > 0) {
      const byWeek = {};
      solves.forEach((x) => { (byWeek[mondayOf(x.date)] ||= []).push(x); });
    ...
    if (dailyTiers.length) {
      const perDay = {};
      solves.forEach((x) => { (perDay[x.date] ||= []).push(x); });
