...
(function () {
  ...
  function defaultSettings() {
    return {
      ...
      tagline: 'A structured academy for students who want to learn problem solving, grow through practice and discover the joy of competition.',
      taglineFr: 'Une académie structurée pour apprendre à résoudre des problèmes, progresser avec la pratique et découvrir le plaisir de la compétition.',
      cohortDates: 'Late August',
      cohortDatesFr: 'Fin août',
      programDuration: '16 hours',
      programDurationFr: '16 heures',
      course: {
        level: 'LEVEL 1', status: 'COMING SOON', title: '16 hours to start coding with confidence.',
        description: 'Learn C++, problem solving and upgrade your CV to another level.',
        age: 'Ages 13–17', period: 'Late August', duration: '16 hours',
        experience: 'No experience needed.', learn: ['C++', 'Problem solving', 'CV upgrade'],
        locationName: '', locationAddress: '', locationDescription: '', locationMapUrl: '',
      },
      nextCourses: {
        g_rookies: { enabled: true, level: 'LEVEL 2', status: 'NEXT LEVEL', title: 'Go further with problem solving.', description: 'Build on your C++ foundations with deeper problems and new techniques.', age: 'Ages 13–17', period: 'Dates to be announced', duration: '16 hours', experience: 'For students who completed Level 1.', learn: ['Algorithms', 'Data structures', 'Contest practice'] },
        g_challengers: { enabled: true, level: 'LEVEL 3', status: 'NEXT LEVEL', title: 'Push your problem-solving further.', description: 'Train with harder techniques, more strategy and stronger contest habits.', age: 'Ages 13–17', period: 'Dates to be announced', duration: '16 hours', experience: 'For students ready for the next challenge.', learn: ['Graphs', 'Dynamic programming', 'Contest strategy'] },
        g_elite: { enabled: true, level: 'ELITE', status: 'NEXT LEVEL', title: 'Prepare for harder competition.', description: 'Advanced practice for students targeting serious contests and olympiad-style problems.', age: 'Ages 13–17', period: 'Dates to be announced', duration: '16 hours', experience: 'For advanced academy students.', learn: ['Advanced algorithms', 'Hard problems', 'Competition strategy'] },
      },
      nextCourseAds: null,
      // Coach email used by the server-side notification function. The admin
      // UI falls back to SUPABASE_CONFIG.coachEmail until this is saved.
      notificationEmail: '',
      coachMessages: [],
      academyDescription: 'Abdelmajid CP helps students turn curiosity into confident problem solving. Through small-group coaching, carefully chosen challenges, contests and visible progress tracking, students learn to think clearly, build strong habits and take on harder problems one step at a time. Whether a student is writing their first program or preparing for serious competition, the academy provides a clear path, encouraging guidance and a community that makes progress feel exciting.',
      academyDescriptionFr: 'Abdelmajid CP aide les élèves à transformer leur curiosité en confiance face aux problèmes. Grâce à un accompagnement en petits groupes, des défis soigneusement choisis, des concours et un suivi clair des progrès, les élèves apprennent à raisonner, à construire de bonnes habitudes et à relever des problèmes de plus en plus difficiles. Qu’il s’agisse d’écrire son premier programme ou de préparer une compétition exigeante, l’académie propose un parcours clair, un accompagnement motivant et une communauté qui rend chaque progrès gratifiant.',
      ...
      clinicWeeklyLimit: 2,    // max buggy-code cases a student can send per week (0 = unlimited)
      missionDefaultReward: 50,
      bonusWeeklyStreak: 0,    // pts at week 2, then again each consecutive week; resets after a gap
      bonusMultiDay: 0,        // legacy pts per day with 2+ solves
      newScoringStart: '2026-09-06', // Phase 1 bonus rules apply only from this date
      legacyBonusSnapshot: null, // frozen previous bonus points before new scoring start
      dailyBonusTiers: [       // cumulative daily solve bonuses
        { count: 2, points: 15 },
        { count: 4, points: 40 },
        { count: 7, points: 100 },
      ],
      scoringRules: [
        { key: 'problems', name: 'Problem difficulty points', icon: 'target', order: 10, description: 'Every solved academy problem gives score based on its difficulty and topic XP toward mastery.' },
        { key: 'daily', name: 'Daily solve bonuses', icon: 'zap', order: 20, description: 'Daily bonuses are cumulative: solving 4 problems earns the 2-problem and 4-problem bonuses; solving 7 earns all daily tiers.' },
        { key: 'streak', name: 'Weekly streak bonus', icon: 'flame', order: 30, description: 'Weekly streak bonus rewards students who solve in consecutive weeks.' },
        { key: 'mastery', name: 'Topic mastery bonus', icon: 'award', order: 40, description: 'Topic mastery bonus rewards completing enough XP in a topic.' },
        { key: 'contests', name: 'Contest points', icon: 'trophy', order: 50, description: 'Contest podium and rank points are added from coach-recorded contests.' },
        { key: 'hints', name: 'Hint penalty', icon: 'lock', order: 60, description: 'Unlocked hints can reduce the points earned from that problem, but never below the minimum earnable score.' },
        { key: 'levels', name: 'Level requirements', icon: 'shield', order: 70, description: 'Levels are unlocked by meeting score, topic mastery and achievement requirements.' },
      ],
      scoringRuleDescriptions: {
        referral: 'Top 3 winners in the active leaderboard discount challenge get a referral code for one friend joining Level 1.',
        problems: 'Every solved academy problem gives score based on its difficulty and topic XP toward mastery.',
        daily: 'Daily bonuses are cumulative: solving 4 problems earns the 2-problem and 4-problem bonuses; solving 7 earns all daily tiers.',
        streak: 'Weekly streak bonus rewards students who solve in consecutive weeks.',
        mastery: 'Topic mastery bonus rewards completing enough XP in a topic.',
        contests: 'Contest podium and rank points are added from coach-recorded contests.',
        hints: 'Unlocked hints can reduce the points earned from that problem, but never below the minimum earnable score.',
        levels: 'Levels are unlocked by meeting score, topic mastery and achievement requirements.',
      },
      referralChallenges: [],
      bonusTopicMastery: 0,    // pts per mastered topic
      hintCuts: [10, 15, 20],  // score discount per hint (hint 1/2/3)
      hintFloor: 5,            // minimum points a hinted solve can still earn
      activeSeasonId: null,    // currently running season (see db.seasons)
      levels: [
      ...
      coach: {
        ...
        experience: 'Coach and academy founder focused on building clear problem-solving habits, disciplined practice and confidence through competition.',
        photo: null,
        achievements: [],
      },
    };
  ...
  function emptyDB() {
    return {
      ...
      students: [], groups: [], problems: [], contests: [], missions: [], attendanceSessions: [], trialRequests: [],
      catalog: [], activity: [], joinRequests: [], questions: [], nextCourseRequests: [], seasons: [],
    };
  }
  ...
  function normalizeDB(data) {
    ...
    S.coach = Object.assign(defaultSettings().coach, S.coach || {});
    S.course = Object.assign(defaultSettings().course, S.course || {});
    S.course.learn = Array.isArray(S.course.learn) ? S.course.learn.slice(0, 5).map((x) => String(x || '').slice(0, 80)) : defaultSettings().course.learn.slice();
    const defaultNext = defaultSettings().nextCourses;
    S.nextCourses = Object.assign({}, defaultNext, S.nextCourses || {});
    if (!Array.isArray(S.coachMessages)) S.coachMessages = [];
    Object.keys(S.nextCourses).forEach((gid) => {
      S.nextCourses[gid] = Object.assign({}, defaultNext[gid] || defaultSettings().course, S.nextCourses[gid] || {});
      S.nextCourses[gid].learn = Array.isArray(S.nextCourses[gid].learn) ? S.nextCourses[gid].learn.slice(0, 5).map((x) => String(x || '').slice(0, 80)) : [];
    });
    ...
    S.newScoringStart = S.newScoringStart || '2026-09-06';
    S.missionDefaultReward = Math.max(0, num(S.missionDefaultReward, 50));
    if (!Array.isArray(S.hintCuts) || S.hintCuts.length < 3) S.hintCuts = [10, 15, 20];
    S.hintCuts = S.hintCuts.slice(0, 3).map((v) => Math.max(0, num(v)));
    S.dailyBonusTiers = (Array.isArray(S.dailyBonusTiers) ? S.dailyBonusTiers : [])
      .map((t) => ({ count: Math.max(1, Math.trunc(num(t && t.count))), points: Math.max(0, Math.trunc(num(t && t.points))) }))
      .filter((t) => t.count > 0 && t.points > 0)
      .sort((a, b) => a.count - b.count);
    if (!S.dailyBonusTiers.length) S.dailyBonusTiers = defaultSettings().dailyBonusTiers.map((t) => ({ count: t.count, points: t.points }));
    ['clinicWeeklyLimit', 'bonusWeeklyStreak', 'bonusMultiDay', 'bonusTopicMastery', 'hintFloor'].forEach((k) => { S[k] = Math.max(0, num(S[k])); });
    if (!Array.isArray(data.seasons)) data.seasons = [];
    data.seasons.forEach((s) => { if (!Array.isArray(s.groupIds)) s.groupIds = []; if (!Array.isArray(s.awards)) s.awards = null; });
    if (!Array.isArray(data.joinRequests)) data.joinRequests = [];
    if (!Array.isArray(data.questions)) data.questions = [];
    if (!Array.isArray(data.nextCourseRequests)) data.nextCourseRequests = [];
    if (!Array.isArray(data.missions)) data.missions = [];
    if (!Array.isArray(data.attendanceSessions)) data.attendanceSessions = [];
    if (!Array.isArray(data.trialRequests)) data.trialRequests = [];
    (data.problems || []).forEach((p) => { p.score = num(p.score); p.xp = num(p.xp); p.certificate = !!p.certificate; });
    ...
    (data.students || []).forEach((s) => {
      ...
      if (!s.parentCode) s.parentCode = ('P-' + String(s.id || '').replace(/[^a-z0-9]/gi, '').slice(-6).toUpperCase()).padEnd(8, 'X');
    ...
    function legacyBonusForStudent(student) {
      if (!student) return 0;
      const start = String(S.newScoringStart || '2026-09-06').slice(0, 10);
      const probs = new Map((data.problems || []).map((p) => [p.id, p]));
      const solves = (student.solves || []).map((sv) => ({ sv, p: probs.get(sv.problemId) })).filter((x) => x.p && String(x.sv.date || '') < start);
      let bonus = 0;
      const weekly = Math.max(0, num(S.bonusWeeklyStreak));
      if (weekly > 0) {
        const monday = (dateStr) => { const x = new Date(String(dateStr || todayStr()).slice(0, 10) + 'T12:00:00'); const day = (x.getDay() + 6) % 7; x.setDate(x.getDate() - day); return x.toISOString().slice(0, 10); };
        const weeks = Array.from(new Set(solves.map((x) => monday(x.sv.date)))).sort();
        let previous = null, run = 0;
        weeks.forEach((week) => { const adjacent = previous && (Date.parse(week + 'T12:00:00') - Date.parse(previous + 'T12:00:00')) === 7 * 864e5; run = adjacent ? run + 1 : 1; if (run >= 2) bonus += weekly; previous = week; });
      }
      const oldDaily = Math.max(0, num(S.bonusMultiDay));
      if (oldDaily > 0) {
        const perDay = {};
        solves.forEach((x) => { const d = String(x.sv.date || '').slice(0, 10); if (d) perDay[d] = (perDay[d] || 0) + 1; });
        Object.values(perDay).forEach((n) => { if (n >= 2) bonus += oldDaily; });
      }
      const mastery = Math.max(0, num(S.bonusTopicMastery));
      if (mastery > 0) {
        const xp = {};
        solves.forEach((x) => { const topicName = x.p.topic; xp[topicName] = (xp[topicName] || 0) + (num(x.p.xp)); });
        bonus += (S.topics || []).filter((t) => (xp[t.name] || 0) >= num(t.masteryXP)).length * mastery;
      }
      return Math.max(0, Math.trunc(bonus));
    }
    if (!S.legacyBonusSnapshot || !S.legacyBonusSnapshot.values) {
      const values = {};
      (data.students || []).forEach((student) => { values[student.id] = legacyBonusForStudent(student); });
      S.legacyBonusSnapshot = { __date: S.newScoringStart, createdAt: new Date().toISOString(), values };
    }
    data.version = VER;
    return data;
  }
  ...
  Store._cloudFetch = async function (onApplied) {
    ...
    try {
      ...
      if (res.data && cloudLooksValid(res.data.data)) {
        ...
        ['joinRequests', 'questions', 'nextCourseRequests'].forEach((key) => {
  ...
  Store._cloudPush = async function () {
    ...
    try {
      ...
      const { passwordHash, notificationEmail, ...safeSettings } = db.settings;
      const { joinRequests, questions, nextCourseRequests, ...publicDb } = db;
      const payload = {
        ...publicDb,
        settings: safeSettings,
        // Student/contact submissions are private. They live in their own
        // coach-only tables and must never be copied into the publicly
        // readable app_state document.
        joinRequests: [],
        questions: [],
        nextCourseRequests: [],
      };
      void passwordHash; void notificationEmail; void joinRequests; void questions; void nextCourseRequests;
  ...
  Store.dailyBonusTiers = () => {
    const rows = (db.settings.dailyBonusTiers || [])
      .map((t) => ({ count: Math.max(1, Math.trunc(Number(t && t.count) || 0)), points: Math.max(0, Math.trunc(Number(t && t.points) || 0)) }))
      .filter((t) => t.count > 0 && t.points > 0)
      .sort((a, b) => a.count - b.count);
    return rows.length ? rows : defaultSettings().dailyBonusTiers.map((t) => ({ count: t.count, points: t.points }));
  };
  Store.newScoringStart = () => String(db.settings.newScoringStart || '2026-09-06').slice(0, 10);
  Store.unsolvedProblemsOf = function (sid, allAcademy) {
    const s = Store.student(sid); if (!s) return [];
    const solved = new Set(Store.solvesOf(sid).map((x) => x.problem.id));
    return Store.problems().filter((p) => !solved.has(p.id) && (allAcademy || !Array.isArray(p.groupIds) || !p.groupIds.length || p.groupIds.includes(s.groupId)));
  };
  Store.referralChallenges = () => db.settings.referralChallenges || [];
  Store.activeReferralChallenges = () => Store.referralChallenges().filter((c) => c.enabled !== false && !c.finalized && (!c.start || c.start <= todayStr()) && (!c.end || c.end >= todayStr()));
  Store.visibleReferralChallengesFor = function (sid) {
    const today = todayStr();
    return Store.referralChallenges().filter((c) => {
      if (c.enabled === false) return false;
      if (!c.finalized) return (!c.start || c.start <= today) && (!c.end || c.end >= today);
      return (c.winners || []).some((w) => w.studentId === sid);
    });
  };
  Store.referralStandings = function (challenge) {
    const gid = challenge && challenge.groupId || null;
    return Store.leaderboard(gid).slice().sort((a, b) => b.score - a.score || a.s.name.localeCompare(b.s.name)).map((e, i) => ({ ...e, rank: i + 1 }));
  };
  Store.referralCodeFor = function (student, rank, discount, challenge) {
    const initials = String(student && student.name || 'ACP').replace(/[^A-Za-z ]/g, '').split(/\s+/).filter(Boolean).map((x) => x[0]).join('').slice(0, 3).toUpperCase() || 'ACP';
    const d = new Date((challenge && challenge.end) || todayStr());
    const months = ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'];
    const stamp = isNaN(d.getTime()) ? todayStr().replace(/-/g, '').slice(2, 6).toUpperCase() : `${months[d.getUTCMonth()]}${String(d.getUTCFullYear()).slice(2)}`;
    return `${initials}-${Math.round(discount || 0)}-${stamp}${rank > 1 ? '-R' + rank : ''}`;
  };
  Store.finalizeReferralChallenge = function (id) {
    const c = Store.referralChallenges().find((x) => x.id === id); if (!c) return { ok: false, error: 'Challenge not found' };
    const top = Store.referralStandings(c).slice(0, 3);
    c.finalized = true; c.finalizedAt = new Date().toISOString();
    c.winners = top.map((e, i) => ({ studentId: e.s.id, name: e.s.name, rank: i + 1, score: e.score, discount: (c.discounts || [30, 20, 10])[i] || 0, code: Store.referralCodeFor(e.s, i + 1, (c.discounts || [30, 20, 10])[i] || 0, c), used: false }));
    Store.save(); return { ok: true, challenge: c };
  };
  Store.saveReferralChallenges = function (list) { db.settings.referralChallenges = Array.isArray(list) ? list : []; Store.save(); };
  ...
  /* ---------------- activity ---------------- */
  Store.log = function (type, text, at) {
    const stamp = at ? new Date(typeof at === 'string' && at.length === 10 ? at + 'T12:00:00' : at) : new Date();
    db.activity.unshift({ id: uid('ev'), t: isNaN(stamp) ? new Date().toISOString() : stamp.toISOString(), type, text });
  ...
  Store.solvesOf = function (sid) {
    ...
    if (!s) return [];
    return s.solves
      .map((sv) => ({ problem: Store.problem(sv.problemId), date: sv.date, via: sv.via || null, subId: sv.subId || null, pts: sv.pts != null ? Number(sv.pts) : null }))
      .filter((x) => x.problem)
      .sort((a, b) => a.date.localeCompare(b.date));
  };
  Store.solvePoints = (sid) => Store.solvesOf(sid).reduce((a, x) => a + (x.pts != null ? x.pts : (x.problem.score || 0)), 0);

  /* Coach-configured score bonuses (all default 0 = off).
     A weekly streak rewards every qualifying week after the first:
     weeks 1+2 = one reward, week 3 = another, week 4 = another.
     A missing week starts a new run. Rewards are derived from history, so
     an already-earned bonus does not disappear when a later streak breaks. */
  Store.streakBonusCount = function (sid) {
    const start = Store.newScoringStart ? Store.newScoringStart() : '2026-09-06';
    const weeks = Array.from(new Set(Store.solvesOf(sid).filter((x) => x.date >= start).map((x) => mondayOf(x.date)))).sort();
    let previous = null, run = 0, rewards = 0;
    weeks.forEach((week) => {
      const adjacent = previous && (Date.parse(week + 'T12:00:00') - Date.parse(previous + 'T12:00:00')) === 7 * 864e5;
  ...
  function masteryBonusEventsForStudent(sid) {
    ...
    Store.solvesOf(sid).forEach((x) => {
      const topic = topicByName.get(x.problem.topic);
      if (!topic) return;
      const required = Number(topic.masteryXP) || 0;
      if (required <= 0 || awarded.has(topic.name)) return;
      const before = xp[topic.name] || 0;
      const after = before + (Number(x.problem.xp) || 0);
      xp[topic.name] = after;
      if (before < required && after >= required) {
        awarded.add(topic.name);
        events.push({ date: x.date, topic: topic.name, required, xp: after });
      }
    });
    return events;
  }
  Store.masteryBonusEvents = masteryBonusEventsForStudent;
  function dailyBonusEventsForStudent(sid) {
    const start = Store.newScoringStart ? Store.newScoringStart() : '2026-09-06';
    const perDay = {};
    Store.solvesOf(sid).filter((x) => String(x.date || '') >= start).forEach((x) => {
      const d = String(x.date || todayStr()).slice(0, 10);
      (perDay[d] ||= []).push(x);
    });
    const events = [];
    const tiers = Store.dailyBonusTiers();
    Object.entries(perDay).forEach(([date, list]) => {
      tiers.forEach((t) => {
        if (list.length >= t.count) events.push({ date, count: t.count, solved: list.length, points: t.points });
      });
    });
    return events.sort((a, b) => a.date.localeCompare(b.date) || a.count - b.count);
  }
  Store.dailyBonusEvents = dailyBonusEventsForStudent;

  function adjustedLegacyBonus(sid) {
    const S = db.settings;
    const snap = S.legacyBonusSnapshot && S.legacyBonusSnapshot.values || {};
    let legacy = Math.max(0, Number(snap[sid]) || 0);
    const mastery = Math.max(0, Number(S.bonusTopicMastery) || 0);
    if (legacy && mastery) {
      const start = Store.newScoringStart ? Store.newScoringStart() : '2026-09-06';
      const oldMasteryCount = masteryBonusEventsForStudent(sid).filter((e) => String(e.date || '') < start).length;
      legacy = Math.max(0, legacy - oldMasteryCount * mastery);
    }
    return legacy;
  }
  Store.adjustedLegacyBonus = adjustedLegacyBonus;

  Store.bonusPoints = function (sid) {
    const S = db.settings;
    let bonus = adjustedLegacyBonus(sid);
    if ((S.bonusWeeklyStreak | 0) > 0) bonus += Store.streakBonusCount(sid) * S.bonusWeeklyStreak;
    dailyBonusEventsForStudent(sid).forEach((e) => { bonus += e.points; });
    if ((S.bonusTopicMastery | 0) > 0) bonus += masteryBonusEventsForStudent(sid).length * S.bonusTopicMastery;
    return bonus;
  };
  ...
  Store.score = (sid) => Math.max(0, Store.solvePoints(sid) + Store.contestPointsSum(sid) + Store.bonusPoints(sid) + Store.missionBonusPoints(sid) + Store.adjustOf(sid).score);
  ...
  // A readable, derived ledger for the student's Score Improvement panel.
  // It intentionally includes every source used by Store.score().
  Store.scoreEventsOf = function (sid) {
    const s = Store.student(sid);
    if (!s) return [];
    const S = db.settings;
    const events = [];
    const add = (date, points, type, label, detail, icon) => {
      const n = Number(points) || 0;
      if (!n) return;
      events.push({ date: String(date || todayStr()).slice(0, 10), points: n, type, label, detail: detail || '', icon: icon || 'trend' });
    };
    const solves = Store.solvesOf(sid);
    const start = Store.newScoringStart ? Store.newScoringStart() : '2026-09-06';
    const bonusSolves = solves.filter((x) => x.date >= start);
    const legacyBonus = adjustedLegacyBonus(sid);
    if (legacyBonus) add(start, legacyBonus, 'legacy-bonus', 'Previous bonus score', 'Non-mastery bonuses earned before the new scoring rules started', 'star');
    solves.forEach((x) => add(x.date, x.pts != null ? x.pts : x.problem.score, 'solve', `Solved “${x.problem.name}”`, x.via ? 'Verified submission' : `${x.problem.difficulty} · ${x.problem.topic}`, 'check'));
    Store.resultsOf(sid).forEach((r) => add(r.contest.date, r.points, 'contest', `${r.contest.name} · rank #${r.rank}`, `${r.solved} solved`, 'trophy'));

    // One reward is generated for each week after the first in every
    // consecutive run. This matches the scoring calculation exactly.
    if ((S.bonusWeeklyStreak | 0) > 0) {
      const byWeek = {};
      bonusSolves.forEach((x) => { (byWeek[mondayOf(x.date)] ||= []).push(x); });
      ...
      weeks.forEach((week) => {
        ...
        run = adjacent ? run + 1 : 1;
        if (run >= 2) {
          const sortedWeek = byWeek[week].slice().sort((a, b) => a.date.localeCompare(b.date));
          const last = sortedWeek[sortedWeek.length - 1];
          add(last.date, S.bonusWeeklyStreak, 'streak', `${run}-week solve streak`, 'Weekly streak bonus', 'flame');
        }
        previous = week;
      });
    }

    dailyBonusEventsForStudent(sid).forEach((e) => {
      add(e.date, e.points, 'habit', `${e.count}-solve daily bonus`, `${e.solved} problems solved that day`, 'zap');
    });

    if ((S.bonusTopicMastery | 0) > 0) {
      masteryBonusEventsForStudent(sid).forEach((e) => add(e.date, S.bonusTopicMastery, 'mastery', `Mastered ${e.topic}`, 'Topic mastery bonus', 'star'));
    }

    Store.missionsFor(sid).forEach((m) => { const st = Store.missionStatus(m); if (st.complete) add(st.completedAt || m.deadline, m.reward || 0, 'mission', `Mission completed: ${m.title || 'Weekly Mission'}`, `${st.total} problems solved before deadline`, 'flag'); });

    const adj = Store.adjustOf(sid);
    const adjustmentPoints = adj.score;
    if (adjustmentPoints) add(s.adjustedAt || todayStr(), adjustmentPoints, 'adjustment', 'Coach score adjustment', 'Manual coach adjustment', 'settings');
    return events.sort((a, b) => b.date.localeCompare(a.date) || b.points - a.points);
  };
  ...
  Store.updateCoach = function (patch) {
    db.settings.coach = Object.assign(
      { name: 'Coach', title: '', bio: '', experience: '', photo: null, achievements: [] },
  ...
  // rank skips the tied positions (1, 1, 3 or 1, 2, 2, 4).
  Store.ranking = function (groupId) {
    ...
    return list.map((entry) => ({ ...entry, rank: 1 + list.filter((x) => x.score > entry.score).length }));
  };
  ...
  Store.leaderboard = function (groupId) {
    return Store.ranking(groupId).map((x) => ({ s: x.s, score: x.score, solved: x.solved, rank: x.rank, level: Store.levelOf(x.s.id) }));
  };
  ...
  Store.addStudent = function (data) {
    ...
    Store.log('student', `${s.name} joined the academy${g ? ' (' + g.name + ')' : ''}`, s.joinDate);
    Store.save();
    return s;
  };
  ...
  Store.updateProblem = function (id, data) {
    ...
    if (data.certificate != null) p.certificate = !!data.certificate;
  ...
  Store.setSolve = function (sid, pid, on, date) {
    ...
    if (on && !has) {
      ...
      Store.log('solve', `${s.name} solved “${p.name}” (+${p.score} pts)`, date || todayStr());
    } else if (!on && has) {
  ...
  Store.addContest = function (data) {
    ...
    Store.log('contest', `Contest “${c.name}” — won by ${first ? first.name : '—'}`, c.date);
    Store.save();
    return c;
  };
  ...
  /* ----- parent mode / attendance / trial ----- */
  Store.makeParentCode = function (sid) {
    const alphabet = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
    let code = '';
    do { code = 'P-' + Array.from({ length: 6 }, () => alphabet[Math.floor(Math.random() * alphabet.length)]).join(''); }
    while (db.students.some((s) => s.id !== sid && String(s.parentCode || '').toUpperCase() === code));
    return code;
  };
  Store.parentCodeOf = function (sid) { const st = Store.student(sid); if (!st) return ''; if (!st.parentCode) { st.parentCode = Store.makeParentCode(sid); Store.save(); } return st.parentCode; };
  Store.regenerateParentCode = function (sid) { const st = Store.student(sid); if (!st) return { ok: false, error: 'Student not found' }; st.parentCode = Store.makeParentCode(sid); Store.log('student', `Parent code generated for ${st.name}`); Store.save(); return { ok: true, code: st.parentCode }; };
  Store.parentLogin = function (code) { const c = String(code || '').trim().toUpperCase(); const st = db.students.find((s) => String(s.parentCode || '').toUpperCase() === c); return st ? { ok: true, studentId: st.id } : { ok: false, error: 'Wrong parent code' }; };
  Store.attendanceSessions = () => db.attendanceSessions || [];
  Store.addAttendanceSession = function (payload) { const groupId = payload.groupId || ''; if (!groupId) return { ok: false, error: 'Choose a group' }; const members = Store.students().filter((s) => s.groupId === groupId); const marks = {}; members.forEach((s) => { marks[s.id] = 'present'; }); db.attendanceSessions.push({ id: uid('att'), groupId, date: payload.date || todayStr(), title: String(payload.title || 'Course session').slice(0, 80), marks, createdAt: new Date().toISOString() }); Store.save(); return { ok: true }; };
  Store.updateAttendance = function (sessionId, marks) { const ses = Store.attendanceSessions().find((x) => x.id === sessionId); if (!ses) return; ses.marks = { ...(ses.marks || {}), ...(marks || {}) }; Store.save(); };
  Store.deleteAttendanceSession = function (id) { db.attendanceSessions = Store.attendanceSessions().filter((x) => x.id !== id); Store.save(); };
  Store.attendanceFor = function (sid) { return Store.attendanceSessions().filter((ses) => ses.marks && ses.marks[sid]).sort((a, b) => (b.date || '').localeCompare(a.date || '')); };
  Store.trialRequests = () => db.trialRequests || [];
  Store.addTrialRequest = function (req) { const clean = (v, n) => String(v || '').trim().slice(0, n); const r = { id: uid('trial'), name: clean(req.name, 60), age: clean(req.age, 8), email: clean(req.email, 90), phone: clean(req.phone, 40), status: 'new', createdAt: new Date().toISOString() }; if (!r.name || !r.email || !r.age) return { ok: false, error: 'Name, age and email are required' }; db.trialRequests.unshift(r); Store.log('trial', `Free trial request from ${r.name}`); Store.save(); return { ok: true }; };
  Store.setTrialStatus = function (id, status) { const r = Store.trialRequests().find((x) => x.id === id); if (r) { r.status = status; Store.save(); } };

  /* ----- missions ----- */
  Store.missions = () => db.missions || [];
  Store.missionsFor = (sid) => Store.missions().filter((m) => m.studentId === sid).sort((a, b) => (b.createdAt || '').localeCompare(a.createdAt || ''));
  Store.missionStatus = function (m) {
    const solved = new Set(Store.solvesOf(m.studentId).map((x) => x.problem.id));
    const dates = Object.fromEntries(Store.solvesOf(m.studentId).map((x) => [x.problem.id, x.date]));
    const ids = Array.isArray(m.problemIds) ? m.problemIds : [];
    const done = ids.filter((id) => solved.has(id));
    const missing = ids.filter((id) => !solved.has(id));
    const completedAt = missing.length ? null : ids.map((id) => dates[id]).sort().pop();
    const onTime = completedAt && (!m.deadline || completedAt <= m.deadline);
    return { total: ids.length, done: done.length, missing: missing.length, complete: !!onTime, completedAt, expired: !onTime && m.deadline && todayStr() > m.deadline };
  };
  Store.missionBonusPoints = (sid) => Store.missionsFor(sid).reduce((sum, m) => sum + (Store.missionStatus(m).complete ? (Number(m.reward) || 0) : 0), 0);
  Store.addMission = function (payload) {
    const studentId = String(payload.studentId || '');
    const problemIds = (payload.problemIds || []).filter(Boolean);
    if (!Store.student(studentId)) return { ok: false, error: 'Choose a student' };
    if (!payload.deadline) return { ok: false, error: 'Deadline is required' };
    if (!problemIds.length) return { ok: false, error: 'Choose at least one problem' };
    db.missions.push({ id: uid('mission'), studentId, title: String(payload.title || 'Weekly Mission').slice(0, 80), problemIds, deadline: payload.deadline, reward: Math.max(0, Number(payload.reward) || 0), createdAt: new Date().toISOString() });
    Store.log('mission', `Mission assigned to ${(Store.student(studentId) || {}).name || 'student'}`);
    Store.save(); return { ok: true };
  };
  Store.deleteMission = function (id) { db.missions = Store.missions().filter((m) => m.id !== id); Store.save(); };
...
`create table public.join_requests (
  ...
  level text, description text, availability jsonb not null default '{}'::jsonb, status text not null default 'new'
);
alter table public.join_requests add column if not exists availability jsonb not null default '{}'::jsonb;
...
create policy "coach deletes" on public.join_requests for delete using (auth.role() = 'authenticated');`;
  ...
  Store.applyVerifiedSolves = function (rows) {
    ...
    (rows || []).forEach((r) => {
      ...
      s.solves.push({
        ...
        pts: r.points != null ? Number(r.points) : null, // hint-discounted by the server
      });
      Store.log('solve', `${s.name} solved “${p.name}” — verified on Codeforces ✓ (+${p.score} pts)`, String(r.created_at || '').slice(0, 10) || todayStr());
      added++;
    });
  ...
  Store._cfCall = async function (body, needsCoach) {
    ...
    if (!cfg || !client) return { ok: false, error: 'offline' };
  ...
  Store.cfClaim = (code, problem, token) => Store._cfCall({ action: 'claim', code: code || '', problem, token: token || '' }, false);
  ...
  Store.cfActivityStatus = () => Store._cfCall({ action: 'activity-status' }, true);
  Store.cfLastVisits = async function () {
    const handles = Array.from(new Set(Store.students().map((s) => String(s.cfHandle || '').trim()).filter(Boolean)));
    const out = {};
    if (!handles.length) return { ok: true, visits: out };
    try {
      for (let i = 0; i < handles.length; i += 100) {
        const chunk = handles.slice(i, i + 100);
        const url = 'https://codeforces.com/api/user.info?handles=' + encodeURIComponent(chunk.join(';'));
        const res = await fetch(url, { cache: 'no-store' });
        const data = await res.json().catch(() => ({}));
        if (!res.ok || data.status !== 'OK') throw new Error(data.comment || 'Codeforces request failed');
        (data.result || []).forEach((u) => {
          if (u && u.handle) out[String(u.handle).toLowerCase()] = {
            handle: u.handle,
            lastOnlineTimeSeconds: u.lastOnlineTimeSeconds || null,
            rating: u.rating || null,
            rank: u.rank || null,
          };
        });
      }
      return { ok: true, visits: out };
    } catch (e) {
  ...
  /* ================================================================
     SEASONS (v1.7) — sprint competitions with a Champion Wall (top 2)
     ================================================================ */
  Store.seasons = () => db.seasons || [];
  Store.addSeason = function (data) {
    const s = { id: uid('sn'), name: String(data.name || '').trim(), start: data.start, end: data.end, groupIds: Array.isArray(data.groupIds) ? data.groupIds.slice() : [], awards: null, champions: null };
    db.seasons = db.seasons || [];
    db.seasons.push(s);
    Store.log('season', `Season “${s.name}” created`);
    Store.save();
    return s;
  };
  Store.deleteSeason = function (id) {
    db.seasons = (db.seasons || []).filter((s) => s.id !== id);
    if (db.settings.activeSeasonId === id) db.settings.activeSeasonId = null;
    Store.save();
  };
  Store.activeSeason = () => (db.seasons || []).find((s) => s.id === db.settings.activeSeasonId) || null;
  Store.setActiveSeason = function (id) { Store.updateSettings({ activeSeasonId: id || null }); };
  // Membership is intentionally live: the selected group filter is evaluated
  // from the student's current group whenever the season is viewed.
  Store.seasonMembers = function (season) {
    const groups = Array.isArray(season && season.groupIds) ? season.groupIds : [];
    return Store.students().filter((s) => !groups.length || groups.includes(s.groupId));
  };

  // points earned by a student strictly inside [start, end] (all score events)
  Store.seasonScore = function (sid, season) {
    if (!Store.seasonMembers(season).some((s) => s.id === sid)) return 0;
    const inWin = (d) => d >= season.start && d <= (season.end || '9999-12-31');
    return Store.scoreEventsOf(sid).filter((e) => inWin(e.date)).reduce((sum, e) => sum + e.points, 0);
  };
  Store.seasonStandings = function (season) {
    const list = Store.seasonMembers(season)
      .map((s) => ({ s, points: Store.seasonScore(s.id, season) }))
      .sort((a, b) => b.points - a.points || a.s.name.localeCompare(b.s.name));
    return list.map((entry) => ({ ...entry, rank: 1 + list.filter((x) => x.points > entry.points).length }));
  };
  // Freeze the Top 2 forever when the coach ends a season
  Store.endSeason = function (id) {
    const s = (db.seasons || []).find((x) => x.id === id);
    if (!s) return;
    if (!s.end) s.end = todayStr();
    if (!s.champions) {
      const standings = Store.seasonStandings(s);
      s.awards = standings.slice(0, 3).map((e) => ({ rank: e.rank, studentId: e.s.id, name: e.s.name, points: e.points }));
      s.champions = s.awards.slice(0, 2);
      Store.log('season', `Season “${s.name}” ended — champion: ${s.champions[0] ? s.champions[0].name : '—'} 🏆`);
    }
    if (db.settings.activeSeasonId === id) db.settings.activeSeasonId = null;
    Store.save();
  };
  ...
  /* ================================================================ CERTIFICATES */
  Store.certificatesOf = function (sid) {
    const s = Store.student(sid); if (!s) return [];
    const out = [];
    const tierForRank = (rank) => Number(rank) === 1 ? 'gold' : Number(rank) === 2 ? 'silver' : 'bronze';
    Store.solvesOf(sid).forEach((solve) => {
      if (solve.problem && solve.problem.certificate) {
        const points = solve.pts != null ? solve.pts : solve.problem.score;
        out.push({ id: `problem_${sid}_${solve.problem.id}`, type: 'problem', tier: 'green', title: 'Problem Mastered', detail: `Solved ${solve.problem.name} · ${String(solve.problem.difficulty || '').toUpperCase()} problem · ${points} points`, date: solve.date, subtitle: 'Problem Mastery', points });
      }
    });
    Store.resultsOf(sid).forEach((result) => {
      if (result.rank <= 3) out.push({ id: `contest_${sid}_${result.contest.id}`, type: 'contest', tier: tierForRank(result.rank), title: `Rank #${result.rank} in the contest`, detail: `${result.contest.name} · ${result.solved} problems solved · ${result.points} points`, date: result.contest.date, subtitle: result.contest.name, points: result.points });
    });
    (db.seasons || []).filter((season) => Array.isArray(season.awards) || Array.isArray(season.champions)).forEach((season) => {
      const awards = Array.isArray(season.awards) && season.awards.length ? season.awards : (season.champions || []);
      const award = awards.find((x) => x.studentId === sid);
      // Recalculate from points so old seasons cannot label tied winners as
      // Winner / Runner-up merely because they were saved in array order.
      const rank = award ? 1 + awards.filter((x) => Number(x.points) > Number(award.points)).length : null;
      if (award && rank <= 3) out.push({ id: `season_${sid}_${season.id}`, type: 'season', tier: tierForRank(rank), title: rank === 1 ? 'Winner' : rank === 2 ? 'Runner-up' : 'Third place', detail: `${season.name} · Season score: ${award.points} points`, date: season.end || season.start, subtitle: season.name, points: award.points });
    });

    // Level certificates are automatic. Walk the historical event dates and
    // record the first date on which each level's score/topic/achievement
    // requirements were satisfied.
    const levels = db.settings.levels || [];
    const scoreEvents = Store.scoreEventsOf(sid).slice().sort((a, b) => a.date.localeCompare(b.date));
    const dates = Array.from(new Set([
      s.joinDate,
      ...scoreEvents.map((e) => e.date),
      ...Store.solvesOf(sid).map((x) => x.date),
      ...s.achievements.map((a) => a.date),
    ].filter(Boolean))).sort();
    const reached = {}; let runningScore = 0;
    dates.forEach((date) => {
      scoreEvents.filter((e) => e.date === date).forEach((e) => { runningScore += e.points; });
      const topicXP = {};
      Store.solvesOf(sid).filter((x) => x.date <= date).forEach((x) => { topicXP[x.problem.topic] = (topicXP[x.problem.topic] || 0) + (x.problem.xp || 0); });
      const mastered = db.settings.topics.filter((t) => (topicXP[t.name] || 0) >= t.masteryXP).length;
      const achievementsAtDate = s.achievements.filter((a) => String(a.date || '') <= date).length;
      levels.forEach((level, index) => {
        if (index === 0 || reached[index]) return;
        if (runningScore >= (level.minScore || 0) && mastered >= (level.minTopics || 0) && achievementsAtDate >= (level.minAchievements || 0)) reached[index] = { date, score: Math.max(0, runningScore) };
      });
    });
    const currentLevel = Store.levelOf(sid).idx;
    for (let index = 1; index <= currentLevel; index++) {
      const level = levels[index]; const reachedAt = reached[index] || { date: s.joinDate, score: Store.score(sid) };
      out.push({ id: `level_${sid}_${index}`, type: 'level', tier: 'green', accent: level.color, title: `New level reached: ${level.name}`, detail: `Score: ${Math.round(reachedAt.score)} points`, date: reachedAt.date, subtitle: level.name, points: reachedAt.score });
    }
    return out.sort((a, b) => String(b.date || '').localeCompare(String(a.date || '')));
  };

  /* ================================================================
     STUDENT PASSPORT (v1.7) — one server-verified code unlocks everything
     student-side. The server mints a 7-day session token; the browser
     never stores the secret code itself.
     ================================================================ */
  const PASSPORT_KEY = 'acp_passport';
  Store.passport = () => { try { return JSON.parse(sessionStorage.getItem(PASSPORT_KEY) || 'null'); } catch (e) { return null; } };
  Store.passportLogin = async function (code) {
    const r = await Store._cfCall({ action: 'passport', code }, false);
    if (r.ok) {
      const sess = { id: r.data.studentId, name: r.data.studentName, token: r.data.token };
      try { sessionStorage.setItem(PASSPORT_KEY, JSON.stringify(sess)); } catch (e) {}
      Store._passportCache = sess;
    }
    return r;
  };
  Store.passportLogout = function () { try { sessionStorage.removeItem(PASSPORT_KEY); sessionStorage.removeItem('acp_me'); } catch (e) {} };
  Store._photoCall = async function (body) {
    const cfg = cloudConfig();
    if (!cfg) return { ok: false, error: 'offline' };
    try {
      const res = await fetch(cfg.url + '/functions/v1/student-photo', { method: 'POST', headers: { 'Content-Type': 'application/json', apikey: cfg.anonKey, Authorization: 'Bearer ' + cfg.anonKey }, body: JSON.stringify(body || {}) });
      const data = await res.json().catch(() => ({}));
      return { ok: data && data.ok === true, data, error: data && data.error };
    } catch (e) { return { ok: false, error: 'offline' }; }
  };
  Store.updateStudentPhoto = async function (photo) {
    const p = Store.passport();
    if (!p || !photo) return { ok: false, error: 'no-session' };
    const r = await Store._photoCall({ token: p.token, photo });
    if (r.ok) { const s = Store.student(p.id); if (s) { s.photo = photo; Store.save(); } }
    return r;
  };
  Store._scheduleCall = async function (body) {
    const cfg = cloudConfig();
    if (!cfg) return { ok: false, error: 'offline' };
    try {
      const res = await fetch(cfg.url + '/functions/v1/student-schedule', { method: 'POST', headers: { 'Content-Type': 'application/json', apikey: cfg.anonKey, Authorization: 'Bearer ' + cfg.anonKey }, body: JSON.stringify(body || {}) });
      const data = await res.json().catch(() => ({}));
      return { ok: data && data.ok === true, data, availability: data && data.availability, updated: data && data.updated, error: data && data.error };
    } catch (e) { return { ok: false, error: 'offline' }; }
  };

  /* ---- Code Clinic (buggy code → coach inbox) ---- */
  Store.clinicSend = (payload) => { const p = Store.passport(); return Store._cfCall({ action: 'clinic-send', token: p && p.token, ...payload }, false); };
  Store.clinicMine = () => { const p = Store.passport(); return Store._cfCall({ action: 'clinic-list', token: p && p.token }, false); };
  // coach side (RLS: authenticated only)
  Store.clinicCases = async function () {
    const client = cloudClient();
    if (!client || !Cloud.authed) return { ok: false };
    const res = await client.from('clinic_cases').select('*').order('created_at', { ascending: false }).limit(200);
    return res.error ? { ok: false, error: res.error.message } : { ok: true, rows: res.data || [] };
  };
  Store.clinicReply = async function (id, reply) {
    const client = cloudClient();
    const res = await client.from('clinic_cases').update({ coach_reply: reply, status: 'answered', answered_at: new Date().toISOString() }).eq('id', id);
    return res.error ? { ok: false, error: res.error.message } : { ok: true };
  };
  Store.clinicDelete = async function (id) {
    const client = cloudClient();
    const res = await client.from('clinic_cases').delete().eq('id', id);
    return res.error ? { ok: false, error: res.error.message } : { ok: true };
  };

  /* ---- Hints (student buys score discounts; text lives server-side only) ---- */
  Store.hintsMine = () => { const p = Store.passport(); return Store._cfCall({ action: 'hint-status', token: p && p.token }, false); };
  Store.hintUnlock = (problemId, idx) => { const p = Store.passport(); return Store._cfCall({ action: 'hint-unlock', token: p && p.token, problemId, idx }, false); };
  // coach side: edit hints per problem (auth RLS only; anonymous can't read them)
  Store.hintsOf = async function (problemId) {
    const client = cloudClient();
    if (!client || !Cloud.authed) return { ok: false, rows: [] };
    const res = await client.from('problem_hints').select('idx,text,cut').eq('problem_id', problemId).order('idx');
    return res.error ? { ok: false, rows: [] } : { ok: true, rows: res.data || [] };
  };
  Store.saveHints = async function (problemId, hints) {
    const client = cloudClient();
    if (!client || !Cloud.authed) return { ok: false, error: 'not signed in' };
    await client.from('problem_hints').delete().eq('problem_id', problemId);
    const rows = hints.filter((h) => h.text && h.text.trim()).map((h, i) => ({ problem_id: problemId, idx: i, text: h.text.trim().slice(0, 600), cut: h.cut == null || h.cut === '' ? null : Math.max(0, Math.trunc(+h.cut || 0)) }));
    if (!rows.length) return { ok: true };
    const res = await client.from('problem_hints').insert(rows);
    return res.error ? { ok: false, error: res.error.message } : { ok: true };
  };
  ...
  Store.weekdays = () => WEEKDAYS.slice();
  const cleanAvailability = (raw) => {
    const out = {};
    WEEKDAYS.forEach((day) => {
      const ranges = raw && Array.isArray(raw[day.key]) ? raw[day.key] : [];
      out[day.key] = ranges.map((r) => ({ start: String(r.start || ''), end: String(r.end || '') }))
        .filter((r) => /^([01]\d|2[0-2]):([0-5]\d)$/.test(r.start) && /^([01]\d|2[0-2]):([0-5]\d)$/.test(r.end) && r.start < r.end)
        .slice(0, 6);
    });
    return out;
  };
  Store.cleanAvailability = cleanAvailability;
  ...
  Store.addJoinRequest = async function (req) {
    ...
    const jr = {
      ...
      level: clean(req.level, 120), description: clean(req.description, 600), referralCode: clean(req.referralCode, 40).toUpperCase(),
      availability: cleanAvailability(req.availability), status: 'new', createdAt: new Date().toISOString(),
    };
    ...
    if (client) { // anonymous insert allowed by RLS policy — no auth needed
      try {
        const res = await client.from('join_requests').insert({
          name: jr.name, age: jr.age, email: jr.email, level: jr.level, description: jr.description, referral_code: jr.referralCode || null, availability: jr.availability, status: 'new',
        });
        if (res.error && /referral_code|column/i.test(String(res.error.message || ''))) {
          const retry = await client.from('join_requests').insert({ name: jr.name, age: jr.age, email: jr.email, level: jr.level, description: jr.description, availability: jr.availability, status: 'new' });
  ...
  Store.fetchJoinRequests = async function () {
    ...
    try {
      ...
      db.joinRequests = (res.data || []).map((r) => ({
        ...
        level: r.level || '', description: r.description || '', referralCode: r.referral_code || '', availability: cleanAvailability(r.availability || {}),
  ...
  /* ---------------- next-course interest requests ---------------- */
  const NEXT_COURSE_SQL_SNIPPET =
`create table if not exists public.next_course_requests (
  id bigint generated always as identity primary key,
  created_at timestamptz not null default now(),
  student_id text not null, student_name text not null, group_id text,
  course_ad_id text, course_level text, availability jsonb not null default '{}'::jsonb, status text not null default 'new'
);
alter table public.next_course_requests add column if not exists availability jsonb not null default '{}'::jsonb;
...
alter table public.next_course_requests enable row level security;
create policy "anyone can request next course" on public.next_course_requests for insert with check (true);
create policy "coach reads next course requests" on public.next_course_requests for select using (auth.role() = 'authenticated');
create policy "coach updates next course requests" on public.next_course_requests for update using (auth.role() = 'authenticated') with check (auth.role() = 'authenticated');
create policy "coach deletes next course requests" on public.next_course_requests for delete using (auth.role() = 'authenticated');`;
  Store.nextCourseSQL = () => NEXT_COURSE_SQL_SNIPPET;
  const hasNextCourseAvailability = (availability) => !!(availability && Object.values(availability).some((ranges) => Array.isArray(ranges) && ranges.some((range) => range && range.start && range.end && range.start < range.end)));
  Store.nextCourseRequests = () => (db.nextCourseRequests || []).filter((request) => hasNextCourseAvailability(request.availability));
  Store.nextCourseRequestFor = function (sid, adId, level) {
    const rows = Store.nextCourseRequests().filter((r) => r.studentId === sid && r.status !== 'deleted' && ((adId && r.courseAdId === adId) || (!r.courseAdId && r.courseLevel === level) || (!adId && r.courseLevel === level)));
    return rows.sort((a, b) => String(b.createdAt || '').localeCompare(String(a.createdAt || '')))[0] || null;
  };
  Store.newNextCourseCount = () => Store.nextCourseRequests().filter((r) => r.status === 'new').length;
  Store.nextCourseAdsList = function () {
    if (Array.isArray(db.settings.nextCourseAds) && db.settings.nextCourseAds.length) return db.settings.nextCourseAds;
    return Object.entries(db.settings.nextCourses || {}).map(([sourceGroupId, course]) => ({ id: 'legacy_' + sourceGroupId, ...course, sourceGroupId, visibleGroupIds: Array.isArray(course.visibleGroupIds) ? course.visibleGroupIds : [sourceGroupId] }));
  };
  Store.nextCourseAdsFor = function (sid) {
    const s = Store.student(sid);
    if (!s) return [];
    return Store.nextCourseAdsList().filter((course) => {
      if (!course || course.enabled === false) return false;
      const visible = Array.isArray(course.visibleGroupIds) ? course.visibleGroupIds : [];
      return visible.includes(s.groupId);
    });
  };
  Store.nextCourseFor = (sid) => Store.nextCourseAdsFor(sid)[0] || null;
  Store.saveNextCourseAds = (ads) => { db.settings.nextCourseAds = Array.isArray(ads) ? ads : []; Store.save(); };

  Store.messagesFor = function (sid) {
    const s = Store.student(sid); if (!s) return [];
    const today = todayStr();
    return (db.settings.coachMessages || []).filter((m) => m.enabled !== false && (!m.startDate || m.startDate <= today) && (!m.endDate || m.endDate >= today) && ((!Array.isArray(m.groupIds) || !m.groupIds.length) && (!Array.isArray(m.studentIds) || !m.studentIds.length) || (Array.isArray(m.groupIds) && m.groupIds.includes(s.groupId)) || (Array.isArray(m.studentIds) && m.studentIds.includes(s.id)))).sort((a, b) => String(b.createdAt || '').localeCompare(String(a.createdAt || '')));
  };
  Store.addCoachMessage = function (data) {
    const m = { id: uid('msg'), title: String(data.title || '').trim().slice(0, 100), body: String(data.body || '').trim().slice(0, 800), groupIds: Array.isArray(data.groupIds) ? data.groupIds.slice() : [], studentIds: Array.isArray(data.studentIds) ? data.studentIds.slice() : [], startDate: data.startDate || todayStr(), endDate: data.endDate || null, enabled: data.enabled !== false, createdAt: new Date().toISOString() };
    if (!m.title || !m.body) return { ok: false, error: 'Title and message are required.' };
    db.settings.coachMessages = db.settings.coachMessages || []; db.settings.coachMessages.unshift(m); Store.save(); return { ok: true, message: m };
  };
  Store.updateCoachMessage = function (id, data) { const m = (db.settings.coachMessages || []).find((x) => x.id === id); if (!m) return { ok: false, error: 'Message not found.' }; Object.assign(m, data); Store.save(); return { ok: true }; };
  Store.deleteCoachMessage = function (id) { db.settings.coachMessages = (db.settings.coachMessages || []).filter((m) => m.id !== id); Store.save(); };

  const cacheNextCourseRequest = (request) => {
    const key = (row) => `${row.studentId}::${row.courseAdId || row.courseLevel || ''}`;
    const current = db.nextCourseRequests || [];
    db.nextCourseRequests = current.filter((row) => key(row) !== key(request));
    db.nextCourseRequests.unshift(request);
  };
  Store.getNextCourseAvailability = async function (sid, course) {
    const local = Store.nextCourseRequestFor(sid, course && course.id, course && course.level);
    const p = Store.passport();
    if (p && p.token && course) {
      const r = await Store._scheduleCall({ action: 'get', token: p.token, courseAdId: course.id, courseLevel: course.level });
      if (r.ok) {
        const received = cleanAvailability(r.availability || {});
        // A read-only visit must never create an interest request. Cache only
        // an actual submitted availability response.
        if (hasNextCourseAvailability(received)) {
          const s = Store.student(sid);
          cacheNextCourseRequest({ ...(local || {}), studentId: sid, studentName: s && s.name || p.name || 'Student', groupId: s && s.groupId || null, courseAdId: course.id, courseLevel: course.level, availability: received, status: 'new', createdAt: new Date().toISOString() });
        }
        return { ok: true, availability: hasNextCourseAvailability(received) ? received : (local ? cleanAvailability(local.availability || {}) : {}) };
      }
    }
    return { ok: true, availability: local ? cleanAvailability(local.availability || {}) : {} };
  };
  Store.saveNextCourseAvailability = async function (sid, course, availability) {
    const s = Store.student(sid), clean = cleanAvailability(availability || {});
    if (!s || !course) return { ok: false, error: 'no-course' };
    const p = Store.passport();
    if (p && p.token) {
      const r = await Store._scheduleCall({ action: 'save', token: p.token, courseAdId: course.id, courseLevel: course.level, availability: clean });
      if (r.ok) {
        cacheNextCourseRequest({ ...(Store.nextCourseRequestFor(sid, course.id, course.level) || {}), studentId: sid, studentName: s.name, groupId: s.groupId, courseAdId: course.id, courseLevel: course.level, availability: clean, status: 'new', createdAt: new Date().toISOString() });
        Store.save();
        return { ok: true, where: 'cloud', updated: !!r.updated };
      }
      // If the new function is not deployed yet, keep the existing insert
      // path as a safe compatibility fallback.
    }
    const existing = Store.nextCourseRequestFor(sid, course.id, course.level);
    const req = { ...(existing || {}), id: existing && existing.id || uid('ncr'), studentId: s.id, studentName: s.name, groupId: s.groupId, courseAdId: course.id, courseLevel: course.level, availability: clean, status: 'new', createdAt: new Date().toISOString() };
    const client = cloudClient();
    if (client) {
      try {
        const payload = { student_id: req.studentId, student_name: req.studentName, group_id: req.groupId, course_ad_id: req.courseAdId, course_level: req.courseLevel, availability: req.availability, status: req.status };
        let res = await client.from('next_course_requests').insert(payload);
        if (res.error && /course_ad_id|column/i.test(String(res.error.message || ''))) { delete payload.course_ad_id; res = await client.from('next_course_requests').insert(payload); }
        if (res.error) throw res.error;
        cacheNextCourseRequest({ ...req, cloudId: res.data && res.data[0] && res.data[0].id || null }); Store.save();
        return { ok: true, where: 'cloud', updated: !!existing };
      } catch (e) { /* local fallback below */ }
    }
    cacheNextCourseRequest(req); Store.save();
    return { ok: true, where: 'local', updated: !!existing };
  };
  Store.addNextCourseRequest = async function (sid, availability) {
    const course = Store.nextCourseFor(sid);
    return course ? Store.saveNextCourseAvailability(sid, course, availability) : { ok: false, error: 'no-course' };
  };
  Store.fetchNextCourseRequests = async function () {
    const client = cloudClient();
    if (!client || !Cloud.authed) return { ok: false, reason: 'not-authed' };
    try {
      const res = await client.from('next_course_requests').select('*').order('created_at', { ascending: false }).limit(300);
      if (res.error) throw res.error;
      const localOnly = Store.nextCourseRequests().filter((x) => !x.cloudId);
      const remote = (res.data || []).map((r) => ({ cloudId: r.id, studentId: r.student_id, studentName: r.student_name, groupId: r.group_id, courseAdId: r.course_ad_id || null, courseLevel: r.course_level, availability: cleanAvailability(r.availability || {}), status: r.status || 'new', createdAt: r.created_at })).filter((request) => hasNextCourseAvailability(request.availability));
      const all = remote.concat(localOnly).filter((request) => hasNextCourseAvailability(request.availability)).sort((a, b) => String(b.createdAt || '').localeCompare(String(a.createdAt || '')));
      const latest = new Map();
      all.forEach((row) => { const k = `${row.studentId}::${row.courseAdId || row.courseLevel || ''}`; if (!latest.has(k)) latest.set(k, row); });
      db.nextCourseRequests = Array.from(latest.values());
      Store.save();
      return { ok: true };
    } catch (e) { return { ok: false, reason: 'fetch-failed', error: String(e && e.message || e) }; }
  };
  Store.setNextCourseStatus = async function (item, status) {
    if (item.cloudId && cloudClient() && Cloud.authed) {
      try { const res = await cloudClient().from('next_course_requests').update({ status }).eq('id', item.cloudId); if (res.error) throw res.error; } catch (e) {}
    }
    const local = Store.nextCourseRequests().find((x) => x === item || (item.cloudId && x.cloudId === item.cloudId) || (item.id && x.id === item.id));
    if (local) local.status = status;
    Store.save();
  };
  Store.deleteNextCourseRequest = async function (item) {
    if (item.cloudId && cloudClient() && Cloud.authed) {
      try { const res = await cloudClient().from('next_course_requests').delete().eq('id', item.cloudId); if (res.error) throw res.error; } catch (e) {}
    }
    db.nextCourseRequests = Store.nextCourseRequests().filter((x) => x !== item && !(item.cloudId && x.cloudId === item.cloudId) && !(item.id && x.id === item.id));
    Store.save();
  };
...
-- nobody (not even you in the browser) can read them — only the secure server function can.`;
  ...
  /* ---- Step 8b · v1.7 tables (Passport sessions, Clinic, Hints) ---- */
  const PASSPORT_SQL_SNIPPET =
`-- Step 8b · Student Passport / Clinic / Hints (safe to run multiple times)
...
alter table public.student_sessions add column if not exists created_at timestamptz not null default now();
alter table public.student_sessions add column if not exists last_seen_at timestamptz not null default now();
create table if not exists public.clinic_cases (
  id uuid primary key default gen_random_uuid(),
  student_id text not null, student_name text,
  problem_link text, symptom text not null default 'wa',
  code text not null, note text,
  status text not null default 'open',
  coach_reply text, answered_at timestamptz,
  created_at timestamptz not null default now()
);
create table if not exists public.problem_hints (
  problem_id text not null, idx int not null,
  text text not null, cut int,
  primary key (problem_id, idx)
);
create table if not exists public.hint_unlocks (
  student_id text not null, problem_id text not null, idx int not null,
  created_at timestamptz not null default now(),
  primary key (student_id, problem_id, idx)
);
...
-- coach can read/reply/delete clinic cases and edit hints; hint unlocks are server-only;
-- sessions & unlocks have NO policies = only the Edge Function (service key) touches them.
create policy "coach reads clinic" on public.clinic_cases for select using (auth.role() = 'authenticated');
create policy "coach writes clinic" on public.clinic_cases for update using (auth.role() = 'authenticated') with check (auth.role() = 'authenticated');
create policy "coach deletes clinic" on public.clinic_cases for delete using (auth.role() = 'authenticated');
create policy "coach reads hints" on public.problem_hints for select using (auth.role() = 'authenticated');
create policy "coach writes hints" on public.problem_hints for insert with check (auth.role() = 'authenticated');
create policy "coach updates hints" on public.problem_hints for update using (auth.role() = 'authenticated') with check (auth.role() = 'authenticated');
create policy "coach deletes hints" on public.problem_hints for delete using (auth.role() = 'authenticated');`;
  Store.passportSQL = () => PASSPORT_SQL_SNIPPET;
