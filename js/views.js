...
(function () {
  ...
  function wireSpace(root, p) {
    ...
    const renderRules = () => {
      ...
      const badge = (txt, bg = 'var(--accent)') => `<span class="chip" style="border-color:${bg}66;color:${bg};background:color-mix(in srgb,${bg} 10%,transparent)">${txt}</span>`;
      const challengeHTML = challenges.length ? challenges.map((c) => {
        ...
        const podium = discounts.map((d, i) => { const medals = ['🥇', '🥈', '🥉']; const colors = ['var(--gold)', 'var(--accent-2)', 'var(--accent)']; const labels = ['Champion', 'Runner-up', 'Third place']; return `<div style="flex:1;min-width:125px;text-align:center;border:1px solid color-mix(in srgb,${colors[i]} 55%,var(--border));background:linear-gradient(180deg,color-mix(in srgb,${colors[i]} 24%,var(--card)),var(--card));border-radius:18px;padding:14px 10px;box-shadow:0 12px 28px rgba(0,0,0,.12);transform:translateY(${i === 0 ? '-6px' : i === 1 ? '4px' : '10px'})"><div style="font-size:2rem;line-height:1">${medals[i]}</div><b style="display:block;margin-top:6px;color:${colors[i]}">Rank #${i + 1}</b><div style="font-size:1.65rem;font-weight:900;color:${colors[i]};margin:4px 0">${d}%</div><small class="muted">${labels[i]} discount</small></div>`; }).join('');
        return `<section class="card reveal in" style="position:relative;overflow:hidden;margin-bottom:16px;border-color:color-mix(in srgb,var(--gold) 45%,var(--border));background:linear-gradient(135deg,color-mix(in srgb,var(--gold) 13%,var(--card)),color-mix(in srgb,var(--accent) 7%,var(--card)))">
          <div style="position:absolute;right:-34px;top:-42px;font-size:9rem;opacity:.08">🏆</div>
          <div class="card-title" style="font-size:1.15rem;color:var(--gold)">${ic('crown')}${U.esc(c.title || 'Leaderboard Discount Challenge')}</div>
          <p class="muted" style="margin:8px 0 14px">Finish in the <b>Top 3</b> of this leaderboard and win a <b>Level 1 discount code</b> for one invited friend.</p>
          <div style="display:flex;gap:12px;align-items:flex-end;margin:12px 0 20px;flex-wrap:wrap">${podium}</div>
          <div class="score-summary-grid" style="grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px">
            <div class="stat-tile"><div class="st-ic">${ic('trophy')}</div><div class="st-val">${mine ? '#' + mine.rank : '—'}</div><div class="st-label">Your rank</div></div>
            <div class="stat-tile"><div class="st-ic">${ic('zap')}</div><div class="st-val">${mine ? U.fmtNum(mine.score) : '0'}</div><div class="st-label">Your score</div></div>
            <div class="stat-tile"><div class="st-ic">${ic('trend')}</div><div class="st-val">${gap ? U.fmtNum(gap) : (mine && mine.rank <= 3 ? 'Top 3' : '—')}</div><div class="st-label">Points from Rank #3</div></div>
          </div>
          <div style="display:flex;flex-wrap:wrap;gap:8px;margin:14px 0">${badge(c.groupId ? U.esc((Store.group(c.groupId) || {}).name || 'Selected group') : 'Global leaderboard', 'var(--accent)')}${badge(`${U.fmtDate(c.start)} → ${c.end ? U.fmtDate(c.end) : 'open'}`, 'var(--accent-2)')}${discounts.map((d, i) => badge(`#${i + 1}: ${d}%`, i === 0 ? 'var(--gold)' : 'var(--accent)')).join('')}</div>
          ${win ? `<div class="vf-result" style="background:color-mix(in srgb,var(--green) 13%,var(--card));border-color:color-mix(in srgb,var(--green) 55%,var(--border));margin-top:10px">${ic('check')}<span>You won! Your code is <b class="mono">${U.esc(win.code)}</b> — ${win.discount}% discount for one friend in Level 1. Status: ${win.used ? 'used' : 'valid until used'}.</span></div>` : ''}
        </section>`;
      }).join('') : `<section class="card reveal in" style="margin-bottom:16px;background:linear-gradient(135deg,color-mix(in srgb,var(--gold) 8%,var(--card)),var(--card))"><div class="card-title" style="color:var(--gold)">${ic('crown')}Leaderboard Discount Challenge</div><p class="muted tiny">No referral discount challenge is visible for you right now.</p></section>`;
      const rules = [
