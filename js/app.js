...
(function () {
  ...
  const TITLES = {
    ...
    leaderboard: 'Leaderboard', problems: 'Problem Library', contests: 'Hall of Fame', champions: 'Champion Wall',
    analytics: 'Analytics', achievements: 'Achievements', join: 'Join the Academy', trial: 'Free Session Trial', parent: 'Parent Mode', submit: 'Submit a Solve', space: 'My Space', seasons: 'Seasons', admin: 'Admin Panel',
  };
  ...
  function parseHash() {
    ...
    if ((mm = h.match(/^\/champions$/))) return { name: 'champions' };
    ...
    if ((mm = h.match(/^\/trial$/))) return { name: 'trial' };
    if ((mm = h.match(/^\/parent$/))) return { name: 'parent' };
  ...
  function route() {
    ...
    switch (r.name) {
      ...
      case 'champions': PublicViews.champions(root); break;
      ...
      case 'trial': PublicViews.trial(root); break;
      case 'parent': PublicViews.parent(root); break;
  ...
  function initFx() { /* Background rain disabled for the production theme. */ }
