...
(function () {
  ...
  function normalizeDB(data) {
    ...
    data.seasons.forEach((s) => { if (!Array.isArray(s.groupIds)) s.groupIds = []; });
...
create policy "coach deletes" on public.join_requests for delete using (auth.role() = 'authenticated');`;
  ...
  Store.addSeason = function (data) {
    const s = { id: uid('sn'), name: String(data.name || '').trim(), start: data.start, end: data.end, groupIds: Array.isArray(data.groupIds) ? data.groupIds.slice() : [], champions: null };
  ...
  Store.endSeason = function (id) {
    ...
    if (!s.champions) {
      s.champions = Store.seasonStandings(s).slice(0, 2).map((e) => ({ rank: e.rank, studentId: e.s.id, name: e.s.name, points: e.points }));
