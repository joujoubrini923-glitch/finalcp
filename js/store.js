...
create policy "coach deletes next course requests" on public.next_course_requests for delete using (auth.role() = 'authenticated');`;
  ...
  Store.nextCourseRequests = () => db.nextCourseRequests || [];
  ...
  Store.getNextCourseAvailability = async function (sid, course) {
    ...
    if (p && p.token && course) {
      ...
      if (r.ok) {
        const s = Store.student(sid);
        cacheNextCourseRequest({ ...(local || {}), studentId: sid, studentName: s && s.name || p.name || 'Student', groupId: s && s.groupId || null, courseAdId: course.id, courseLevel: course.level, availability: cleanAvailability(r.availability || {}), status: 'new', createdAt: new Date().toISOString() });
        return { ok: true, availability: cleanAvailability(r.availability || {}) };
      }
    }
  ...
  Store.fetchNextCourseRequests = async function () {
    ...
    try {
      ...
      const remote = (res.data || []).map((r) => ({ cloudId: r.id, studentId: r.student_id, studentName: r.student_name, groupId: r.group_id, courseAdId: r.course_ad_id || null, courseLevel: r.course_level, availability: cleanAvailability(r.availability || {}), status: r.status || 'new', createdAt: r.created_at }));
      const all = remote.concat(localOnly).sort((a, b) => String(b.createdAt || '').localeCompare(String(a.createdAt || '')));
