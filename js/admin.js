...
(function () {
  ...
  function loginView(root) {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (ok) {
        ...
        Store.isDefaultPassword().then((d) => { if (d) setTimeout(() => U.toast('⚠ You are still using the DEFAULT password — change it in Settings right now!', 'error', 'warn'), 800); });
        App.updateAdminLink();
  ...
  function injectPwWarning(el) {
    ...
    Promise.resolve(Store.isDefaultPassword()).then((isDefault) => {
      ...
      warn.innerHTML = `${ic('warn')}<div><b>Security alert: you are still using the default password “admin123”.</b><span>It is written in the public source code — anyone can try it. Change it right now, it takes 20 seconds.</span></div><a class="btn btn-primary btn-sm" href="#/admin/settings">${ic('key')}Change it now</a>`;
      el.prepend(warn);
    });
  }
  ...
  Admin.render = function (root, tab) {
    ...
    ({
      ...
      achievements: tabAchievements, requests: tabRequests, clinic: tabClinic, seasons: tabSeasons, verify: tabVerify, settings: tabSettings, data: tabData,
    })[tab](content);
  ...
  function tabSeasons(el) {
    ...
    el.innerHTML = `
      <div class="card reveal in">
        ...
        <p class="muted tiny" style="margin-top:6px">A season counts solves + contests inside its dates only. End a season to freeze its <b>Top 3</b> on the public Champions Wall (<a class="link-accent" href="#/seasons">#/seasons</a>). Only one season can be live.</p>
        <div class="field-row" style="margin-top:14px;align-items:flex-end">
          ...
          <button class="btn btn-primary btn-sm" id="sn-create">${ic('plus')}Create &amp; go live</button>
        </div>
      </div>
      <div class="card reveal in" style="margin-top:16px">
        ...
        <div class="table-wrap" style="margin-top:10px">${seasons.length ? `<table class="table"><thead><tr><th>Season</th><th>Window</th><th>Status</th><th>Champion</th><th></th></tr></thead><tbody>
          ${seasons.map((s) => `<tr>
            ...
            <td><div class="row-actions">
              ...
              ${!s.champions ? `<button class="icon-btn" data-sndel="${s.id}" style="color:var(--red)" title="Delete">${ic('trash')}</button>` : ''}
    ...
    U.$('#sn-create', el).onclick = () => {
      ...
      const s = Store.addSeason({ name, start, end: end || null });
    ...
    U.$$('[data-end]', el).forEach((b) => b.onclick = async () => {
      if (!(await U.confirm('End this season? The Top 3 will be frozen on the Champions Wall forever.', { title: 'End season', okLabel: 'Crown the champions 👑' }))) return;
      Store.endSeason(b.dataset.end);
      U.toast('Season ended — champions crowned 👑', 'gold', 'crown');
      rerender('seasons');
    });
    U.$$('[data-sndel]', el).forEach((b) => b.onclick = async () => {
      if (!(await U.confirm('Delete this season?', { title: 'Delete', danger: true, okLabel: 'Delete' }))) return;
    ...
    if (syncBtn && authed) syncBtn.onclick = async () => {
      ...
      U.$('#vv-sync-detail', el).innerHTML = un.length ? `<p class="muted tiny"><b>${un.length}</b> accepted submission${un.length === 1 ? '' : 's'} skipped because the problem is not in the library:</p>
        <div class="jr-list" style="margin-top:8px">${un.slice(0, 12).map((u) => `<div class="jr-item"><div class="jr-main">
          ...
          <a class="link-accent mono tiny" href="${U.esc(u.link || '#')}" target="_blank" rel="noopener">${U.esc(u.name || u.link || '')}</a>
  ...
  function tabSettings(el) {
    ...
    el.innerHTML = `
      ...
      <div class="card" style="margin-top:18px">
        ...
        <div class="field-row">
          <label class="field" style="margin:0"><span>Weekly perfect streak</span><input class="input" type="number" min="0" id="bn-streak" value="${S.bonusWeeklyStreak || 0}"></label>
      ...
      <div class="card" style="margin-top:18px" id="pw-card">
