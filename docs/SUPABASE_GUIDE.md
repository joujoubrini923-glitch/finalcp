...
## Step 8 — Codeforces Auto-Verify tables (optional but awesome)

This powers the **Submit a Solve** page (`#/submit`): students type a secret
code + a Codeforces problem link, and the solve is recorded only if Codeforces
confirms an **Accepted** submission by that exact student. Open **SQL Editor**,
paste (Admin → **Auto-Verify** tab has a copy button with exactly this), Run:
...
`student_codes` has **no policies on purpose**: nobody can read it — not even
you from the browser — only the secure server function from Step 9 can.

## Step 8b — v1.7 tables (Passport, Clinic, Hints)

Needed for Student Space, Code Clinic and Hints-for-points. SQL Editor → paste → Run (also copyable in Admin → Code Clinic → "Setup SQL"):

```sql
create table if not exists public.student_sessions (
  token_hash text primary key, student_id text not null, expires_at timestamptz not null
);
...
create table if not exists public.problem_hints (
  problem_id text not null, idx int not null, text text not null, cut int,
...
Then **re-deploy the `cf-verify` function** (Step 9) with the updated code — the new
actions (passport, clinic, hints) live inside it.

## Step 9 — Deploy the secure verifier (2 minutes, no coding)

The checker runs on Supabase's servers, so nobody can read or bypass it.

1. In the Supabase dashboard → **Edge Functions** (left sidebar) → **Create a new function** (or **Deploy a new function**).
2. Name it **exactly** `cf-verify`.
3. Replace everything in the code editor with the content of this file from the
   project: `supabase/functions/cf-verify/index.ts` → **Deploy**.
4. Leave the **JWT verification ON** (default) — the app already sends the right
   keys; the function checks coach permissions itself.
5. Done — try Admin → **Auto-Verify → Sync now**, or submit a test solve at `#/submit`.

That's it. The function uses your **service key server-side only** (injected by
Supabase automatically — you never paste it anywhere). Free tier includes
~500,000 function calls per month; an academy uses a few hundred.

> If students see *"Auto-verify server unreachable"*, the function isn't
> deployed yet or the name differs — it must be exactly `cf-verify`.
...
| Auto-Verify says the server is unreachable | The **cf-verify** Edge Function isn't deployed (or has a different name) — do Step 9. Also confirm the Step 8 SQL ran. |
| Claims say *Wrong secret code* for a fresh code | Codes are per-student and one-time-generated: regenerate it in Admin → Auto-Verify and share the NEW one. |
| Sync records nothing | Students need their **Codeforces username** set (edit student), the problems must exist in the Problem library with their codeforces.com link, and the submissions must be newer than the Activation date if one is set. |
