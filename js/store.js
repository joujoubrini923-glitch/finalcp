...
(function () {
  ...
  Store.bonusPoints = function (sid) {
    ...
    if ((S.bonusTopicMastery | 0) > 0) {
      const xp = {};
      bonusSolves.forEach((x) => { const topic = db.settings.topics.find((t) => t.name === x.problem.topic); if (topic) xp[topic.name] = (xp[topic.name] || 0) + (x.problem.xp || 0); });
      bonus += Object.entries(xp).filter(([name, val]) => { const topic = db.settings.topics.find((t) => t.name === name); return topic && val >= topic.masteryXP; }).length * S.bonusTopicMastery;
    }
    return bonus;
  };
  ...
  Store.scoreEventsOf = function (sid) {
    ...
    if ((S.bonusTopicMastery | 0) > 0) {
      const xp = {};
      bonusSolves.forEach((x) => {
        const topic = db.settings.topics.find((t) => t.name === x.problem.topic);
        if (!topic) return;
        const before = xp[topic.name] || 0;
        xp[topic.name] = before + (x.problem.xp || 0);
        if (before < topic.masteryXP && xp[topic.name] >= topic.masteryXP) add(x.date, S.bonusTopicMastery, 'mastery', `Mastered ${topic.name}`, 'Topic mastery bonus', 'star');
      });
    }
