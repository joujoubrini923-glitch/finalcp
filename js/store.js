...
(function () {
  ...
  function defaultSettings() {
    return {
      ...
      bonusMultiDay: 0,        // pts per day with 2+ solves
  ...
  Store.bonusPoints = function (sid) {
    ...
    if ((S.bonusMultiDay | 0) > 0) {
      const perDay = {};
      Store.solvesOf(sid).forEach((x) => { perDay[x.date] = (perDay[x.date] || 0) + 1; });
      Object.values(perDay).forEach((n) => { if (n >= 2) bonus += S.bonusMultiDay; });
    }
  ...
  Store.scoreEventsOf = function (sid) {
    ...
    if ((S.bonusMultiDay | 0) > 0) {
      ...
      Object.entries(perDay).forEach(([date, list]) => {
        if (list.length >= 2) add(date, S.bonusMultiDay, 'habit', 'Multi-solve day', `${list.length} problems solved`, 'zap');
      });
    }
    ...
    if (client) { // anonymous insert allowed by RLS policy — no auth needed
      try {
        const res = await client.from('join_requests').insert({
          name: jr.name, age: jr.age, email: jr.email, level: jr.level, description: jr.description, availability: jr.availability, status: 'new',
        });
        if (res.error) throw res.error;
        Store.save();
  ...
  Store.fetchJoinRequests = async function () {
    ...
    try {
      ...
      db.joinRequests = (res.data || []).map((r) => ({
        ...
        level: r.level || '', description: r.description || '', availability: cleanAvailability(r.availability || {}),
