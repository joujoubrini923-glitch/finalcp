...
- **Submit a Solve** (`#/submit`) — students enter their personal **secret code** + a Codeforces problem link; a secure server-side verifier asks the **real Codeforces API** and records the solve only if it finds an **Accepted** submission by that student's own username. Anti-cheat by design: wrong submissions, other people's submissions, old submissions (optional activation date) and replays are all refused; wrong codes get locked after repeated tries. Verified solves show a green **✓ CF** badge everywhere.
- **My Space (Student Passport)** (`#/space`) — one secret code unlocks a personal zone: **My AI Coach** (pace predictor: *"you reach Specialist in ~4 weeks"* + next-problem recommendations) and the **Code Clinic** (send buggy code, read the coach's answers).
- **Code Clinic** 🏥 — students send buggy code (rate-limited per week, coach-set); coach answers from a new admin inbox; answers appear in the student's space.
- **Hints for points** 🪙 — coach writes up to 3 hints per problem (stored server-side only); students reveal them at the cost of part of that problem's score — discounted by the server at verification time. Never costs anything if the problem is never solved.
- **Seasons & Champions Wall** 🏆 (`#/seasons`) — coach creates sprint seasons (solves + contests inside the window only); one live season at a time; ending freezes the Top 3 forever on the public wall.
- **Score bonuses** — coach-set extra points for weekly perfect streaks, multi-solve days and mastered topics (Settings → Gamification).
...
- **Auto-Verify (Codeforces)** — set each student's Codeforces username, generate/revoke secret codes (stored as server-side hashes only), one-click **Sync from Codeforces** to auto-record recent accepted solves, set an activation date, and browse the full verification log with links to the actual submissions.
...
- **Secret codes are verified server-side only**: they live (as SHA-256 hashes) in a table with zero read policies — invisible to everyone, including the site itself. All checking runs inside a Supabase **Edge Function** (Deno) that also calls the Codeforces API, enforces per-code lockouts, dedupes by submission ID, and writes results with the private service key that never leaves the server.
