...
async function actClaim(codeRaw: string, problemRaw: string) {
  ...
  // hint discounts reduce the earned score (server-side, un-forgeable)
  const unlockedHere = ((await rest(`hint_unlocks?student_id=eq.${encodeURIComponent(studentId)}&problem_id=eq.${encodeURIComponent(problem.id)}&select=idx`)).body || []).map((r: any) => r.idx);
  const basePts = Number(problem.score) || 0;
  const cut = unlockedHere.length ? await hintCutFor(doc, problem.id, unlockedHere) : 0;
  const floorPts = Math.max(1, Number(doc.settings && doc.settings.hintFloor) || 5);
  const finalPts = Math.max(floorPts, basePts - cut);
  ...
  return reply({
    ...
    points: finalPts, basePoints: basePts, hintCut: cut, row: ins.body[0],
  });
}
...
async function actSync(req: Request) {
  ...
  // hint discounts prep (bulk, once)
  const allUnlocks = (await rest('hint_unlocks?select=student_id,problem_id,idx&limit=5000')).body || [];
  const allHints = (await rest('problem_hints?select=problem_id,idx,cut&limit=5000')).body || [];
  const unlockMap: Record<string, number[]> = {};
  allUnlocks.forEach((u: any) => { (unlockMap[u.student_id + '|' + u.problem_id] = unlockMap[u.student_id + '|' + u.problem_id] || []).push(u.idx); });
  const hintMap: Record<string, Record<number, number | null>> = {};
  allHints.forEach((h: any) => { (hintMap[h.problem_id] = hintMap[h.problem_id] || {})[h.idx] = h.cut; });
  const globalCuts = (doc.settings && doc.settings.hintCuts) || [10, 15, 20];
  const floorPts = Math.max(1, Number(doc.settings && doc.settings.hintFloor) || 5);
  const discounted = (sid: string, pid: string, base: number) => {
    const idxes = unlockMap[sid + '|' + pid] || [];
    if (!idxes.length) return Math.max(0, base);
    let cut = 0;
    idxes.forEach((ix: number) => { const c = hintMap[pid] && hintMap[pid][ix]; cut += c != null ? Math.max(0, Number(c)) : Math.max(0, Number(globalCuts[ix]) || 0); });
    return Math.max(floorPts, base - cut);
  };
  ...
  for (const st of students) {
    ...
    for (const s of subs || []) {
      ...
      rows.push({
        ...
        points: discounted(st.id, problem.id, Number(problem.score) || 0), source: 'sync',
      });
    }
...
/* ---------- v1.7: Passport sessions, Clinic, Hints ---------- */
function newToken(): string {
  const b = crypto.getRandomValues(new Uint8Array(24));
  return Array.from(b).map((x) => x.toString(16).padStart(2, '0')).join('');
}
async function tokenStudent(token: string): Promise<string | null> {
  if (!token || token.length < 20) return null;
  const th = await sha256Hex(String(token));
  const r = await rest(`student_sessions?token_hash=eq.${th}&select=student_id,expires_at`);
  const row = r.body && r.body[0];
  if (!row || new Date(row.expires_at).getTime() < Date.now()) return null;
  return row.student_id;
}
// escalating lockout, shared with claim
async function checkLock(hash: string): Promise<number> {
  const att = await rest(`code_attempts?key=eq.${hash}&select=fails,last`);
  const row = att.body && att.body[0];
  if (!row) return 0;
  const lock = lockSeconds(row.fails);
  const waited = (Date.now() - new Date(row.last).getTime()) / 1000;
  return lock && waited < lock ? Math.ceil(lock - waited) : 0;
}
async function findStudentByCode(codeRaw: string) {
  const code = normalizeCode(codeRaw);
  if (code.length < 6) return null;
  const hash = await sha256Hex(code);
  const codeRow = (await rest(`student_codes?code_hash=eq.${hash}&select=student_id`)).body;
  return codeRow && codeRow[0] ? codeRow[0].student_id : null;
}
async function actPassport(codeRaw: string) {
  const code = normalizeCode(codeRaw);
  if (code.length < 6) return reply({ ok: false, error: 'bad-code' });
  const hash = await sha256Hex(code);
  if (await checkLock(hash)) return reply({ ok: false, error: 'locked' }, 429);
  const studentId = await findStudentByCode(codeRaw);
  if (!studentId) {
    await rest('code_attempts', { method: 'POST', headers: { Prefer: 'resolution=merge-duplicates' }, body: JSON.stringify({ key: hash, fails: 1, last: new Date().toISOString() }) });
    return reply({ ok: false, error: 'bad-code' });
  }
  const doc = await loadDoc();
  const student = doc && doc.students.find((s: any) => s.id === studentId);
  if (!student) return reply({ ok: false, error: 'no-code' });
  const token = newToken();
  await rest('student_sessions', { method: 'POST', body: JSON.stringify({ token_hash: await sha256Hex(token), student_id: studentId, expires_at: new Date(Date.now() + 7 * 864e5).toISOString() }) });
  await rest(`code_attempts?key=eq.${hash}`, { method: 'DELETE' });
  return reply({ ok: true, studentId, studentName: student.name, token });
}
const clip = (s: any, n: number) => String(s == null ? '' : s).slice(0, n);

async function actClinicSend(body: any) {
  const sid = await tokenStudent(String(body.token || ''));
  if (!sid) return reply({ ok: false, error: 'no-session', message: 'Enter your secret code first.' }, 401);
  const doc = await loadDoc();
  const student = doc && doc.students.find((s: any) => s.id === sid);
  if (!student) return reply({ ok: false, error: 'no-code' });
  const code = clip(body.code, 12000).trim();
  if (code.length < 10) return reply({ ok: false, error: 'too-short', message: 'Paste your buggy code first.' });
  const limit = Number(doc.settings && doc.settings.clinicWeeklyLimit) || 0;
  if (limit > 0) {
    const since = new Date(Date.now() - 7 * 864e5).toISOString();
    const cnt = (await rest(`clinic_cases?student_id=eq.${encodeURIComponent(sid)}&created_at=gte.${since}&select=id`)).body || [];
    if (cnt.length >= limit) return reply({ ok: false, error: 'weekly-limit', message: `Limit reached — the coach allows ${limit} case${limit === 1 ? '' : 's'} per week.` });
  }
  const link = clip(body.problemLink, 200), note = clip(body.note, 600);
  const symptom = ['wa', 'tle', 're', 'other'].includes(body.symptom) ? body.symptom : 'wa';
  const ins = await rest('clinic_cases', { method: 'POST', headers: { Prefer: 'return=representation' }, body: JSON.stringify({ student_id: sid, student_name: student.name, problem_link: link, symptom, code, note }) });
  if (ins.status >= 400) return reply({ ok: false, error: 'offline' });
  return reply({ ok: true });
}
async function actClinicList(body: any) {
  const sid = await tokenStudent(String(body.token || ''));
  if (!sid) return reply({ ok: false, error: 'no-session', message: 'Enter your secret code first.' }, 401);
  const r = await rest(`clinic_cases?student_id=eq.${encodeURIComponent(sid)}&select=id,created_at,symptom,problem_link,note,code,status,coach_reply,answered_at&order=created_at.desc&limit=50`);
  return reply({ ok: true, cases: r.body || [] });
}
async function actHintStatus(body: any) {
  const sid = await tokenStudent(String(body.token || ''));
  if (!sid) return reply({ ok: false, error: 'no-session', message: 'Enter your secret code first.' }, 401);
  const unlocks = (await rest(`hint_unlocks?student_id=eq.${encodeURIComponent(sid)}&select=problem_id,idx`)).body || [];
  const out: Record<string, Record<string, string>> = {};
  for (const u of unlocks) {
    const h = (await rest(`problem_hints?problem_id=eq.${encodeURIComponent(u.problem_id)}&idx=eq.${u.idx}&select=text,cut`)).body || [];
    if (h[0]) { (out[u.problem_id] = out[u.problem_id] || {})[u.idx] = h[0].text; }
  }
  // availability counts only (no hint text!) so the page can draw the lock buttons
  const avail: Record<string, number> = {};
  ((await rest('problem_hints?select=problem_id&limit=5000')).body || []).forEach((h: any) => { avail[h.problem_id] = (avail[h.problem_id] || 0) + 1; });
  return reply({ ok: true, hints: out, available: avail });
}
async function hintCutFor(doc: any, problemId: string, idxes: number[]) {
  const global = (doc.settings && doc.settings.hintCuts) || [10, 15, 20];
  let cut = 0;
  for (const idx of idxes) {
    const h = (await rest(`problem_hints?problem_id=eq.${encodeURIComponent(problemId)}&idx=eq.${idx}&select=cut`)).body || [];
    cut += h[0] && h[0].cut != null ? Math.max(0, Number(h[0].cut)) : Math.max(0, Number(global[idx]) || 0);
  }
  return cut;
}
async function actHintUnlock(body: any) {
  const sid = await tokenStudent(String(body.token || ''));
  if (!sid) return reply({ ok: false, error: 'no-session', message: 'Enter your secret code first.' }, 401);
  const pid = clip(body.problemId, 60), idx = Math.trunc(Number(body.idx));
  if (!pid || !(idx >= 0 && idx <= 2)) return reply({ ok: false, error: 'bad-request' }, 400);
  const doc = await loadDoc();
  const problem = doc && doc.problems.find((p: any) => p.id === pid);
  if (!problem) return reply({ ok: false, error: 'not-found' });
  const hrow = (await rest(`problem_hints?problem_id=eq.${encodeURIComponent(pid)}&idx=eq.${idx}&select=text,cut`)).body || [];
  if (!hrow[0]) return reply({ ok: false, error: 'no-hint' });
  // already solved? (manual or verified) — too late for hints
  const st = doc.students.find((s: any) => s.id === sid);
  const solvedDoc = st && (st.solves || []).some((sv: any) => sv.problemId === pid);
  const solvedVer = ((await rest(`verified_solves?student_id=eq.${encodeURIComponent(sid)}&problem_id=eq.${encodeURIComponent(pid)}&select=id`)).body || []).length > 0;
  if (solvedDoc || solvedVer) return reply({ ok: false, error: 'already-solved', message: 'You already solved it — no hints needed. 💪' });
  await rest('hint_unlocks', { method: 'POST', headers: { Prefer: 'resolution=ignore-duplicates' }, body: JSON.stringify({ student_id: sid, problem_id: pid, idx }) });
  const unlocked = ((await rest(`hint_unlocks?student_id=eq.${encodeURIComponent(sid)}&problem_id=eq.${encodeURIComponent(pid)}&select=idx`)).body || []).map((r: any) => r.idx);
  const totalCut = await hintCutFor(doc, pid, unlocked);
  const floor = Math.max(1, Number(doc.settings && doc.settings.hintFloor) || 5);
  const earns = Math.max(floor, (Number(problem.score) || 0) - totalCut);
  return reply({ ok: true, text: hrow[0].text, totalCut, earns, base: Number(problem.score) || 0 });
}
...
Deno.serve(async (req: Request) => {
  ...
  if (action === 'passport') return actPassport(String(body.code || ''));
  if (action === 'clinic-send') return actClinicSend(body);
  if (action === 'clinic-list') return actClinicList(body);
  if (action === 'hint-status') return actHintStatus(body);
  if (action === 'hint-unlock') return actHintUnlock(body);
