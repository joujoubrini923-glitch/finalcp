...
(function () {
  ...
  function tabVerify(el) {
    ...
    el.innerHTML = `
      ...
      <div class="card reveal in">
        <div class="card-title">${ic('users')}Student activity check</div>
        <p class="muted tiny" style="margin-top:8px">One click shows each student's latest public Codeforces visit and latest Student Mode login recorded by the verifier.</p>
        <div style="margin-top:10px;display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          <button class="btn btn-ghost btn-sm" id="vv-activity" ${authed ? '' : 'disabled'}>${ic('users')}Check activity</button>
          <span class="muted tiny" id="vv-activity-out"></span>
        </div>
        <div id="vv-activity-detail" style="margin-top:10px"></div>
      </div>
      <div class="card reveal in">
      ...
      <div class="jr-item" data-kind="${kind}" data-cid="${r.cloudId || ''}" data-lid="${r.id || ''}">
        ...
        <div class="jr-main">
          <div class="jr-head">
            ...
            ${kind === 'join' && r.referralCode ? `<span class="chip mono" title="Referral discount code">${U.esc(r.referralCode)}</span>` : ''}
