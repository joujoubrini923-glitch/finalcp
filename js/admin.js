...
(function () {
  ...
  function tabVerify(el) {
    ...
    if (activityBtn && authed) activityBtn.onclick = async () => {
      ...
      detail.innerHTML = cfWarn + loginWarn + `<div class="table-wrap"><table class="table"><thead><tr><th>Student</th><th>Codeforces</th><th>Last Codeforces visit</th><th>Last Student Mode login</th></tr></thead><tbody>${rows}</tbody></table></div>`;
    };
  ...
  function tabSettings(el) {
    ...
    el.innerHTML = `
      <div class="settings-command-bar card">
        ...
        <div class="settings-command-links">
          ${[['academy-settings','Academy & course'],['next-course-settings','Next courses'],['notification-settings','Email notifications'],['scoring-settings','Scoring'],['gamification-settings','Bonuses'],['rules-settings','Rule text'],['referral-settings','Referral challenges'],['levels-settings','Levels'],['topics-settings','Topics'],['security-settings','Security']].map(([id,label]) => `<button class="btn btn-ghost btn-sm" type="button" data-settings-jump="${id}">${label}</button>`).join('')}
        </div>
      </div>
      ...
      <div class="card" id="rules-settings" style="margin-top:18px">
        <div class="card-title">${ic('shield')}Student Scoring Rules text</div>
        <p class="muted tiny" style="margin:6px 0 12px">Edit the names, descriptions, icons and order shown in My Space → Scoring Rules.</p>
        <div class="table-wrap"><table class="table" style="min-width:780px"><thead><tr><th>Key</th><th>Name</th><th>Icon</th><th>Order</th><th>Description</th></tr></thead><tbody id="rules-body">${scoringRuleRows}</tbody></table></div>
        <button class="btn btn-primary btn-sm" id="rules-save" type="button" style="margin-top:12px">${ic('save')}Save Rule Text</button>
      </div>

      <div class="card" id="referral-settings" style="margin-top:18px">
        <div class="card-title">${ic('crown')}Leaderboard Discount Challenge</div>
        <p class="muted tiny" style="margin:6px 0 12px">Top 3 by leaderboard score win one Level 1 referral discount code for an invited friend. You can run multiple challenges.</p>
        <button class="btn btn-ghost btn-sm" id="ref-add" type="button">${ic('plus')}Add Challenge</button>
        <div id="ref-list">${referralRows || '<p class="muted tiny" style="margin-top:12px">No referral discount challenges yet.</p>'}</div>
      </div>
    ...
    const rulesSave = U.$('#rules-save', el);
    if (rulesSave) rulesSave.onclick = () => { Store.updateSettings({ scoringRules: U.$$('#rules-body tr', el).map((tr) => ({ key: U.$('[name="rule-key"]', tr).value, name: U.$('[name="rule-name"]', tr).value.trim() || U.$('[name="rule-key"]', tr).value, icon: U.$('[name="rule-icon"]', tr).value.trim() || 'shield', order: +U.$('[name="rule-order"]', tr).value || 0, description: U.$('[name="rule-desc"]', tr).value.trim() })) }); U.toast('Scoring rule text saved', 'success'); };
