...
(function () {
  ...
  // Only the raster data URLs produced by fileToAvatar may become image src
  // values. Imported/cloud data cannot inject javascript: or remote tracking URLs.
  U.safeImage = (s) => {
    const str = String(s == null ? '' : s).trim();
    return /^data:image\/(?:jpeg|png|webp|gif|bmp);base64,[a-z0-9+/=\s]+$/i.test(str) ? U.esc(str) : '';
  };
  U.avatarHTML = (student, cls) => {
    cls = cls || 'avatar-44';
    const photo = student && U.safeImage(student.photo);
    if (photo) {
      return `<span class="avatar ${cls}" title="${U.esc(student.name)}"><img src="${photo}" alt="${U.esc(student.name)}"></span>`;
    }
  ...
  U.confirm = (opts) => new Promise((resolve) => {
    ...
    const { close } = U.modal({
      ...
      body: `<p class="muted" style="line-height:1.6">${U.esc(opts.message || '')}</p>`,
      actions: [
