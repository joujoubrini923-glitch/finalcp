...
(function () {
  ...
  function wireSpace(root, p) {
    ...
    U.$('#sp-tabs', root).addEventListener('click', (e) => {
      ...
      (b.dataset.t === 'coach' ? renderCoach : renderClinic)();
    });
    renderCoach();
  }
  ...
  PV.seasons = (root) => {
    ...
    <div class="container page">
      ...
      <div class="section-head reveal" style="margin-top:26px"><div><h2>${ic('crown')}Champions Wall</h2><div class="sub">Top 3 of every finished season — forever remembered</div></div></div>
  ...
  PV.problems = (root) => {
    ...
    let myHints = {}, hintsAvail = {};
    const loadMyHints = async () => {
      if (!Store.passport || !Store.passport()) return;
      const r = await Store.hintsMine();
      if (r.ok && r.data) { myHints = r.data.hints || {}; hintsAvail = r.data.available || {}; }
    };
    ...
    // hint strip under a problem name (student mode only; text lives on the server)
    const hintStrip = (p) => {
      if (!me || meSolved.has(p.id) || !hintsAvail[p.id]) return '';
      const S = Store.settings();
      ...
      for (let i = 0; i < hintsAvail[p.id]; i++) {
        ...
        else bits.push(`<button class="hint-lock" data-hint="${p.id}|${i}" title="Unlocking this hint lowers the points this problem can earn you">${ic('lock')}Hint ${i + 1} <span class="hint-cost">−${cuts[i]} pts</span></button>`);
      }
