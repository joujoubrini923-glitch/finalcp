...
(function () {
  ...
  function timeline(items, limit) {
    ...
    const map = { solve: ['check', 'var(--green)'], contest: ['trophy', 'var(--gold)'], achievement: ['medal', 'var(--accent-2)'], student: ['userPlus', 'var(--accent)'], group: ['layers', 'var(--accent)'], problem: ['target', 'var(--gold)'], request: ['userPlus', 'var(--accent-2)'], question: ['help', 'var(--gold)'], clinic: ['code', 'var(--accent-2)'], solve: ['check', 'var(--green)'], mission: ['flag', 'var(--gold)'], streak: ['flame', 'var(--gold)'], habit: ['zap', 'var(--accent)'], mastery: ['award', 'var(--green)'], adjustment: ['settings', 'var(--accent-2)'], system: ['info', 'var(--dim)'] };
  ...
  function tabRecentActivity(el) {
    let limit = 20;
    let clinicRows = [];
    const build = () => {
      const scoreEvents = Store.students().flatMap((st) => (Store.scoreEventsOf(st.id) || []).map((e) => ({ type: e.type || 'score', text: `${st.name}: ${e.label} (${e.points > 0 ? '+' : ''}${e.points} pts)`, t: (e.date || U.today()) + 'T12:00:00' })));
      const reqEvents = [
        ...Store.joinRequests().map((r) => ({ type: 'request', text: `Join request: ${r.name}`, t: r.createdAt })),
        ...Store.trialRequests().map((r) => ({ type: 'request', text: `Free trial request: ${r.name} (${r.status})`, t: r.createdAt })),
        ...Store.nextCourseRequests().map((r) => ({ type: 'request', text: `Next-course request: ${r.studentName || r.name || 'student'}`, t: r.createdAt })),
        ...Store.questions().map((r) => ({ type: 'question', text: `Question: ${r.name}`, t: r.createdAt })),
        ...clinicRows.map((c) => ({ type: 'clinic', text: `Code clinic: ${c.student_name || 'student'} · ${c.symptom || 'case'} · ${c.status || 'open'}`, t: c.created_at })),
      ];
      const derivedActivityTypes = new Set(['solve', 'contest', 'achievement']);
      const manualActivity = Store.activity().filter((a) => !(derivedActivityTypes.has(a.type) && !/removed|deleted/i.test(String(a.text || ''))));
      const all = [...scoreEvents, ...reqEvents, ...manualActivity].sort((a, b) => Date.parse(b.t || 0) - Date.parse(a.t || 0));
      el.innerHTML = `<div class="page-head reveal in" style="margin-top:0"><h2>${ic('clock')}Recent Activity</h2><p>All score changes, solves, missions, streaks, topic mastery, requests and system activity in one timeline.</p></div><div class="card reveal in"><div class="card-title">${ic('clock')}Activity feed <span class="chip" style="margin-left:auto">${all.length}</span></div><div style="margin-top:14px">${timeline(all, limit)}</div>${all.length > limit ? `<button class="btn btn-ghost btn-sm" id="ra-more" style="margin-top:12px">Load more (${all.length - limit} remaining)</button>` : ''}</div>`;
      const more = U.$('#ra-more', el); if (more) more.onclick = () => { limit += 20; build(); };
    };
    build();
    if (Store.cloudEnabled && Store.cloudEnabled() && Store.cloudStatus().authed && Store.clinicCases) Store.clinicCases().then((r) => { if (r.ok) { clinicRows = r.rows || []; build(); } }).catch(() => {});
  }

  function tabActivityCheck(el) {
    const cloud = Store.cloudEnabled && Store.cloudEnabled();
    const authed = cloud && Store.cloudStatus().authed;
    const fmtTime = (value, seenMode) => {
      if (!value) return '<span class="muted tiny">never</span>';
      const ms = typeof value === 'number' ? value * 1000 : Date.parse(value);
      if (!isFinite(ms)) return '<span class="muted tiny">unknown</span>';
      if (seenMode && Date.now() - ms <= 5 * 60 * 1000) return '<span class="chip" style="color:var(--green);border-color:color-mix(in srgb,var(--green) 45%,var(--border))">Online</span>';
      return `<span title="${U.esc(new Date(ms).toISOString())}">${U.timeAgo(new Date(ms).toISOString())}</span><div class="muted tiny mono">${U.fmtDate(new Date(ms).toISOString().slice(0, 10))}</div>`;
    };
    const norm = (data) => { if (!data) return {}; if (data.students && !Array.isArray(data.students)) return data.students; if (Array.isArray(data.students)) { const m = {}; data.students.forEach((x) => { if (x.student_id) m[x.student_id] = x; if (x.studentId) m[x.studentId] = x; }); return m; } if (data.activity && !Array.isArray(data.activity)) return data.activity; return {}; };
    el.innerHTML = `<div class="page-head reveal in" style="margin-top:0"><h2>${ic('users')}Check Activity</h2><p>Check Codeforces last visit, Student Mode status and parent codes from one page.</p></div><div class="card reveal in"><div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap"><button class="btn btn-primary btn-sm" id="act-check" ${authed ? '' : 'disabled'}>${ic('users')}Check activity</button><span class="muted tiny" id="act-out">${authed ? '' : 'Sign in with cloud sync to check activity.'}</span></div><div id="act-detail" style="margin-top:12px"></div></div>`;
    U.$('#act-check', el).onclick = async () => {
      const out = U.$('#act-out', el), detail = U.$('#act-detail', el); out.textContent = 'Checking…'; detail.innerHTML = '';
      const [cf, logins] = await Promise.all([Store.cfLastVisits(), Store.cfActivityStatus()]); out.textContent = '';
      const loginMap = logins.ok ? norm(logins.data || {}) : {};
      const rows = Store.students().map((st) => { const cfInfo = st.cfHandle ? (cf.visits || {})[String(st.cfHandle).toLowerCase()] : null; const li = loginMap[st.id] || {}; const lastSeen = li.lastSeenAt || li.last_seen_at; const lastLogin = li.lastLoginAt || li.last_login_at; return `<tr><td><b>${U.esc(st.name)}</b></td><td><span class="chip mono" data-pcode-val="${U.esc(st.id)}">${U.esc(Store.parentCodeOf(st.id))}</span> <button class="btn btn-ghost btn-xs" data-gen-parent="${U.esc(st.id)}">Generate</button></td><td class="mono tiny">${st.cfHandle ? U.esc(st.cfHandle) : '<span style="color:var(--red)">not set</span>'}</td><td>${st.cfHandle ? (cfInfo ? fmtTime(cfInfo.lastOnlineTimeSeconds) : '<span class="muted tiny">not found</span>') : '<span class="muted tiny">—</span>'}</td><td>${fmtTime(lastSeen || lastLogin, true)}</td></tr>`; }).join('');
      const warns = (cf.ok ? '' : `<div class="vf-err vf-result" style="margin-bottom:8px">${ic('warn')}<span>Could not read Codeforces last-visit data.</span></div>`) + (logins.ok ? '' : `<div class="vf-err vf-result" style="margin-bottom:8px">${ic('warn')}<span>Student Mode activity is not available yet. Deploy the activity-status verifier function.</span></div>`);
      detail.innerHTML = warns + `<div class="table-wrap"><table class="table"><thead><tr><th>Student</th><th>Parent Code</th><th>Codeforces</th><th>Last Codeforces visit</th><th>Student Mode status</th></tr></thead><tbody>${rows}</tbody></table></div>`;
      detail.onclick = (ev) => { const b = ev.target.closest('[data-gen-parent]'); if (!b) return; const r = Store.regenerateParentCode(b.dataset.genParent); if (!r.ok) return U.toast(r.error || 'Could not generate parent code', 'error'); const cell = U.$(`[data-pcode-val="${b.dataset.genParent}"]`, detail); if (cell) cell.textContent = r.code; U.toast('Parent code generated', 'success', 'key'); };
    };
  }

  function tabStudentView(el) {
    const render = () => {
      const students = Store.students().slice().sort((a, b) => a.name.localeCompare(b.name));
      el.innerHTML = `<div class="page-head reveal in" style="margin-top:0"><h2>${ic('eye')}Student View</h2><p>Open a student's My Space view for checking only. This does not call the login function and does not update their last-login activity.</p></div><div class="card reveal in"><input class="input" id="sv-q" placeholder="Search students…"><div class="table-wrap" style="margin-top:12px"><table class="table"><thead><tr><th>Student</th><th>Group</th><th>Score</th><th></th></tr></thead><tbody id="sv-body"></tbody></table></div></div>`;
      const draw = () => { const q = U.$('#sv-q', el).value.trim().toLowerCase(); const list = q ? students.filter((s) => s.name.toLowerCase().includes(q)) : students; U.$('#sv-body', el).innerHTML = list.map((s) => { const g = Store.group(s.groupId); return `<tr><td><div style="display:flex;align-items:center;gap:10px">${U.avatarHTML(s, 'avatar-36')}<b>${U.esc(s.name)}</b></div></td><td>${g ? U.esc(g.name) : '—'}</td><td class="num">${Store.score(s.id)}</td><td><button class="btn btn-primary btn-sm" data-sv-open="${U.esc(s.id)}">Open Student Mode</button></td></tr>`; }).join('') || '<tr><td colspan="4" class="muted">No students found.</td></tr>'; };
      U.$('#sv-q', el).oninput = draw; draw();
      el.addEventListener('click', (ev) => { const b = ev.target.closest('[data-sv-open]'); if (!b) return; const st = Store.student(b.dataset.svOpen); if (!st) return; sessionStorage.setItem('acp_passport', JSON.stringify({ id: st.id, name: st.name, token: '', adminPreview: true })); sessionStorage.setItem('acp_me', JSON.stringify({ id: st.id, name: st.name })); location.hash = '#/space'; });
    };
    render();
  }
  ...
  function tabStudents(el) {
    ...
    const render = () => {
      ...
      U.$('#as-table', el).innerHTML = list.length ? `<table class="table">
        <thead><tr><th>Student</th><th>Group</th><th>Parent Code</th><th>Level</th><th>Score</th><th>Solved</th><th>Joined</th><th></th></tr></thead>
        <tbody>${list.map((s) => {
        ...
        return `<tr>
          ...
          <td><span class="chip mono">${U.esc(Store.parentCodeOf(s.id))}</span></td>
  ...
  /* ================= ATTENDANCE ================= */
  function tabAttendance(el) {
    const render = () => {
      const sessions = Store.attendanceSessions();
      el.innerHTML = `<div class="page-head reveal in" style="margin-top:0"><h2>${ic('calendar')}Attendance</h2><p>Mark course session attendance manually. Attendance is visible in parent reports and does not affect score.</p></div>
        <div class="card reveal in"><div class="card-title">${ic('plus')}New course session</div><div class="field-row" style="margin-top:12px"><label class="field"><span>Group</span><select class="select" id="att-group">${Store.groups().map((g) => `<option value="${U.esc(g.id)}">${U.esc(g.name)}</option>`).join('')}</select></label><label class="field"><span>Date</span><input class="input" type="date" id="att-date" value="${new Date().toISOString().slice(0,10)}"></label><label class="field"><span>Title</span><input class="input" id="att-title" value="Course session"></label></div><button class="btn btn-primary btn-sm" id="att-add">${ic('plus')}Create session</button></div>
        <div style="display:flex;flex-direction:column;gap:14px;margin-top:16px">${sessions.length ? sessions.map((ses) => { const group = Store.group(ses.groupId); const members = Store.students().filter((s) => s.groupId === ses.groupId); return `<section class="card reveal in" data-att="${U.esc(ses.id)}"><div class="card-title">${ic('calendar')}${U.esc(ses.title || 'Course session')} <span class="chip" style="margin-left:auto">${U.fmtDate(ses.date)}</span></div><p class="muted tiny">${U.esc(group ? group.name : 'Removed group')}</p><div class="table-wrap"><table class="table"><thead><tr><th>Student</th><th>Status</th></tr></thead><tbody>${members.map((st) => `<tr><td><b>${U.esc(st.name)}</b></td><td><select class="select" data-att-mark="${U.esc(st.id)}"><option value="present" ${(ses.marks || {})[st.id] === 'present' ? 'selected' : ''}>Present</option><option value="absent" ${(ses.marks || {})[st.id] === 'absent' ? 'selected' : ''}>Absent</option></select></td></tr>`).join('')}</tbody></table></div><div style="margin-top:10px;display:flex;gap:8px"><button class="btn btn-ghost btn-sm" data-att-save>${ic('save')}Save attendance</button><button class="icon-btn" data-att-del style="color:var(--red)">${ic('trash')}</button></div></section>`; }).join('') : '<div class="card"><p class="muted">No attendance sessions yet.</p></div>'}</div>`;
      U.$('#att-add', el).onclick = () => { const r = Store.addAttendanceSession({ groupId: U.$('#att-group', el).value, date: U.$('#att-date', el).value, title: U.$('#att-title', el).value }); if (!r.ok) return U.toast(r.error, 'error'); U.toast('Session created', 'success'); render(); };
      U.$$('[data-att-save]', el).forEach((b) => b.onclick = () => { const card = b.closest('[data-att]'); const marks = {}; U.$$('[data-att-mark]', card).forEach((x) => marks[x.dataset.attMark] = x.value); Store.updateAttendance(card.dataset.att, marks); U.toast('Attendance saved', 'success'); render(); });
      U.$$('[data-att-del]', el).forEach((b) => b.onclick = async () => { const ok = await U.confirm('Delete this attendance session?', { title: 'Delete session', okLabel: 'Delete' }); if (!ok) return; Store.deleteAttendanceSession(b.closest('[data-att]').dataset.att); render(); });
    };
    render();
  }

  /* ================= TRIAL REQUESTS ================= */
  function tabTrials(el) {
    const render = () => {
      const list = Store.trialRequests();
      el.innerHTML = `<div class="page-head reveal in" style="margin-top:0"><h2>${ic('userPlus')}Trial Requests</h2><p>Free first Level 1 session requests. Accepted means accepted for free trial only.</p></div><div class="card reveal in"><div class="table-wrap"><table class="table"><thead><tr><th>Student</th><th>Contact</th><th>Status</th><th>Date</th><th>Actions</th></tr></thead><tbody>${list.length ? list.map((r) => `<tr><td><b>${U.esc(r.name)}</b><br><span class="muted tiny">Age ${U.esc(r.age || '')}</span></td><td><a class="link-accent" href="mailto:${U.esc(r.email)}">${U.esc(r.email)}</a>${r.phone ? `<br><span class="muted tiny">${U.esc(r.phone)}</span>` : ''}</td><td><span class="chip">${U.esc(r.status)}</span></td><td>${U.timeAgo(r.createdAt)}</td><td><button class="btn btn-ghost btn-sm" data-trial="accepted" data-id="${U.esc(r.id)}">Accept</button><button class="btn btn-ghost btn-sm" data-trial="rejected" data-id="${U.esc(r.id)}">Reject</button><button class="btn btn-ghost btn-sm" data-trial="new" data-id="${U.esc(r.id)}">New</button></td></tr>`).join('') : '<tr><td colspan="5" class="muted">No trial requests yet.</td></tr>'}</tbody></table></div></div>`;
      U.$$('[data-trial]', el).forEach((b) => b.onclick = () => { Store.setTrialStatus(b.dataset.id, b.dataset.trial); U.toast('Trial status updated', 'success'); render(); });
    };
    render();
  }
  ...
  function tabMissions(el) {
    const S = Store.settings();
    const today = () => new Date().toISOString().slice(0, 10);
    const render = () => {
      const students = Store.students().slice().sort((a, b) => a.name.localeCompare(b.name));
      const groups = Store.groups().slice().sort((a, b) => a.name.localeCompare(b.name));
      const problems = Store.problems().slice().sort((a, b) => a.name.localeCompare(b.name));
      const missions = Store.missions().slice().sort((a, b) => (b.createdAt || '').localeCompare(a.createdAt || ''));
      el.innerHTML = `<div class="page-head reveal in" style="margin-top:0"><h2>${ic('flag')}Weekly Missions</h2><p>Assign selected Codeforces problems to one student or to every student in a group.</p></div>
        <div class="card reveal in">
          <div class="card-title">${ic('plus')}Create Mission</div>
          <div class="field-row" style="margin-top:12px">
            <label class="field" style="max-width:210px"><span>Assign to</span><select class="select" id="ms-target-type"><option value="student">One student</option><option value="group">All students in a group</option></select></label>
            <label class="field" id="ms-student-wrap"><span>Student</span><select class="select" id="ms-student">${students.map((st) => `<option value="${U.esc(st.id)}">${U.esc(st.name)}</option>`).join('')}</select></label>
            <label class="field hidden" id="ms-group-wrap"><span>Group</span><select class="select" id="ms-group">${groups.map((g) => `<option value="${U.esc(g.id)}">${U.esc(g.name)} · ${students.filter((s) => s.groupId === g.id).length} students</option>`).join('')}</select></label>
          </div>
          <div class="field-row">
            <label class="field"><span>Mission title</span><input class="input" id="ms-title" value="Weekly Mission"></label>
            <label class="field"><span>Deadline *</span><input class="input" type="date" id="ms-deadline" value="${today()}"></label>
            <label class="field"><span>Reward points</span><input class="input" type="number" min="0" id="ms-reward" value="${S.missionDefaultReward || 50}"></label>
          </div>
          <div class="card" style="background:var(--bg-2);padding:12px;margin:10px 0">
            <div class="field-row" style="align-items:flex-end">
              <label class="field" style="margin:0"><span>Search problems by name/topic</span><input class="input" id="ms-problem-search" placeholder="Search problem name, topic, difficulty…"></label>
              <select class="select" id="ms-problem-topic" style="max-width:220px"><option value="">All topics</option>${Store.topics().map((t) => `<option>${U.esc(t.name)}</option>`).join('')}</select>
              <select class="select" id="ms-problem-diff" style="max-width:180px"><option value="">All difficulties</option><option value="easy">Easy</option><option value="medium">Medium</option><option value="hard">Hard</option></select>
              <span class="chip" id="ms-selected-count">0 selected</span>
            </div>
            <div id="ms-problem-list" style="max-height:340px;overflow:auto;display:grid;gap:8px;margin-top:12px"></div>
          </div>
          <button class="btn btn-primary btn-sm" id="ms-add">${ic('flag')}Assign mission</button>
        </div>
        <div class="card reveal in" style="margin-top:16px"><div class="card-title">${ic('flag')}Current Missions <span class="chip" style="margin-left:auto">${missions.length}</span></div>
          <div class="table-wrap" style="margin-top:12px"><table class="table"><thead><tr><th>Student</th><th>Mission</th><th>Progress</th><th>Deadline</th><th>Reward</th><th>Status</th><th></th></tr></thead><tbody>${missions.length ? missions.map((m) => { const st = Store.student(m.studentId); const status = Store.missionStatus(m); return `<tr><td><b>${U.esc(st ? st.name : 'Removed student')}</b></td><td>${U.esc(m.title || 'Weekly Mission')}<br><span class="muted tiny">${(m.problemIds || []).map((id) => (Store.problem(id) || {}).name).filter(Boolean).map(U.esc).join(' · ')}</span></td><td>${status.done}/${status.total}</td><td>${U.fmtDate(m.deadline)}</td><td class="num" style="color:var(--gold)">+${m.reward || 0}</td><td><span class="chip">${status.complete ? 'Completed' : (status.expired ? 'Expired' : 'Active')}</span></td><td><button class="icon-btn" data-ms-del="${U.esc(m.id)}" style="color:var(--red)">${ic('trash')}</button></td></tr>`; }).join('') : '<tr><td colspan="7" class="muted">No missions assigned yet.</td></tr>'}</tbody></table></div>
        </div>`;
      const selected = new Set();
      const syncTarget = () => { const groupMode = U.$('#ms-target-type', el).value === 'group'; U.$('#ms-student-wrap', el).classList.toggle('hidden', groupMode); U.$('#ms-group-wrap', el).classList.toggle('hidden', !groupMode); };
      const drawProblems = () => {
        const q = U.$('#ms-problem-search', el).value.trim().toLowerCase(); const tp = U.$('#ms-problem-topic', el).value; const df = U.$('#ms-problem-diff', el).value;
        let list = problems;
        if (q) list = list.filter((p) => (`${p.name} ${p.topic} ${p.difficulty}`).toLowerCase().includes(q));
        if (tp) list = list.filter((p) => p.topic === tp);
        if (df) list = list.filter((p) => p.difficulty === df);
        U.$('#ms-selected-count', el).textContent = selected.size + ' selected';
        U.$('#ms-problem-list', el).innerHTML = list.length ? list.map((p) => `<label class="rec-row" style="cursor:pointer"><input type="checkbox" data-ms-problem="${U.esc(p.id)}" ${selected.has(p.id) ? 'checked' : ''}><span style="flex:1"><b>${U.esc(p.name)}</b><br><small class="muted">${U.esc(p.topic)} · ${U.esc(p.difficulty)} · +${p.score}</small></span>${C.diffTag(p.difficulty)}</label>`).join('') : '<p class="muted tiny">No problems match your search.</p>';
      };
      U.$('#ms-target-type', el).onchange = syncTarget; syncTarget();
      ['ms-problem-search', 'ms-problem-topic', 'ms-problem-diff'].forEach((id) => U.$('#' + id, el).addEventListener('input', drawProblems));
      U.$('#ms-problem-list', el).addEventListener('change', (e) => { const cb = e.target.closest('[data-ms-problem]'); if (!cb) return; cb.checked ? selected.add(cb.dataset.msProblem) : selected.delete(cb.dataset.msProblem); U.$('#ms-selected-count', el).textContent = selected.size + ' selected'; });
      drawProblems();
      U.$('#ms-add', el).onclick = () => {
        const studentIds = U.$('#ms-target-type', el).value === 'group' ? students.filter((st) => st.groupId === U.$('#ms-group', el).value).map((st) => st.id) : [U.$('#ms-student', el).value];
        if (!studentIds.length) return U.toast('No students found in this group', 'error');
        let firstError = null;
        studentIds.forEach((sid) => { const r = Store.addMission({ studentId: sid, title: U.$('#ms-title', el).value, deadline: U.$('#ms-deadline', el).value, reward: U.$('#ms-reward', el).value, problemIds: Array.from(selected) }); if (!r.ok && !firstError) firstError = r.error; });
        if (firstError) return U.toast(firstError, 'error');
        U.toast(`Mission assigned to ${studentIds.length} student${studentIds.length === 1 ? '' : 's'}`, 'success', 'flag'); render();
      };
      U.$$('[data-ms-del]', el).forEach((b) => b.onclick = async () => { const ok = await U.confirm('Delete this mission?', { title: 'Delete mission', okLabel: 'Delete' }); if (!ok) return; Store.deleteMission(b.dataset.msDel); U.toast('Mission deleted', 'info'); render(); });
    };
    render();
  }

  /* ================= CHALLENGES ================= */
  function tabChallenges(el) {
    const today = () => new Date().toISOString().slice(0, 10);
    const buildGroupOptions = (selected) => `<option value="" ${!selected ? 'selected' : ''}>Global leaderboard</option>${Store.groups().map((g) => `<option value="${U.esc(g.id)}" ${g.id === selected ? 'selected' : ''}>${U.esc(g.name)}</option>`).join('')}`;
    const readCards = () => U.$$('[data-ch-card]', el).map((card) => {
      const old = (Store.referralChallenges() || [])[+card.dataset.chCard] || {};
      return {
        id: old.id || ('ref_' + Date.now() + '_' + card.dataset.chCard),
        title: U.$('[name="ch-title"]', card).value.trim() || 'Leaderboard Discount Challenge',
        groupId: U.$('[name="ch-group"]', card).value,
        start: U.$('[name="ch-start"]', card).value,
        end: U.$('[name="ch-end"]', card).value,
        discounts: [1, 2, 3].map((n) => Math.max(0, Math.min(100, +U.$(`[name="ch-d${n}"]`, card).value || 0))),
        enabled: U.$('[name="ch-enabled"]', card).checked,
        finalized: !!old.finalized,
        finalizedAt: old.finalizedAt || null,
        winners: old.winners || [],
      };
    });
    const save = (silent) => { Store.saveReferralChallenges(readCards()); if (!silent) U.toast('Challenges saved', 'success'); render(); };
    const challengeCard = (c, i) => {
      const standings = Store.referralStandings(c).slice(0, 10);
      const scopeName = c.groupId ? ((Store.group(c.groupId) || {}).name || 'Selected group') : 'Global leaderboard';
      const discounts = c.discounts || [30, 20, 10];
      return `<article class="card reveal in" data-ch-card="${i}" style="margin-top:16px;border-color:${c.finalized ? 'color-mix(in srgb,var(--green) 45%,var(--border))' : 'var(--border)'}">
        <div class="card-title">${ic('crown')}${U.esc(c.title || 'Leaderboard Discount Challenge')} <span class="chip" style="margin-left:auto">${c.finalized ? 'Finalized' : (c.enabled === false ? 'Hidden' : 'Live draft')}</span></div>
        <div class="field-row" style="margin-top:12px">
          <label class="field"><span>Challenge name</span><input class="input" name="ch-title" value="${U.esc(c.title || 'Leaderboard Discount Challenge')}"></label>
          <label class="field" style="max-width:240px"><span>Leaderboard</span><select class="select" name="ch-group">${buildGroupOptions(c.groupId || '')}</select></label>
        </div>
        <div class="field-row">
          <label class="field"><span>Start date</span><input class="input" type="date" name="ch-start" value="${U.esc(c.start || today())}"></label>
          <label class="field"><span>End date</span><input class="input" type="date" name="ch-end" value="${U.esc(c.end || '')}"></label>
          <label class="field"><span>Top 3 discounts (%)</span><div style="display:flex;gap:8px"><input class="input" type="number" min="0" max="100" name="ch-d1" value="${discounts[0] || 0}" title="Rank 1"><input class="input" type="number" min="0" max="100" name="ch-d2" value="${discounts[1] || 0}" title="Rank 2"><input class="input" type="number" min="0" max="100" name="ch-d3" value="${discounts[2] || 0}" title="Rank 3"></div></label>
        </div>
        <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin:8px 0 14px">
          <label class="chip" style="cursor:pointer"><input type="checkbox" name="ch-enabled" ${c.enabled !== false ? 'checked' : ''}> show to students while active</label>
          <span class="chip">${U.esc(scopeName)}</span>
          <span class="chip">Metric: total score at end</span>
          <button class="btn btn-ghost btn-sm" type="button" data-ch-save>${ic('save')}Save</button>
          <button class="btn btn-primary btn-sm" type="button" data-ch-finalize="${U.esc(c.id)}" ${c.finalized ? 'disabled' : ''}>${ic('crown')}Finalize winners</button>
          <button class="icon-btn" type="button" data-ch-del title="Delete" style="color:var(--red)">${ic('trash')}</button>
        </div>
        <div class="table-wrap"><table class="table"><thead><tr><th colspan="5">Live preview</th></tr><tr><th>#</th><th>Student</th><th>Score</th><th>Discount</th><th>Winner code</th></tr></thead><tbody>
          ${standings.length ? standings.map((e, rankIndex) => { const w = (c.winners || []).find((x) => x.studentId === e.s.id); return `<tr><td class="rank-cell ${e.rank <= 3 ? 'r' + e.rank : ''}">${e.rank}</td><td><b>${U.esc(e.s.name)}</b></td><td class="num">${U.fmtNum(e.score)}</td><td>${rankIndex < 3 ? (discounts[rankIndex] || 0) + '%' : '—'}</td><td class="mono">${w ? U.esc(w.code) : (rankIndex < 3 ? 'created on finalize' : '—')}</td></tr>`; }).join('') : '<tr><td colspan="5" class="muted">No students in this leaderboard yet.</td></tr>'}
        </tbody></table></div>
        ${c.finalized && (c.winners || []).length ? `<div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap">${c.winners.map((w) => `<span class="chip">#${w.rank} ${U.esc(w.name)} · <b class="mono">${U.esc(w.code)}</b> · ${w.discount}%</span>`).join('')}</div>` : ''}
      </article>`;
    };
    function render() {
      const list = Store.referralChallenges();
      el.innerHTML = `<div class="page-head reveal in" style="margin-top:0"><h2>${ic('crown')}Challenges</h2><p>Create and finalize Leaderboard Discount Challenges. Winners receive a Level 1 referral discount code for one invited friend.</p></div>
        <div class="card reveal in" style="background:linear-gradient(135deg,color-mix(in srgb,var(--gold) 10%,var(--card)),var(--card));border-color:color-mix(in srgb,var(--gold) 35%,var(--border))">
          <div class="card-title">${ic('zap')}Challenge rules</div>
          <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px"><span class="chip">Top 3 win</span><span class="chip">Global or one group</span><span class="chip">Total score including manual adjustments</span><span class="chip">Code valid until used</span><span class="chip">Level 1 only</span></div>
          <button class="btn btn-primary btn-sm" id="ch-add" type="button" style="margin-top:14px">${ic('plus')}Add Challenge</button>
        </div>
        <div id="ch-list">${list.length ? list.map(challengeCard).join('') : '<div class="card reveal in" style="margin-top:16px"><p class="muted">No challenges yet. Click Add Challenge to create one.</p></div>'}</div>`;
      U.$('#ch-add', el).onclick = () => { const l = Store.referralChallenges().slice(); l.push({ id: 'ref_' + Date.now(), title: 'Leaderboard Discount Challenge', groupId: '', start: today(), end: '', discounts: [30, 20, 10], enabled: true, finalized: false, winners: [] }); Store.saveReferralChallenges(l); render(); };
      U.$$('[data-ch-save]', el).forEach((b) => b.onclick = () => save(false));
      U.$$('[data-ch-del]', el).forEach((b) => b.onclick = async () => { const ok = await U.confirm('Delete this challenge?', { title: 'Delete challenge', okLabel: 'Delete' }); if (!ok) return; const card = b.closest('[data-ch-card]'); const l = readCards(); l.splice(+card.dataset.chCard, 1); Store.saveReferralChallenges(l); U.toast('Challenge deleted', 'info'); render(); });
      U.$$('[data-ch-finalize]', el).forEach((b) => b.onclick = async () => { Store.saveReferralChallenges(readCards()); const ok = await U.confirm('Finalize winners now? This locks the current top 3 and creates their referral discount codes.', { title: 'Finalize winners', okLabel: 'Finalize' }); if (!ok) return; const r = Store.finalizeReferralChallenge(b.dataset.chFinalize); if (!r.ok) return U.toast(r.error || 'Could not finalize', 'error'); U.toast('Winners finalized and codes generated', 'success', 'crown'); render(); });
    }
    render();
  }
  ...
  function tabVerify(el) {
    ...
    const auditHTML = () => {
      ...
      U.$('#vv-audit', el).innerHTML = rows.length ? `<div class="jr-list">${rows.slice(0, auditLimit).map((r) => `
        <div class="jr-item">
        ...
        </div>`).join('')}</div>${rows.length > auditLimit ? `<button class="btn btn-ghost btn-sm" id="vv-audit-more" style="margin-top:10px">Load more (${rows.length - auditLimit} remaining)</button>` : ''}` : '<p class="muted tiny">No verified solves yet. Share codes with students or hit “Sync now”.</p>';
      const more = U.$('#vv-audit-more', el); if (more) more.onclick = () => { auditLimit += 20; auditHTML(); };
    };
    auditHTML();
    ...
    const normalizeActivityRows = (data) => {
      ...
      if (Array.isArray(data.students)) { const m = {}; data.students.forEach((x) => { if (x && x.student_id) m[x.student_id] = x; if (x && x.studentId) m[x.studentId] = x; }); return m; }
      if (data.activity && !Array.isArray(data.activity)) return data.activity;
      return {};
    };
    const activityBtn = U.$('#vv-activity', el);
    if (activityBtn && authed) activityBtn.onclick = async () => {
      const out = U.$('#vv-activity-out', el), detail = U.$('#vv-activity-detail', el);
      activityBtn.disabled = true;
      out.textContent = 'Checking Codeforces and student logins…';
      detail.innerHTML = '';
      const [cf, logins] = await Promise.all([Store.cfLastVisits(), Store.cfActivityStatus()]);
      activityBtn.disabled = false;
      out.textContent = '';
      const loginMap = logins.ok ? normalizeActivityRows(logins.data || {}) : {};
      const rows = Store.students().map((st) => {
        const cfInfo = st.cfHandle ? (cf.visits || {})[String(st.cfHandle).toLowerCase()] : null;
        const li = loginMap[st.id] || {};
        const lastLogin = li.lastLoginAt || li.last_login_at || li.lastSeenAt || li.last_seen_at || li.created_at || li.expires_at;
        const lastCodeUse = li.lastCodeUsedAt || li.last_code_used_at || li.last_used_at;
        return `<tr>
          <td><b>${U.esc(st.name)}</b></td>
          <td><span class="chip mono" data-pcode-val="${U.esc(st.id)}">${U.esc(Store.parentCodeOf(st.id))}</span> <button class="btn btn-ghost btn-xs" data-gen-parent="${U.esc(st.id)}" type="button">Generate</button></td>
          <td class="mono tiny">${st.cfHandle ? U.esc(st.cfHandle) : '<span style="color:var(--red)">not set</span>'}</td>
          <td>${st.cfHandle ? (cfInfo ? fmtActivityTime(cfInfo.lastOnlineTimeSeconds) : '<span class="muted tiny">not found</span>') : '<span class="muted tiny">—</span>'}</td>
          <td>${fmtActivityTime(lastLogin || lastCodeUse)}</td>
        </tr>`;
      }).join('');
      const cfWarn = cf.ok ? '' : `<div class="vf-err vf-result" style="margin-bottom:8px">${ic('warn')}<span>Could not read Codeforces last-visit data: ${U.esc(cf.error || 'Codeforces is unavailable')}</span></div>`;
      const loginWarn = logins.ok ? '' : `<div class="vf-err vf-result" style="margin-bottom:8px">${ic('warn')}<span>Student Mode login times are not available yet. Update the verifier function so it supports the <b>activity-status</b> action and records session login time.</span></div>`;
      detail.innerHTML = cfWarn + loginWarn + `<div class="table-wrap"><table class="table"><thead><tr><th>Student</th><th>Parent Code</th><th>Codeforces</th><th>Last Codeforces visit</th><th>Last Student Mode login</th></tr></thead><tbody>${rows}</tbody></table></div>`;
      detail.onclick = (ev) => {
        const b = ev.target.closest('[data-gen-parent]'); if (!b) return;
        const r = Store.regenerateParentCode(b.dataset.genParent);
        if (!r.ok) return U.toast(r.error || 'Could not generate parent code', 'error');
        const outCell = U.$(`[data-pcode-val="${b.dataset.genParent}"]`, detail);
        if (outCell) outCell.textContent = r.code;
        U.toast('Parent code generated', 'success', 'key');
      };
    };
  ...
  function tabSettings(el) {
    ...
    const dailyBonusRows = (S.dailyBonusTiers || []).map((t, i) => `<tr data-db-tier="${i}"><td><input class="input" type="number" min="1" name="db-count" value="${t.count}"></td><td><input class="input" type="number" min="0" name="db-points" value="${t.points}"></td><td><button class="icon-btn" data-db-del="${i}" title="Remove" style="color:var(--red)">${ic('trash')}</button></td></tr>`).join('');
    const scoringRuleRows = (S.scoringRules || []).slice().sort((a, b) => (a.order || 0) - (b.order || 0)).map((r, i) => `<tr data-rule="${i}"><td><input class="input mono" name="rule-key" value="${U.esc(r.key)}" readonly></td><td><input class="input" name="rule-name" value="${U.esc(r.name)}"></td><td><input class="input" name="rule-icon" value="${U.esc(r.icon)}"></td><td><input class="input" type="number" name="rule-order" value="${r.order || ((i + 1) * 10)}"></td><td><textarea class="input" name="rule-desc" rows="2" maxlength="600">${U.esc(r.description || '')}</textarea></td></tr>`).join('');
    const groupOptions = `<option value="">Global leaderboard</option>${Store.groups().map((g) => `<option value="${U.esc(g.id)}">${U.esc(g.name)}</option>`).join('')}`;
    const referralRows = (S.referralChallenges || []).map((c, i) => { const rows = Store.referralStandings ? Store.referralStandings(c).slice(0, 5) : []; return `<article class="card" data-ref-card="${i}" style="padding:14px;margin-top:12px"><div class="field-row"><label class="field"><span>Name</span><input class="input" name="ref-title" value="${U.esc(c.title || 'Leaderboard Discount Challenge')}"></label><label class="field" style="max-width:200px"><span>Leaderboard</span><select class="select" name="ref-group">${groupOptions.replace(`value=\"${U.esc(c.groupId || '')}\"`, `value=\"${U.esc(c.groupId || '')}\" selected`)}</select></label></div><div class="field-row"><label class="field"><span>Start</span><input class="input" type="date" name="ref-start" value="${U.esc(c.start || '')}"></label><label class="field"><span>End</span><input class="input" type="date" name="ref-end" value="${U.esc(c.end || '')}"></label><label class="field"><span>#1 / #2 / #3 discount %</span><div style="display:flex;gap:8px"><input class="input" type="number" min="0" max="100" name="ref-d1" value="${(c.discounts || [30,20,10])[0] || 0}"><input class="input" type="number" min="0" max="100" name="ref-d2" value="${(c.discounts || [30,20,10])[1] || 0}"><input class="input" type="number" min="0" max="100" name="ref-d3" value="${(c.discounts || [30,20,10])[2] || 0}"></div></label></div><div class="field-row"><label class="chip" style="cursor:pointer"><input type="checkbox" name="ref-enabled" ${c.enabled !== false ? 'checked' : ''}> visible/live</label><span class="chip">${c.finalized ? 'Finalized' : 'Not finalized'}</span><button class="btn btn-ghost btn-sm" type="button" data-ref-save>Save challenge</button><button class="btn btn-primary btn-sm" type="button" data-ref-finalize="${U.esc(c.id)}" ${c.finalized ? 'disabled' : ''}>Finalize winners</button><button class="icon-btn" type="button" data-ref-del style="color:var(--red)">${ic('trash')}</button></div><div class="table-wrap" style="margin-top:10px"><table class="table"><thead><tr><th colspan="4">Live preview</th></tr><tr><th>#</th><th>Student</th><th>Score</th><th>Discount/code</th></tr></thead><tbody>${rows.map((e, ri) => { const d = (c.discounts || [30,20,10])[ri] || ''; const w = (c.winners || []).find((x) => x.studentId === e.s.id); return `<tr><td>${e.rank}</td><td>${U.esc(e.s.name)}</td><td class="num">${U.fmtNum(e.score)}</td><td>${ri < 3 ? `${d}%${w ? ' · ' + U.esc(w.code) : ''}` : '—'}</td></tr>`; }).join('') || '<tr><td colspan="4" class="muted">No students in this leaderboard yet.</td></tr>'}</tbody></table></div></article>`; }).join('');
    ...
    el.innerHTML = `
      ...
      <div class="card" id="gamification-settings" style="margin-top:18px">
        ...
        <div class="field-row">
          ...
          <label class="field" style="margin:0;max-width:210px"><span>Apply new bonus scoring from</span><input class="input" type="date" id="bn-start" value="${S.newScoringStart || '2026-09-06'}"></label>
        </div>
        <div class="table-wrap" style="margin-top:12px"><table class="table" style="min-width:360px"><thead><tr><th>Daily solved count</th><th>Bonus points</th><th></th></tr></thead><tbody id="db-body">${dailyBonusRows}</tbody></table></div>
        <button class="btn btn-ghost btn-sm" id="db-add" type="button" style="margin-top:10px">${ic('plus')}Add daily tier</button>
        <div class="field-row" style="margin-top:12px">
          ...
          <label class="field" style="margin:0;max-width:190px"><span>Default mission reward</span><input class="input" type="number" min="0" id="ms-default-reward" value="${S.missionDefaultReward || 50}"></label>
        </div>
    ...
    if (gmBtn) gmBtn.onclick = () => {
      Store.updateSettings({
        ...
        newScoringStart: U.$('#bn-start', el).value || '2026-09-06',
        dailyBonusTiers: U.$$('#db-body tr', el).map((tr) => ({ count: Math.max(1, +U.$('[name="db-count"]', tr).value || 1), points: Math.max(0, +U.$('[name="db-points"]', tr).value || 0) })).sort((a, b) => a.count - b.count),
        bonusMultiDay: Math.max(0, (U.$$('#db-body tr', el).find((tr) => +U.$('[name="db-count"]', tr).value === 2) ? +U.$('[name="db-points"]', U.$$('#db-body tr', el).find((tr) => +U.$('[name="db-count"]', tr).value === 2)).value : 0) || 0),
        ...
        missionDefaultReward: Math.max(0, +U.$('#ms-default-reward', el).value || 0),
      });
      U.toast('Gamification saved — scores update live ⚡', 'success');
    };

    const dbAdd = U.$('#db-add', el);
    if (dbAdd) dbAdd.onclick = () => { const tb = U.$('#db-body', el); const tr = document.createElement('tr'); tr.innerHTML = `<td><input class="input" type="number" min="1" name="db-count" value="2"></td><td><input class="input" type="number" min="0" name="db-points" value="0"></td><td><button class="icon-btn" data-db-del style="color:var(--red)">${ic('trash')}</button></td>`; tb.appendChild(tr); };
    U.$('#db-body', el)?.addEventListener('click', (e) => { if (e.target.closest('[data-db-del]')) e.target.closest('tr').remove(); });
    const readReferralCards = () => U.$$('[data-ref-card]', el).map((card) => ({ id: (S.referralChallenges[+card.dataset.refCard] || {}).id, finalized: !!(S.referralChallenges[+card.dataset.refCard] || {}).finalized, finalizedAt: (S.referralChallenges[+card.dataset.refCard] || {}).finalizedAt || null, winners: (S.referralChallenges[+card.dataset.refCard] || {}).winners || [], title: U.$('[name="ref-title"]', card).value.trim() || 'Leaderboard Discount Challenge', groupId: U.$('[name="ref-group"]', card).value, start: U.$('[name="ref-start"]', card).value, end: U.$('[name="ref-end"]', card).value, enabled: U.$('[name="ref-enabled"]', card).checked, discounts: [1,2,3].map((n) => Math.max(0, Math.min(100, +U.$(`[name="ref-d${n}"]`, card).value || 0))) }));
    const saveReferrals = () => { Store.saveReferralChallenges(readReferralCards()); U.toast('Referral challenges saved', 'success'); tabSettings(el); };
    U.$('#ref-add', el)?.addEventListener('click', () => { const list = readReferralCards(); list.push({ id: 'ref_' + Date.now(), title: 'Leaderboard Discount Challenge', groupId: '', start: U.today ? U.today() : new Date().toISOString().slice(0, 10), end: '', discounts: [30, 20, 10], enabled: true, finalized: false, winners: [] }); Store.saveReferralChallenges(list); tabSettings(el); });
    U.$$('#ref-list [data-ref-save]', el).forEach((b) => b.onclick = saveReferrals);
    U.$$('#ref-list [data-ref-del]', el).forEach((b) => b.onclick = () => { const card = b.closest('[data-ref-card]'); const list = readReferralCards(); list.splice(+card.dataset.refCard, 1); Store.saveReferralChallenges(list); tabSettings(el); });
    U.$$('#ref-list [data-ref-finalize]', el).forEach((b) => b.onclick = async () => { const list = readReferralCards(); Store.saveReferralChallenges(list); const ok = await U.confirm('Finalize winners now? This locks the current top 3 and creates their referral discount codes.', { title: 'Finalize winners', okLabel: 'Finalize' }); if (!ok) return; const r = Store.finalizeReferralChallenge(b.dataset.refFinalize); if (!r.ok) return U.toast(r.error || 'Could not finalize', 'error'); U.toast('Winners finalized and codes generated', 'success', 'crown'); tabSettings(el); });
  ...
  function tabRequests(el) {
    let fetched = false;
    const reqCard = (title, icon, count, body) => `<section class="card reveal in" style="min-width:280px;flex:1"><div class="card-title">${ic(icon)}${title} <span class="chip" style="margin-left:auto">${count}</span></div><div style="display:flex;flex-direction:column;gap:10px;margin-top:12px;max-height:620px;overflow:auto">${body}</div></section>`;
    const smallItem = (r, kind) => `<div class="jr-item" data-kind="${kind}" data-cid="${r.cloudId || ''}" data-lid="${r.id || ''}" style="align-items:flex-start"><div class="jr-main"><div class="jr-head"><b>${U.esc(r.name || r.studentName || 'Student')}</b><span class="jr-status ${r.status === 'new' ? 'new' : 'read'}">${r.status || 'new'}</span>${r.age ? `<span class="chip">Age ${U.esc(r.age)}</span>` : ''}<span class="muted tiny">${U.timeAgo(r.createdAt)}</span></div>${r.email ? `<a class="jr-mail" href="mailto:${U.esc(r.email)}">${U.esc(r.email)}</a>` : ''}${r.phone ? `<div class="muted tiny">${U.esc(r.phone)}</div>` : ''}${r.level ? `<div class="muted tiny">Level: ${U.esc(r.level)}</div>` : ''}${r.referralCode ? `<span class="chip mono">${U.esc(r.referralCode)}</span>` : ''}${r.description ? `<div class="jr-desc">${U.esc(r.description)}</div>` : ''}${r.question ? `<div class="jr-desc"><span class="jr-q-mark">?</span>${U.esc(r.question)}</div>` : ''}${r.courseLevel ? `<div class="muted tiny">Interested in <b>${U.esc(r.courseLevel)}</b></div>` : ''}</div><div class="jr-actions"><button class="icon-btn jr-toggle" title="Mark read/new">${ic(r.status === 'new' ? 'check' : 'refresh')}</button><button class="icon-btn jr-del" title="Delete" style="color:var(--red)">${ic('trash')}</button></div></div>`;
    const render = () => {
      const joins = Store.joinRequests(); const trials = Store.trialRequests(); const next = Store.nextCourseRequests(); const qs = Store.questions();
      el.innerHTML = `<div class="page-head reveal in" style="margin-top:0"><h2>${ic('userPlus')}Requests</h2><p>Join requests, free trial requests, next-course requests and questions in one smooth dashboard.</p><button class="btn btn-ghost btn-sm" id="req-refresh">${ic('refresh')}Refresh cloud requests</button></div><div style="display:grid;grid-template-columns:repeat(4,minmax(260px,1fr));gap:14px;align-items:start;overflow:auto">${reqCard('Join Requests','userPlus',joins.length, joins.length ? joins.map((x)=>smallItem(x,'join')).join('') : '<p class="muted tiny">No join requests.</p>')}${reqCard('Free Trial','star',trials.length, trials.length ? trials.map((x)=>smallItem(x,'trial')).join('') : '<p class="muted tiny">No free trial requests.</p>')}${reqCard('Next Course','star',next.length, next.length ? next.map((x)=>smallItem(x,'next')).join('') : '<p class="muted tiny">No next-course requests.</p>')}${reqCard('Questions','help',qs.length, qs.length ? qs.map((x)=>smallItem(x,'q')).join('') : '<p class="muted tiny">No questions.</p>')}</div>`;
      U.$('#req-refresh', el).onclick = () => { fetched = false; start(); };
      U.$$('[data-kind]', el).forEach((row) => {
        const kind = row.dataset.kind; const id = row.dataset.cid || row.dataset.lid;
        const find = (list) => list.find((x) => String(x.cloudId || x.id || '') === id);
        const item = kind === 'join' ? find(joins) : kind === 'trial' ? find(trials) : kind === 'next' ? find(next) : find(qs);
        if (!item) return;
        row.querySelector('.jr-toggle').onclick = async () => { if (kind === 'join') await Store.setJoinStatus(item, item.status === 'new' ? 'read' : 'new'); else if (kind === 'q') await Store.setQuestionStatus(item, item.status === 'new' ? 'read' : 'new'); else if (kind === 'next') await Store.setNextCourseStatus(item, item.status === 'new' ? 'read' : 'new'); else Store.setTrialStatus(item.id, item.status === 'new' ? 'accepted' : 'new'); render(); };
        row.querySelector('.jr-del').onclick = async () => { if (!await U.confirm('Delete this request?', { title: 'Delete request', danger: true, okLabel: 'Delete' })) return; if (kind === 'join') await Store.deleteJoinRequest(item); else if (kind === 'q') await Store.deleteQuestion(item); else if (kind === 'next') await Store.deleteNextCourseRequest(item); else { Store.setTrialStatus(item.id, 'deleted'); Store.trialRequests().splice(Store.trialRequests().indexOf(item), 1); Store.save(); } render(); };
      });
    };
    const start = async () => { render(); if (fetched) return; fetched = true; if (Store.cloudEnabled && Store.cloudEnabled() && Store.cloudStatus().authed) { await Promise.allSettled([Store.fetchJoinRequests(), Store.fetchQuestions(), Store.fetchNextCourseRequests()]); render(); } };
    start();
  }
