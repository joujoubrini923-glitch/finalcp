...
(function () {
  ...
  PV.problems = (root) => {
    ...
    const ME_KEY = 'acp_me';
    let me = null;
    try { me = JSON.parse(sessionStorage.getItem(ME_KEY) || 'null'); } catch (e) { me = null; }
      ...
      U.$('#problems-table', root).innerHTML = list.length ? `<table class="table">
        ...
        <tbody>${list.map((p) => `
          <tr${meSolved.has(p.id) ? ' class="row-solved"' : ''}>
            <td><a class="link-accent" href="${U.safeURL(p.link)}" target="_blank" rel="noopener">${U.esc(p.name)} ${ic('external')}</a>${meSolved.has(p.id) ? ' <span class="solved-badge">✓ Solved</span>' : ''}</td>
    ...
    const renderMeBar = () => {
      ...
      if (me) {
        ...
        U.$('#me-forget', bar).onclick = () => {
          sessionStorage.removeItem(ME_KEY); me = null; refreshSolved(); renderMeBar(); render();
        };
      } else {
        bar.innerHTML = `
        ...
        go.onclick = async () => {
          ...
          const r = await Store.cfWhoAmI(code);
