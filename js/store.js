...
create policy "coach deletes" on public.join_requests for delete using (auth.role() = 'authenticated');`;
  ...
  Store.seasonStandings = function (season) {
    return Store.seasonMembers(season)
  ...
  Store.endSeason = function (id) {
    ...
    if (!s.champions) {
      s.champions = Store.seasonStandings(s).slice(0, 2).map((e, i) => ({ rank: i + 1, studentId: e.s.id, name: e.s.name, points: e.points }));
...
create policy "coach deletes next course requests" on public.next_course_requests for delete using (auth.role() = 'authenticated');`;
  ...
  Store.nextCourseFor = (sid) => {
    const s = Store.student(sid);
    if (!s || !db.settings.nextCourses) return null;
    const candidates = Object.entries(db.settings.nextCourses).map(([sourceGroupId, course]) => ({ sourceGroupId, course })).filter(({ sourceGroupId, course }) => {
      if (!course || course.enabled === false) return false;
      const visible = Array.isArray(course.visibleGroupIds) ? course.visibleGroupIds : [sourceGroupId];
      return visible.includes(s.groupId);
    });
    return candidates.length ? { ...candidates[0].course, sourceGroupId: candidates[0].sourceGroupId } : null;
  };
