...
(function () {
  ...
  function defaultSettings() {
    return {
      ...
      bonusWeeklyStreak: 0,    // pts per week of perfect weekly solving streak
  ...
  /* Coach-configured score bonuses (all default 0 = off):
     weekly perfect streak × N, each "2+ solves in a day" × M, mastered topic × K */
  Store.bonusPoints = function (sid) {
    const S = db.settings;
    let bonus = 0;
    if ((S.bonusWeeklyStreak | 0) > 0) bonus += (Store.weeklyStreak(sid) | 0) * S.bonusWeeklyStreak;
...
create policy "coach deletes" on public.join_requests for delete using (auth.role() = 'authenticated');`;
  ...
  /* ================================================================
     SEASONS (v1.7) — sprint competitions with a Champions Wall (top 3)
     ================================================================ */
  Store.seasons = () => db.seasons || [];
  Store.addSeason = function (data) {
    const s = { id: uid('sn'), name: String(data.name || '').trim(), start: data.start, end: data.end, champions: null };
  ...
  Store.deleteSeason = function (id) { db.seasons = (db.seasons || []).filter((s) => s.id !== id); Store.save(); };
  ...
  // points earned by a student strictly inside [start, end] (solves + contests)
  Store.seasonScore = function (sid, season) {
    const inWin = (d) => d >= season.start && d <= (season.end || '9999-12-31');
    let pts = Store.solvesOf(sid).filter((x) => inWin(x.date)).reduce((a, x) => a + (x.pts != null ? x.pts : x.problem.score || 0), 0);
    db.contests.forEach((c) => c.results.forEach((r) => {
      if (r.studentId === sid && inWin(c.date)) pts += Number(r.points) || 0;
    }));
    return pts;
  };
  ...
  // Freeze the Top 3 forever when the coach ends a season
  Store.endSeason = function (id) {
    const s = (db.seasons || []).find((x) => x.id === id);
    if (!s) return;
    if (!s.end) s.end = todayStr();
    if (!s.champions) {
      s.champions = Store.seasonStandings(s).slice(0, 3).map((e, i) => ({ rank: i + 1, studentId: e.s.id, name: e.s.name, points: e.points }));
