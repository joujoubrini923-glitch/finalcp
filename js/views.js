...
(function () {
      ...
      <section class="course-launch-card card reveal" id="level-1-course">
        <div class="course-launch-copy">
          ...
          <div class="course-actions">
            ...
            <a class="btn btn-ghost btn-sm" href="#/join#trial">${ic('star')}Free session trial</a>
          </div>
        </div>
  ...
  PV.join = (root, focus) => {
    ...
    const trialForm = U.$('#trial-form', root);
    if (trialForm) trialForm.addEventListener('submit', (e) => { e.preventDefault(); const payload = { name: U.$('#tf-name', root).value.trim(), age: U.$('#tf-age', root).value, email: U.$('#tf-email', root).value.trim(), phone: U.$('#tf-phone', root).value.trim() }; if (!payload.name || !payload.age || !emailRe.test(payload.email)) return U.toast('Name, age and valid email are required', 'error'); const r = Store.addTrialRequest(payload); if (!r.ok) return U.toast(r.error || 'Could not send trial request', 'error'); trialForm.reset(); U.$('#trial-done', root).classList.remove('hidden'); U.toast('Trial request sent', 'success', 'star'); });
  ...
  function wireSpace(root, p) {
    ...
    const renderMissions = () => {
      const s = me(); if (!s) return;
      const missions = Store.missionsFor ? Store.missionsFor(s.id) : [];
      body.innerHTML = `<div class="card reveal in"><div class="card-title">${ic('flag')}Weekly Missions <span class="chip" style="margin-left:auto">${missions.length}</span></div><p class="muted tiny" style="margin:6px 0 14px">Finish every selected problem before the deadline to earn the mission reward.</p>
        ${missions.length ? `<div style="display:flex;flex-direction:column;gap:12px">${missions.map((m) => { const st = Store.missionStatus(m); const probs = (m.problemIds || []).map((id) => Store.problem(id)).filter(Boolean); return `<article style="border:1px solid var(--border);border-radius:16px;padding:14px;background:linear-gradient(135deg,color-mix(in srgb,var(--accent) 7%,var(--card)),var(--card))"><div class="jr-head"><b>${U.esc(m.title || 'Weekly Mission')}</b><span class="chip">${st.done}/${st.total} solved</span><span class="chip" style="color:var(--gold)">+${m.reward || 0} pts</span><span class="chip">Due ${U.fmtDate(m.deadline)}</span><span class="chip">${st.complete ? '✅ Completed' : (st.expired ? 'Expired' : 'Active')}</span></div><div style="margin-top:10px;display:flex;flex-direction:column;gap:8px">${probs.map((p) => `<div class="rec-row"><a class="link-accent" href="${U.safeURL(p.link)}" target="_blank" rel="noopener">${U.esc(p.name)} ${ic('external')}</a><span class="chip">${U.esc(p.topic)}</span>${C.diffTag(p.difficulty)}<span class="chip">${Store.solvesOf(s.id).some((x) => x.problem.id === p.id) ? '✓ solved' : 'unsolved'}</span></div>`).join('')}</div></article>`; }).join('')}</div>` : '<p class="muted">No missions assigned right now.</p>'}</div>`;
    };
    const renderSubmit = () => {
