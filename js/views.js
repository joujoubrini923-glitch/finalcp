...
(function () {
  ...
  const publishedAvailabilityHTML = (availability) => {
    ...
    return `<div class="published-schedule"><small>AVAILABLE WEEKLY TIMES</small><div>${rows || '<span>Schedule to be announced</span>'}</div></div>`;
  };
  ...
  const nextCourseSpaceCardHTML = (course, index) => {
    ...
    const availability = course.availability || course.weeklyAvailability || course.schedule || {};
    return `<section class="course-launch-card next-course-space-ad card reveal in" id="next-course-ad-${index}">
      <div class="course-launch-copy">
        ...
        ${studentAvailabilityPickerHTML('next-' + index, availability)}
        <div class="course-actions"><button class="btn btn-primary btn-sm" type="button" data-next-interest="${index}">${ic('star')}I’m interested</button><a class="btn btn-ghost btn-sm" href="#/join#ask">Ask the coach</a></div>
      </div>
    ...
    const renderCoach = () => {
      ...
      if (nextCourses.length) {
        ...
        U.$$('[data-next-choice]', body).forEach((button) => button.onclick = () => {
          ...
          panel.innerHTML = nextCourseSpaceCardHTML(nextCourses[index], index);
