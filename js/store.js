...
create policy "coach deletes" on public.join_requests for delete using (auth.role() = 'authenticated');`;
  ...
  Store.certificatesOf = function (sid) {
    ...
    (db.seasons || []).filter((season) => Array.isArray(season.awards) || Array.isArray(season.champions)).forEach((season) => {
      ...
      if (award && award.rank <= 3) out.push({ id: `season_${sid}_${season.id}`, type: 'season', tier: tierForRank(award.rank), title: Number(award.rank) === 1 ? 'Winner' : Number(award.rank) === 2 ? 'Runner-up' : 'Third place', detail: `${season.name} · Season score: ${award.points} points`, date: season.end || season.start, subtitle: season.name, points: award.points });
    });
