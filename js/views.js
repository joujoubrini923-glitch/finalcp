...
(function () {
  ...
  const STUDENT_ERRORS = {
    verify: 'We could not verify your solve right now. Please try again later, or ask the coach to record it manually.',
    passport: 'Student mode is temporarily unavailable. Please try again later or ask the coach for help.',
    photo: 'Your photo could not be saved right now. Please try again later or ask the coach for help.',
    clinic: 'Your code clinic request could not be sent right now. Please try again later or send your code directly to the coach.',
    availability: 'Your availability could not be saved right now. Please try again later or tell the coach your available times.',
    join: 'Your request could not be sent right now. Please try again later or contact the coach directly.',
    hint: 'The hint could not be unlocked right now. Please try again later.',
  };
  const studentError = (result, context) => {
    const data = result && result.data || {};
    const key = data.error || result && result.error || '';
    const map = {
      'bad-code': 'Wrong secret code. Check for typos, or ask the coach for a new code.',
      'no-session': 'Your session expired. Please enter your secret code again.',
      'locked': 'Too many tries. Your code is locked for a short time — please try again later.',
      'no-code': 'This code is no longer active. Ask the coach for a new one.',
      'not-accepted': 'No <b>Accepted</b> submission was found for this problem on your Codeforces account. Solve it first, then come back!',
      'no-handle': 'Your Codeforces username is not linked yet. Ask the coach to add it to your profile.',
      'not-in-catalog': 'This problem is not in the academy library yet, so it cannot be counted. Ask the coach to add it.',
      'already-counted': 'This solve was already counted — each accepted submission can only be used once. 😉',
      'old-submission': 'This submission was made before the academy verification start date, so it cannot be counted.',
      'cf-down': 'Codeforces is not responding right now. Please try again in a few minutes.',
      'too-large': 'The file is too large. Please choose a smaller image.',
      'bad-image': 'This image format is not supported. Please upload a JPG, PNG, WebP, GIF or BMP image.',
      'limit': 'You reached your weekly code clinic limit. Wait until next week or ask the coach directly.',
      'weekly-limit': 'You reached your weekly code clinic limit. Wait until next week or ask the coach directly.',
      'missing-fields': 'Please fill in all required fields before sending.',
      'no-course': 'This course option is not available right now. Ask the coach for the next course schedule.',
      'offline': STUDENT_ERRORS[context] || 'This feature is temporarily unavailable. Please try again later.',
      'not-configured': STUDENT_ERRORS[context] || 'This feature is temporarily unavailable. Please try again later.',
    };
    if (map[key]) return map[key];
    const msg = String(data.message || result && result.error || '');
    if (/supabase|edge function|sql|step|deploy|cloud|service key|function/i.test(msg)) return STUDENT_ERRORS[context] || 'This feature is temporarily unavailable. Please try again later.';
    return msg ? U.esc(msg) : (STUDENT_ERRORS[context] || 'Something went wrong. Please try again later.');
  };
  ...
  C.activityIcon = (type) => ({
    ...
    group: ['layers', 'var(--accent)'], problem: ['target', 'var(--gold)'],
      ...
      <section class="hero">
        <div>
          ...
          <div class="hero-ctas">
            ...
            <a class="btn btn-ghost" href="#/trial">${ic('star')}Free session trial</a>
          </div>
          <div class="hero-mini">
      ...
      <section class="course-launch-card card reveal" id="level-1-course">
        <div class="course-launch-copy">
          ...
          <div class="course-actions">
            ...
            <a class="btn btn-ghost btn-sm" href="#/trial">${ic('star')}Free session trial</a>
          </div>
        </div>
  ...
  PV.join = (root, focus) => {
      ...
      <section class="grid grid-2 join-grid" id="apply">
        ...
        <div class="card reveal">
          ...
          <form id="join-form" style="margin-top:14px">
            ...
            <label class="field"><span>Referral discount code <span class="muted tiny">(optional)</span></span><input class="input mono" id="jf-ref" maxlength="40" placeholder="YA-30-OCT26"></label>
    ...
    const wire = (formId, honeyId, doneId, build, send, toastMsg, toastIcon) => {
      ...
      form.addEventListener('submit', async (e) => {
        ...
        if (!r.ok) return U.toast(studentError(r, formId === 'join-form' ? 'join' : 'join'), 'error');
        form.reset();
    ...
    wire('join-form', 'jf-web', 'join-done',
      () => ({
        ...
        description: U.$('#jf-desc', root).value, referralCode: U.$('#jf-ref', root).value.trim().toUpperCase(),
    ...
    if (focus === 'ask' || focus === 'apply' || focus === 'trial') {
      setTimeout(() => {
  ...
  function submitBlockHTML(p) {
    ...
    <section class="grid grid-2 join-grid">
      ...
      <div class="card reveal in">
        ...
        ${cloud ? '' : `<div class="vf-offline">${ic('warn')}<span>Solve verification is temporarily unavailable. Please try again later, or ask the coach to record your solve manually.</span></div>`}
  ...
  function wireVerifyForm(root, p) {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      const msgs = {
        ...
        'offline': STUDENT_ERRORS.verify,
      };
      show(false, `${ic('warn')}<span>${msgs[d.error] || studentError(r, 'verify')}</span>`);
    });
  }
  ...
  C.passportGate = (root, html) => {
    ...
    <div class="container page">
      <div class="passport-card card reveal in">
        ...
        <div class="vf-offline" style="margin-top:16px">${ic('warn')}<span>Student mode is temporarily unavailable. Please try again later or ask the coach for help.</span></div>`}
      </div>
    </div>`;
  };
  C.wirePassportGate = (root, done) => {
    ...
    go.onclick = async () => {
      ...
      if (!r.ok) {
        ...
        err.innerHTML = m[r.data && r.data.error] || studentError(r, 'passport');
  ...
  /* ======================================================= FREE TRIAL */
  PV.trial = (root) => {
    root.innerHTML = `<div class="container page"><div class="page-head reveal in"><h1>${ic('star')}Free Session Trial</h1><p>Request one free first Level 1 session. If the student continues after the trial, they join the paid course.</p></div>
      <section class="grid grid-2 join-grid"><div class="card join-pitch reveal in"><div class="jp-badge jp-badge-q">${ic('star')}</div><h3>Try before joining.</h3><p class="muted">The coach reviews every request and will contact you with the next step.</p><ul class="jp-list"><li>${ic('check')}Free if the student does not continue</li><li>${ic('check')}Level 1 only</li><li>${ic('check')}Name, age and email required</li></ul></div><div class="card reveal in"><div class="card-title">${ic('star')}Request free trial</div><form id="trial-form" style="margin-top:14px"><div class="field-row"><label class="field"><span>Student name *</span><input class="input" id="tf-name" maxlength="60"></label><label class="field" style="max-width:110px"><span>Age *</span><input class="input" id="tf-age" type="number" min="4" max="120"></label></div><label class="field"><span>Email *</span><input class="input" id="tf-email" type="email" maxlength="90"></label><label class="field"><span>Phone <span class="muted tiny">(optional)</span></span><input class="input" id="tf-phone" maxlength="40"></label><div class="join-done hidden" id="trial-done">${ic('check')}<span>Trial request sent! The coach will review it.</span></div><button class="btn btn-primary btn-block" type="submit">${ic('arrowRight')}Send trial request</button></form></div></section></div>`;
    U.$('#trial-form', root).addEventListener('submit', (e) => { e.preventDefault(); const payload = { name: U.$('#tf-name', root).value.trim(), age: U.$('#tf-age', root).value, email: U.$('#tf-email', root).value.trim(), phone: U.$('#tf-phone', root).value.trim() }; if (!payload.name || !payload.age || !/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/i.test(payload.email)) return U.toast('Name, age and valid email are required', 'error'); const r = Store.addTrialRequest(payload); if (!r.ok) return U.toast(r.error || 'Could not send trial request', 'error'); e.target.reset(); U.$('#trial-done', root).classList.remove('hidden'); U.toast('Trial request sent', 'success', 'star'); });
  };

  /* ======================================================= PARENT MODE */
  PV.parent = (root) => {
    let sid = null;
    try { sid = sessionStorage.getItem('acp_parent_sid') || null; } catch (e) {}
    const renderReport = () => {
      const s = sid && Store.student(sid);
      if (!s) { try { sessionStorage.removeItem('acp_parent_sid'); } catch (e) {} return renderGate(); }
      const solves = Store.solvesOf(s.id).slice().reverse();
      const mastery = Store.mastery(s.id).slice().sort((a, b) => b.xp - a.xp).slice(0, 5);
      const certs = Store.certificatesOf(s.id);
      const att = Store.attendanceFor ? Store.attendanceFor(s.id) : [];
      const present = att.filter((x) => x.marks && x.marks[s.id] === 'present').length;
      const rank = (Store.leaderboard('') || []).find((e) => e.s.id === s.id);
      root.innerHTML = `<div class="container page"><div class="page-head reveal in" style="display:flex;align-items:center;gap:14px;flex-wrap:wrap">${U.avatarHTML(s, 'avatar-56')}<div style="flex:1"><h1>${ic('users')}Parent Report</h1><p>Auto-updating progress report for <b>${U.esc(s.name)}</b>.</p></div><button class="btn btn-ghost btn-sm" id="parent-logout">${ic('logout')}Logout</button></div>
        <div class="stats-row"><div class="stat-tile"><div class="st-ic">${ic('trend')}</div><div class="st-val">${U.fmtNum(Store.score(s.id))}</div><div class="st-label">Score</div></div><div class="stat-tile"><div class="st-ic">${ic('check')}</div><div class="st-val">${Store.solvedCount(s.id)}</div><div class="st-label">Problems solved</div></div><div class="stat-tile"><div class="st-ic">${ic('trophy')}</div><div class="st-val">${rank ? '#' + rank.rank : '—'}</div><div class="st-label">Leaderboard rank</div></div><div class="stat-tile"><div class="st-ic">${ic('calendar')}</div><div class="st-val">${present}/${att.length}</div><div class="st-label">Attendance</div></div></div>
        <div class="grid grid-2" style="margin-top:16px"><div class="card"><div class="card-title">${ic('target')}Recent solved problems</div>${solves.length ? solves.slice(0, 8).map((x) => `<div class="rec-row"><b>${U.esc(x.problem.name)}</b><span class="chip">${U.fmtDate(x.date)}</span>${C.diffTag(x.problem.difficulty)}</div>`).join('') : '<p class="muted tiny">No solves yet.</p>'}</div><div class="card"><div class="card-title">${ic('award')}Strong topics</div>${mastery.length ? mastery.map((m) => `<div class="rec-row"><b>${U.esc(m.name)}</b><span class="chip">${m.xp}/${m.need} XP</span><span class="chip">${m.done ? 'mastered' : 'training'}</span></div>`).join('') : '<p class="muted tiny">No topic progress yet.</p>'}</div></div>
        <div class="grid grid-2" style="margin-top:16px"><div class="card"><div class="card-title">${ic('calendar')}Attendance</div>${att.length ? att.map((a) => `<div class="rec-row"><b>${U.esc(a.title || 'Session')}</b><span class="chip">${U.fmtDate(a.date)}</span><span class="chip">${a.marks[s.id] === 'present' ? 'Present' : 'Absent'}</span></div>`).join('') : '<p class="muted tiny">No attendance recorded yet.</p>'}</div><div class="card"><div class="card-title">${ic('award')}Certificates & badges</div>${certs.length ? certs.slice(0, 6).map((c) => `<div class="rec-row"><b>${U.esc(c.title)}</b><span class="chip">${U.fmtDate(c.date)}</span></div>`).join('') : '<p class="muted tiny">No certificates yet.</p>'}</div></div>
      </div>`;
      U.$('#parent-logout', root).onclick = () => { sessionStorage.removeItem('acp_parent_sid'); sid = null; renderGate(); };
    };
    const renderGate = () => {
      root.innerHTML = `<div class="container page"><div class="card reveal in" style="max-width:560px;margin:auto;text-align:center"><div class="login-lock">${ic('users')}</div><h1>Parent Mode</h1><p class="muted">Enter the private parent code given by the coach to open your child's report.</p><input class="input mono" id="parent-code" placeholder="Parent code" style="text-align:center;text-transform:uppercase"><div style="height:12px"></div><button class="btn btn-primary" id="parent-go">${ic('arrowRight')}Open report</button><p class="muted tiny" style="margin-top:12px">Ask the coach if you do not have a parent code.</p></div></div>`;
      U.$('#parent-go', root).onclick = () => { const r = Store.parentLogin(U.$('#parent-code', root).value); if (!r.ok) return U.toast('Wrong parent code — check with the coach.', 'error'); sid = r.studentId; sessionStorage.setItem('acp_parent_sid', sid); renderReport(); };
    };
    sid ? renderReport() : renderGate();
  };
  ...
  function spaceHTML(p) {
    ...
    <div class="container page">
      ...
      <div class="lb-tabs reveal in" style="margin-bottom:16px" id="sp-tabs">
        ...
        <button class="btn btn-sm btn-ghost" data-t="unsolved">${ic('target')}Unsolved Problems</button>
        <button class="btn btn-sm btn-ghost" data-t="rules">${ic('shield')}Scoring Rules</button>
        <button class="btn btn-sm btn-ghost" data-t="missions">${ic('flag')}Weekly Missions</button>
  ...
  function wireSpace(root, p) {
    ...
    if (photoBtn && photoInput) {
      ...
      photoInput.onchange = async () => {
        try {
          ...
          if (!r.ok) return U.toast(studentError(r, 'photo'), 'error');
    ...
    let spHintsMine = {}, spHintsAvail = {}, spHintsCuts = {};
    const loadSpaceHints = async () => {
      if (!Store.cloudEnabled || !Store.cloudEnabled() || !Store.hintsMine) return;
      const r = await Store.hintsMine();
      if (r.ok && r.data) { spHintsMine = r.data.hints || {}; spHintsAvail = r.data.available || {}; spHintsCuts = r.data.cuts || {}; }
    };
    const spaceHintStrip = (prob, solvedSet) => {
      if (solvedSet.has(prob.id) || !spHintsAvail[prob.id]) return '';
      const S = Store.settings(); const cuts = S.hintCuts || [10, 15, 20]; const bits = [];
      for (let i = 0; i < spHintsAvail[prob.id]; i++) {
        if (spHintsMine[prob.id] && spHintsMine[prob.id][i] != null) bits.push(`<span class="hint-open">${ic('zap')}<b>Hint ${i + 1}:</b> ${U.esc(spHintsMine[prob.id][i])}</span>`);
        else { const cut = (spHintsCuts[prob.id] && spHintsCuts[prob.id][i] != null) ? spHintsCuts[prob.id][i] : cuts[i]; bits.push(`<button class="hint-lock" data-sp-hint="${prob.id}|${i}" title="Unlocking this hint lowers the points this problem can earn you">${ic('lock')}Hint ${i + 1} <span class="hint-cost">−${cut} pts</span></button>`); }
      }
    ...
    const renderCoach = () => {
      ...
      const nextBest = recList[0] || recs[0];
      const scoreNow = Store.score(s.id);
      const globalAbove = Store.leaderboard('').filter((e) => e.s.id !== s.id && e.score > scoreNow).sort((a, b) => a.score - b.score)[0];
      const groupAbove = Store.leaderboard(s.groupId).filter((e) => e.s.id !== s.id && e.score > scoreNow).sort((a, b) => a.score - b.score)[0];
      const rivalBox = (title, rival) => rival ? `<div class="rec-row"><span>${ic('users')}<b>${title}</b>: ${U.esc(rival.s.name)}</span><span class="chip">${U.fmtNum(rival.score - scoreNow)} pts ahead</span></div>` : `<div class="rec-row"><span>${ic('crown')}<b>${title}</b></span><span class="chip">You are on top</span></div>`;
      body.innerHTML = `
        ...
        <div class="grid grid-2" style="margin-top:16px">
          <div class="card reveal in">
            <div class="card-title">${ic('target')}Next Best Problem</div>
            <p class="muted tiny" style="margin:6px 0 12px">Press the button when you want one clear target.</p>
            <button class="btn btn-primary btn-sm" id="next-best-btn">${ic('zap')}Give me my next problem</button>
            <div id="next-best-result" style="margin-top:12px"></div>
          </div>
          <div class="card reveal in">
            <div class="card-title">${ic('users')}Rival Mode</div>
            <div style="display:flex;flex-direction:column;gap:10px;margin-top:10px">${rivalBox('Nearest above globally', globalAbove)}${rivalBox('Nearest above in your group', groupAbove)}</div>
          </div>
        </div>
        ${nextCourseChoices}
      `;
      const nb = U.$('#next-best-btn', body);
      if (nb) nb.onclick = () => { const out = U.$('#next-best-result', body); out.innerHTML = nextBest ? `<div class="rec-row"><a class="link-accent" href="${U.safeURL(nextBest.link)}" target="_blank" rel="noopener"><b>${U.esc(nextBest.name)}</b> ${ic('external')}</a><span class="chip">${U.esc(nextBest.topic)}</span>${C.diffTag(nextBest.difficulty)}</div>` : '<p class="muted tiny">No unsolved problems available.</p>'; };
      const wireNextCourse = (scope, course, index) => {
        ...
        interest.onclick = async () => {
          ...
          if (!r.ok) return U.toast(studentError(r, 'availability'), 'error');
    ...
    const renderUnsolved = async () => {
      const s = me(); if (!s) return;
      await loadSpaceHints();
      const topics = Store.topics().map((t) => t.name);
      body.innerHTML = `
        <div class="filter-bar reveal in">
          <input class="input" id="us-q" placeholder="Search unsolved problems…">
          <select class="select" id="us-topic"><option value="">All topics</option>${topics.map((t) => `<option>${U.esc(t)}</option>`).join('')}</select>
          <select class="select" id="us-diff"><option value="">All difficulties</option><option value="easy">Easy</option><option value="medium">Medium</option><option value="hard">Hard</option></select>
          <label class="chip" style="cursor:pointer;gap:8px"><input type="checkbox" id="us-all"> All academy problems</label>
          <span class="spacer"></span><span class="chip" id="us-count"></span>
        </div>
        <div class="card reveal in"><div class="table-wrap" id="us-table"></div></div>`;
      const solvedSet = new Set(Store.solvesOf(s.id).map((x) => x.problem.id));
      const diffOrder = { easy: 0, medium: 1, hard: 2 };
      const draw = () => {
        const q = U.$('#us-q', body).value.trim().toLowerCase(); const tp = U.$('#us-topic', body).value; const df = U.$('#us-diff', body).value; const all = U.$('#us-all', body).checked;
        let list = Store.unsolvedProblemsOf(s.id, all).sort((a, b) => (diffOrder[a.difficulty] - diffOrder[b.difficulty]) || (a.score - b.score) || a.name.localeCompare(b.name));
        if (q) list = list.filter((p) => p.name.toLowerCase().includes(q)); if (tp) list = list.filter((p) => p.topic === tp); if (df) list = list.filter((p) => p.difficulty === df);
        U.$('#us-count', body).textContent = list.length + ' unsolved';
        U.$('#us-table', body).innerHTML = list.length ? `<table class="table"><thead><tr><th>Problem</th><th>Topic</th><th>Difficulty</th><th>Score</th><th>XP</th><th>Groups</th><th>Open</th></tr></thead><tbody>${list.map((p) => `<tr><td><a class="link-accent" href="${U.safeURL(p.link)}" target="_blank" rel="noopener">${U.esc(p.name)} ${ic('external')}</a>${spaceHintStrip(p, solvedSet)}</td><td class="muted">${U.esc(p.topic)}</td><td>${C.diffTag(p.difficulty)}</td><td class="num">${p.score}</td><td class="num" style="color:var(--accent-2)">${p.xp}</td><td>${(p.groupIds || []).map((gid) => { const g = Store.group(gid); return g ? `<span class="chip" style="margin:1px 3px 1px 0;padding:3px 9px;font-size:.72rem">${U.esc(g.name)}</span>` : ''; }).join('') || '<span class="muted">—</span>'}</td><td><a class="btn btn-ghost btn-sm" href="${U.safeURL(p.link)}" target="_blank" rel="noopener">Open Codeforces</a></td></tr>`).join('')}</tbody></table>` : `<div style="padding:10px">${C.empty({ icon: 'check', title: 'You cleared all your assigned problems 🔥 Ask the coach for a harder set.', text: '' })}</div>`;
      };
      ['us-q', 'us-topic', 'us-diff', 'us-all'].forEach((id) => U.$('#' + id, body).addEventListener('input', draw));
      U.$('#us-table', body).addEventListener('click', async (e) => {
        const b = e.target.closest('[data-sp-hint]'); if (!b) return;
        const [pid, idxS] = b.dataset.spHint.split('|'); const idx = +idxS;
        const okGo = await U.confirm('Unlock this hint? If you later solve this problem, it earns fewer points.', { title: 'Spend score for a hint?', okLabel: 'Unlock hint' }); if (!okGo) return;
        b.disabled = true; const r = await Store.hintUnlock(pid, idx); b.disabled = false;
        if (!r.ok) return U.toast(studentError(r, 'hint'), 'error');
        spHintsMine[pid] = spHintsMine[pid] || {}; spHintsMine[pid][idx] = r.data.text; U.toast(`Hint unlocked — this problem now earns ${r.data.earns} pts`, 'info', 'zap'); draw();
      });
      draw();
    };
    const renderRules = () => {
      const s = me(); if (!s) return; const S = Store.settings(); const challenges = Store.visibleReferralChallengesFor ? Store.visibleReferralChallengesFor(s.id) : [];
      const challengeHTML = challenges.length ? challenges.map((c) => {
        const standings = Store.referralStandings(c); const mine = standings.find((x) => x.s.id === s.id); const third = standings[2];
        const gap = third && mine && mine.rank > 3 ? Math.max(0, third.score - mine.score + 1) : 0; const win = (c.winners || []).find((w) => w.studentId === s.id);
        const discounts = c.discounts || [30, 20, 10];
        const scope = c.groupId ? U.esc((Store.group(c.groupId) || {}).name || 'Selected group') : 'Global leaderboard';
        const dateLabel = `${U.fmtDate(c.start)} → ${c.end ? U.fmtDate(c.end) : 'open'}`;
        const distLabel = mine && mine.rank <= 3 ? 'You are currently in the prize zone' : (gap ? `${U.fmtNum(gap)} pts from Rank #3` : 'Climb into the Top 3');
        const prizeCard = (rank, pct, color, label, lift) => `<div style="flex:1;min-width:150px;transform:translateY(${lift}px);border:1px solid color-mix(in srgb,${color} 58%,var(--border));background:linear-gradient(180deg,color-mix(in srgb,${color} 22%,var(--card)),var(--card));border-radius:22px;padding:16px 14px;text-align:center;box-shadow:0 18px 35px rgba(0,0,0,.22)">
          <div style="display:inline-grid;place-items:center;width:42px;height:42px;border-radius:14px;background:color-mix(in srgb,${color} 20%,transparent);color:${color};margin-bottom:8px">${ic(rank === 1 ? 'crown' : 'trophy')}</div>
          <b style="display:block;color:${color};font-size:.9rem;letter-spacing:.08em;text-transform:uppercase">Rank #${rank}</b>
          <div style="font-size:2.25rem;line-height:1;font-weight:950;color:${color};margin:8px 0 5px">${pct}%</div>
          <small class="muted">${label}</small>
        </div>`;
        return `<section class="card reveal in" style="position:relative;overflow:hidden;margin-bottom:18px;padding:0;border-color:color-mix(in srgb,var(--accent) 34%,var(--border));background:radial-gradient(circle at 94% 8%,color-mix(in srgb,var(--accent) 16%,transparent),transparent 38%),linear-gradient(135deg,color-mix(in srgb,var(--accent) 8%,var(--card)),var(--card) 56%,color-mix(in srgb,var(--gold) 7%,var(--card)))">
          <div style="position:absolute;right:-52px;top:-54px;font-size:10rem;opacity:.055;color:var(--gold)">🏆</div>
          <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:18px;padding:24px;align-items:center;position:relative;z-index:1">
            <div>
              <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:12px"><span class="chip" style="color:var(--gold);border-color:color-mix(in srgb,var(--gold) 50%,var(--border));background:color-mix(in srgb,var(--gold) 9%,var(--card))">${ic('crown')}${U.esc(c.title || 'Leaderboard Discount Challenge')}</span><span class="chip">${scope}</span></div>
              <h2 style="margin:0 0 10px;font-size:clamp(1.55rem,3.6vw,2.55rem);line-height:1.05;letter-spacing:-.03em">Win a discount for your friend.</h2>
              <p class="muted" style="margin:0 0 16px;max-width:58ch;line-height:1.6">Finish in the <b>Top 3</b> when the challenge ends. You get a one-use discount code for <b>one invited friend</b> joining <b>Level 1</b>.</p>
              <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:16px"><span class="chip">${ic('calendar')}${dateLabel}</span><span class="chip">${ic('trend')}Total score counts</span><span class="chip">${ic('userPlus')}One friend only</span></div>
              <div style="border:1px solid color-mix(in srgb,var(--gold) 45%,var(--border));background:color-mix(in srgb,var(--gold) 8%,var(--card));border-radius:18px;padding:14px 16px;display:flex;align-items:center;gap:14px;flex-wrap:wrap">
                <div style="width:44px;height:44px;border-radius:14px;background:color-mix(in srgb,var(--gold) 16%,transparent);display:grid;place-items:center;color:var(--gold)">${ic('trend')}</div>
                <div style="flex:1;min-width:180px"><b style="display:block;color:var(--gold)">${distLabel}</b><small class="muted">Your rank: ${mine ? '#' + mine.rank + ' · ' + U.fmtNum(mine.score) + ' pts' : 'not ranked yet'}</small></div>
              </div>
            </div>
            <div style="display:flex;gap:12px;align-items:flex-end;justify-content:center;flex-wrap:wrap;padding-top:12px">
              ${prizeCard(1, discounts[0] || 0, 'var(--gold)', 'Champion discount', -10)}
              ${prizeCard(2, discounts[1] || 0, 'var(--green)', 'Runner-up discount', 6)}
              ${prizeCard(3, discounts[2] || 0, 'var(--accent)', 'Third place discount', 14)}
            </div>
          </div>
          ${win ? `<div style="margin:0 24px 24px;padding:14px 16px;border:1px solid color-mix(in srgb,var(--green) 55%,var(--border));border-radius:18px;background:color-mix(in srgb,var(--green) 11%,var(--card));display:flex;gap:12px;align-items:center;position:relative;z-index:1">${ic('check')}<span>You won! Your friend code is <b class="mono" style="color:var(--green)">${U.esc(win.code)}</b> — ${win.discount}% discount for one friend in Level 1. Status: ${win.used ? 'used' : 'valid until used'}.</span></div>` : ''}
        </section>`;
      }).join('') : `<section class="card reveal in" style="margin-bottom:16px;background:linear-gradient(135deg,color-mix(in srgb,var(--gold) 8%,var(--card)),var(--card));border-color:color-mix(in srgb,var(--gold) 32%,var(--border))"><div class="card-title" style="color:var(--gold)">${ic('crown')}Leaderboard Discount Challenge</div><p class="muted tiny">No referral discount challenge is visible for you right now.</p></section>`;
      const rules = [
        { key: 'problems', icon: 'target', color: 'var(--accent)', title: 'Problem difficulty points', value: ['easy','medium','hard'].map((d) => `${d}: ${S.difficultyScores[d] || 0} pts`).join(' · '), text: 'Every solved academy problem adds score using its difficulty value. Topic XP grows automatically from the same solve.' },
        { key: 'daily', icon: 'zap', color: 'var(--gold)', title: 'Daily solve bonuses', value: Store.dailyBonusTiers().map((t) => `${t.count}+ solves: +${t.points}`).join(' · ') || 'No daily bonus active', text: 'Daily bonuses are cumulative. If you reach a higher tier, you also receive all smaller tiers for that day.' },
        { key: 'streak', icon: 'flame', color: 'var(--gold)', title: 'Weekly streak bonus', value: `+${S.bonusWeeklyStreak || 0} pts`, text: 'Solving in consecutive weeks rewards consistency and keeps your training rhythm alive.' },
        { key: 'mastery', icon: 'award', color: 'var(--green)', title: 'Topic mastery bonus', value: `+${S.bonusTopicMastery || 0} pts`, text: 'When your topic XP reaches the mastery target, the topic becomes mastered and awards its bonus.' },
        { key: 'contests', icon: 'trophy', color: 'var(--gold)', title: 'Contest points', value: `Top ranks: ${(S.contestPoints || []).join(' / ')} · later ranks: +${S.contestPointsBeyond || 0}`, text: 'Coach-recorded contest results add points to your score according to your rank.' },
        { key: 'hints', icon: 'lock', color: 'var(--red)', title: 'Hint penalty', value: `Costs: ${(S.hintCuts || []).join(' / ')} · floor: ${S.hintFloor || 0}`, text: 'Hints help you continue, but unlocked hints reduce the points earned from that problem without going below the minimum score.' },
        { key: 'levels', icon: 'shield', color: 'var(--accent-2)', title: 'Level requirements', value: `${(S.levels || []).length} levels`, text: 'Levels unlock when your score, mastered topics, and achievements meet the current requirement.' },
      ];
      body.innerHTML = `${challengeHTML}<section class="card reveal in"><div class="card-title" style="font-size:1.2rem">${ic('shield')}Scoring Rules</div><p class="muted tiny" style="margin:6px 0 16px">Descriptions stay fixed; only the live numbers below change when the coach updates scoring settings. New bonus rules apply from ${U.fmtDate(S.newScoringStart || '2026-09-06')} onward.</p><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:12px">${rules.map((r) => `<article style="border:1px solid color-mix(in srgb,${r.color} 35%,var(--border));background:linear-gradient(160deg,color-mix(in srgb,${r.color} 12%,var(--card)),var(--card));border-radius:18px;padding:16px;position:relative;overflow:hidden"><div style="position:absolute;right:-12px;top:-18px;font-size:4.6rem;opacity:.08">${ic(r.icon)}</div><div style="width:42px;height:42px;border-radius:14px;display:grid;place-items:center;background:color-mix(in srgb,${r.color} 18%,transparent);color:${r.color};margin-bottom:10px">${ic(r.icon)}</div><h3 style="margin:0 0 7px;font-size:1rem">${U.esc(r.title)}</h3><div class="chip" style="margin-bottom:10px;border-color:color-mix(in srgb,${r.color} 50%,var(--border));color:${r.color}">${U.esc(r.value)}</div><p class="muted tiny" style="margin:0;line-height:1.55">${U.esc(r.text)}</p></article>`).join('')}</div></section>`;
    };
    const renderClinic = async () => {
      body.innerHTML = '<p class="muted tiny">Loading your cases…</p>';
      const S = Store.settings();
      const r = await Store.clinicMine();
      if (!r.ok) { body.innerHTML = `<div class="vf-result vf-err">${ic('warn')}<span>${studentError(r, 'clinic')}</span></div>`; return; }
      ...
      U.$('#cl-form', body).addEventListener('submit', async (e) => {
        ...
        if (!r.ok) { err.innerHTML = studentError(r, 'clinic'); err.style.display = 'flex'; return; }
    ...
    const renderMissions = () => {
      ...
      const draw = () => {
        const missions = (Store.missionsFor ? Store.missionsFor(s.id) : []).slice().sort((a, b) => (b.createdAt || '').localeCompare(a.createdAt || ''));
        const solvedSet = new Set(Store.solvesOf(s.id).map((x) => x.problem.id));
        const banner = (m, idx) => {
          const st = Store.missionStatus(m);
          const isOpen = openMissionId === m.id;
          const probs = (m.problemIds || []).map((id) => Store.problem(id)).filter(Boolean);
          const stateColor = st.complete ? 'var(--green)' : (st.expired ? 'var(--red)' : 'var(--accent)');
          const glow = idx % 3 === 0 ? 'var(--accent)' : idx % 3 === 1 ? 'var(--gold)' : 'var(--accent-2)';
          return `<article class="reveal in" style="position:relative;overflow:hidden;border:1px solid color-mix(in srgb,${glow} 45%,var(--border));border-radius:22px;padding:16px;background:linear-gradient(135deg,color-mix(in srgb,${glow} 16%,var(--card)),color-mix(in srgb,var(--bg) 70%,var(--card)));box-shadow:0 18px 45px rgba(0,0,0,.18)">
            <div style="position:absolute;right:-22px;top:-30px;font-size:7rem;opacity:.07">${ic('flag')}</div>
            <div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap;position:relative;z-index:1">
              <div style="width:54px;height:54px;border-radius:18px;display:grid;place-items:center;background:color-mix(in srgb,${glow} 20%,transparent);color:${glow};box-shadow:inset 0 0 0 1px color-mix(in srgb,${glow} 40%,transparent)">${ic('flag')}</div>
              <div style="flex:1;min-width:210px">
                <h3 style="margin:0 0 6px;font-size:1.08rem">${U.esc(m.title || 'Weekly Mission')}</h3>
                <div style="display:flex;gap:8px;flex-wrap:wrap"><span class="chip" style="color:${stateColor};border-color:color-mix(in srgb,${stateColor} 45%,var(--border))">${st.complete ? '✅ Completed' : (st.expired ? 'Expired' : 'Active')}</span><span class="chip">Due ${U.fmtDate(m.deadline)}</span></div>
              </div>
              <div style="display:flex;gap:10px;align-items:stretch;flex-wrap:wrap">
                <div style="min-width:112px;text-align:center;border-radius:16px;padding:10px 12px;background:color-mix(in srgb,var(--accent) 10%,transparent);border:1px solid color-mix(in srgb,var(--accent) 35%,var(--border))"><b style="display:block;font-size:1.25rem;color:var(--accent)">${st.done}/${st.total}</b><small class="muted">solved</small></div>
                <div style="min-width:128px;text-align:center;border-radius:16px;padding:10px 12px;background:color-mix(in srgb,var(--gold) 12%,transparent);border:1px solid color-mix(in srgb,var(--gold) 45%,var(--border))"><b style="display:block;font-size:1.25rem;color:var(--gold)">+${m.reward || 0}</b><small class="muted">reward points</small></div>
                <button class="btn btn-primary btn-sm" type="button" data-open-mission="${U.esc(m.id)}">${isOpen ? ic('chevronRight') + 'Close mission' : ic('arrowRight') + 'Open mission'}</button>
              </div>
            </div>
            ${isOpen ? `<div style="margin-top:14px;display:flex;flex-direction:column;gap:9px;position:relative;z-index:1">${probs.length ? probs.map((prob) => { const solved = solvedSet.has(prob.id); return `<div class="rec-row" style="background:var(--bg-2)"><a class="link-accent" href="${U.safeURL(prob.link)}" target="_blank" rel="noopener"><b>${U.esc(prob.name)}</b> ${ic('external')}</a><span class="chip">${U.esc(prob.topic)}</span>${C.diffTag(prob.difficulty)}<span class="chip" style="color:${solved ? 'var(--green)' : 'var(--dim)'};border-color:${solved ? 'color-mix(in srgb,var(--green) 45%,var(--border))' : 'var(--border)'}">${solved ? '✓ solved' : 'unsolved'}</span></div>`; }).join('') : '<p class="muted tiny">No problems in this mission.</p>'}</div>` : ''}
          </article>`;
        };
        body.innerHTML = `<div class="card reveal in" style="background:linear-gradient(135deg,color-mix(in srgb,var(--accent) 8%,var(--card)),var(--card))"><div class="card-title">${ic('flag')}Weekly Missions <span class="chip" style="margin-left:auto">${missions.length}</span></div><p class="muted tiny" style="margin:6px 0 14px">Each banner is a mission. Open it to see the problems inside. Newest missions appear first.</p>
          ${missions.length ? `<div style="display:flex;flex-direction:column;gap:14px">${missions.map(banner).join('')}</div>` : '<p class="muted">No missions assigned right now.</p>'}</div>`;
        U.$$('[data-open-mission]', body).forEach((btn) => btn.onclick = () => { openMissionId = openMissionId === btn.dataset.openMission ? null : btn.dataset.openMission; draw(); });
      };
      draw();
    };
    const renderSubmit = () => {
    ...
    U.$('#sp-tabs', root).addEventListener('click', (e) => {
      ...
      ({ coach: renderCoach, unsolved: renderUnsolved, rules: renderRules, missions: renderMissions, certificates: renderCertificates, score: renderScore, submit: renderSubmit, clinic: renderClinic }[b.dataset.t] || renderCoach)();
    });
    renderCoach();
  }
  ...
  PV.problems = (root) => {
      ...
      if (me) {
        ...
        go.onclick = async () => {
          ...
          if (r.ok) {
            ...
            const m = { 'bad-code': 'Wrong code — check for typos or ask the coach.', 'locked': 'Too many tries — this code is locked for a while.', 'offline': STUDENT_ERRORS.passport };
            fail(m[r.data && r.data.error] || studentError(r, 'passport'));
          }
        };
    ...
    U.$('#problems-table', root).addEventListener('click', async (e) => {
      ...
      if (!r.ok) {
        ...
        U.toast(m[r.data && r.data.error] || studentError(r, 'hint'), 'error');
        return;
      }
