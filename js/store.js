...
`create table if not exists public.next_course_requests (
  ...
  course_level text, status text not null default 'new'
);
...
create policy "coach deletes next course requests" on public.next_course_requests for delete using (auth.role() = 'authenticated');`;
  ...
  Store.addNextCourseRequest = async function (sid) {
    ...
    const req = { studentId: s.id, studentName: s.name, groupId: s.groupId, courseLevel: course.level, status: 'new', createdAt: new Date().toISOString() };
    const client = cloudClient();
    if (client) {
      try {
        const res = await client.from('next_course_requests').insert({ student_id: req.studentId, student_name: req.studentName, group_id: req.groupId, course_level: req.courseLevel, status: req.status });
        if (res.error) throw res.error;
        return { ok: true, where: 'cloud' };
  ...
  Store.fetchNextCourseRequests = async function () {
    ...
    try {
      ...
      db.nextCourseRequests = (res.data || []).map((r) => ({ cloudId: r.id, studentId: r.student_id, studentName: r.student_name, groupId: r.group_id, courseLevel: r.course_level, status: r.status || 'new', createdAt: r.created_at })).concat(localOnly);
      Store.save();
      return { ok: true };
