...
(function () {
  ...
  const studentAvailabilityPickerHTML = (prefix, availability) => {
    const days = Store.weekdays().map((day) => {
      const aliases = [day.key, day.label.toLowerCase(), day.label.slice(0, 3).toLowerCase()];
      const ranges = availability ? (aliases.map((key) => availability[key]).find((value) => Array.isArray(value)) || []) : [];
      if (!ranges.length) return '';
      return ranges.map((range, i) => `<label class="availability-slot" data-student-av-day="${prefix}-${day.key}"><input type="checkbox" data-student-av-slot data-av-key="${day.key}" data-av-start="${U.esc(range.start)}" data-av-end="${U.esc(range.end)}"><span><b>${U.esc(day.label)}</b><small>${U.esc(range.start)}–${U.esc(range.end)}</small></span></label>`).join('');
    }).join('');
    return `<div class="availability-picker student-availability-picker"><div class="availability-picker-head"><b>Choose your availability</b><span>Select the weekly times that work for you.</span></div><div class="student-availability-grid">${days || '<div class="availability-unpublished"><b>Times will appear here</b><small>The coach has not published this advertisement’s weekly schedule yet.</small></div>'}</div></div>`;
  };
  const readStudentAvailability = (root) => {
    const out = {};
    U.$$('[data-student-av-slot]', root).forEach((input) => {
      if (!input.checked) return;
      (out[input.dataset.avKey] ||= []).push({ start: input.dataset.avStart, end: input.dataset.avEnd });
    });
    return out;
  };
  const adminAvailabilityPickerHTML = (prefix, availability) => `<div class="admin-availability-picker"><div class="availability-picker-head"><b>Available weekly times</b><span>Each advertisement has its own schedule.</span></div><div class="availability-grid">${Store.weekdays().map((day) => { const ranges = availability && Array.isArray(availability[day.key]) ? availability[day.key] : []; const r = ranges[0] || {}; const start = r.start || '08:00', end = r.end || '22:00'; const optsStart = availabilityOptions.replace(`value="${start}"`, `value="${start}" selected`); const optsEnd = availabilityEndOptions.replace(`value="${end}"`, `value="${end}" selected`); return `<div class="availability-day" data-admin-av-day="${prefix}-${day.key}" data-av-key="${day.key}"><label class="availability-day-name"><input type="checkbox" data-admin-av-enabled ${ranges.length ? 'checked' : ''}><span>${U.esc(day.label)}</span></label><div class="availability-range"><select class="select" data-admin-av-start>${optsStart}</select><span>to</span><select class="select" data-admin-av-end>${optsEnd}</select></div></div>`; }).join('')}</div></div>`;
  const wireAdminAvailabilityPicker = (root, prefix) => {
    U.$$(`[data-admin-av-day^="${prefix}-"]`, root).forEach((row) => {
      const check = U.$('[data-admin-av-enabled]', row);
      const inputs = U.$$('[data-admin-av-start], [data-admin-av-end]', row);
      inputs.forEach((input) => { input.disabled = !check.checked; });
      check.addEventListener('change', () => inputs.forEach((input) => { input.disabled = !check.checked; }));
    });
  };
  const readAdminAvailability = (root, prefix) => {
    ...
    U.$$(`[data-admin-av-day^="${prefix}-"]`, root).forEach((row) => {
      const check = U.$('[data-admin-av-enabled]', row);
      if (check && check.checked) out[row.dataset.avKey] = [{ start: U.$('[data-admin-av-start]', row).value, end: U.$('[data-admin-av-end]', row).value }];
    });
    return out;
  };
  ...
  PV.join = (root, focus) => {
    ...
    <div class="container page join-page">
      ...
      <section class="join-grid join-choice-grid" id="join-choice-grid">
        <div class="card join-option join-apply-card reveal" id="apply">
          <div class="join-option-top"><span class="join-option-kicker">JOIN IN 30 SEC</span><span class="join-option-icon">${ic('userPlus')}</span></div>
          <h2>Ready to start?</h2>
          <p class="join-option-sub">Send your details and the coach will help you choose the right starting point.</p>
          <div class="join-pitch">
            <ul class="jp-list">
              <li>${ic('check')}No experience needed</li>
              <li>${ic('check')}Clear training groups</li>
              <li>${ic('check')}Problems, contests and visible progress</li>
            </ul>
          </div>
          <form id="join-form" style="margin-top:16px">
            <div class="field-row">
            ...
            <label class="field"><span>Your level <span class="muted tiny">(optional)</span></span><input class="input" id="jf-level" maxlength="120" placeholder="Beginner, C++ learner, contest experience…"></label>
            <label class="field"><span>Anything else? <span class="muted tiny">(optional)</span></span><textarea class="input" id="jf-desc" rows="2" maxlength="600" placeholder="Your goals, experience or questions…"></textarea></label>
        ...
        <div class="card join-option join-ask-card reveal" id="ask">
          <div class="join-option-top"><span class="join-option-kicker">JUST ASK</span><span class="join-option-icon">${ic('help')}</span></div>
          <h2>Need more details?</h2>
          <p class="join-option-sub">Ask about schedules, groups, the 16-hour course or anything else before applying.</p>
          <div class="join-pitch ask-pitch">
            <ul class="jp-list jp-list-q">
              <li>${ic('check')}No application required</li>
              <li>${ic('check')}Your question goes directly to the coach</li>
              <li>${ic('check')}You can decide later</li>
            </ul>
          </div>
          <form id="question-form" style="margin-top:16px">
            <div class="field-row">
            ...
            <label class="field"><span>Your level <span class="muted tiny">(optional)</span></span><input class="input" id="qf-level" maxlength="120" placeholder="Total beginner, C++ learner…"></label>
            <label class="field" style="margin-bottom:14px"><span>Your question *</span><textarea class="input" id="qf-question" rows="3" maxlength="800" placeholder="What would you like to know?"></textarea></label>
            ...
            <button class="btn btn-ghost btn-block" type="submit">${ic('help')}Send question</button>
          </form>
        </div>
      </section>
    </div>`;
  ...
  function wireSpace(root, p) {
    ...
    const renderCoach = () => {
      ...
      const nextCourse = Store.nextCourseFor(s.id);
      const nextPeriod = nextCourse && (nextCourse.period || nextCourse.dates || nextCourse.date || 'Schedule to be announced');
      const nextDuration = nextCourse && (nextCourse.duration || nextCourse.hours || 'Duration to be announced');
      const nextAvailability = nextCourse && (nextCourse.availability || nextCourse.weeklyAvailability || nextCourse.schedule || {});
      const nextLearn = nextCourse && (Array.isArray(nextCourse.learn) ? nextCourse.learn : []).filter(Boolean);
      body.innerHTML = `
        ...
        ${nextCourse ? `
        <section class="course-launch-card next-course-space-ad card reveal in" id="next-course-ad">
          <div class="course-launch-copy">
            <span class="course-kicker"><i class="dot"></i>${U.esc(nextCourse.level)} · ${U.esc(nextCourse.status)}</span>
            <h2>${U.esc(nextCourse.title)}</h2>
            <p>${U.esc(nextCourse.description)}</p>
            <div class="course-facts"><span>${ic('calendar')}${U.esc(nextPeriod)}</span><span>${ic('clock')}${U.esc(nextDuration)}</span><span class="course-age-fact">${ic('users')}${U.esc(nextCourse.age || 'Age to be announced')}</span></div>
            ${studentAvailabilityPickerHTML('next', nextAvailability)}
            <div class="course-actions"><button class="btn btn-primary btn-sm" type="button" data-next-interest>${ic('star')}I’m interested</button><a class="btn btn-ghost btn-sm" href="#/join#ask">Ask the coach</a></div>
          </div>
          <div class="course-launch-side" aria-label="Next course highlights">
            ${(nextLearn[0] ? `<span class="course-topic-float topic-cpp">${U.esc(nextLearn[0])}</span>` : '')}
            ${(nextLearn[1] ? `<span class="course-topic-float topic-problem">${U.esc(nextLearn[1])}</span>` : '')}
            ${(nextLearn[2] ? `<span class="course-topic-float topic-cv">${U.esc(nextLearn[2])}</span>` : '')}
            <div class="course-no-experience-side">${U.esc(nextCourse.experience)}</div>
            <div class="course-hours-mark"><span>${U.esc(String(nextCourse.duration || '').replace(/[^0-9]/g, '') || '16')}</span><small>HOURS</small></div>
            <div class="course-level-label">${U.esc(nextCourse.level)}</div>
          </div>
        </section>` : ''}
      `;
      const interest = U.$('[data-next-interest]', body);
      if (interest) {
        interest.onclick = async () => {
          const availability = readStudentAvailability(body);
          ...
          const r = await Store.addNextCourseRequest(s.id, availability);
        interest.disabled = false;
        if (!r.ok) return U.toast('Could not send your interest right now.', 'error');
        interest.textContent = r.duplicate ? 'Already sent' : 'Interest sent ✓';
        U.toast(r.duplicate ? 'Your interest is already with the coach.' : 'Your interest was sent to the coach.', 'success', 'star');
        };
      }
    };
    const renderScore = () => {
    ...
    U.$('#sp-tabs', root).addEventListener('click', (e) => {
      ...
      ({ coach: renderCoach, score: renderScore, submit: renderSubmit, clinic: renderClinic }[b.dataset.t] || renderCoach)();
    });
    renderCoach();
  }
