...
// password hashing is async (PBKDF2) — covered in the async block at the end
...
// --- contests with MORE than 5 ranks ---
(function () {
  const mkS = (nm) => Store.addStudent({ name: nm, groupId: 'g_rookies' });
  const kids = ['R1', 'R2', 'R3', 'R4', 'R5', 'R6', 'R7', 'R8'].map(mkS);
  const c2 = Store.addContest({
    name: 'Big Bracket Cup', date: '2026-07-29',
    entries: kids.map((k, i) => ({ studentId: k.id, solved: 8 - i })),
  });
  assert(c2.results.length === 8, 'contest records all 8 ranks, not just top 5');
  assert(c2.results[5].points === Store.settings().contestPointsBeyond, 'rank #6 gets beyond-Top5 points: ' + c2.results[5].points);
  assert(c2.results[7].rank === 8 && c2.results[7].solved === 1, 'rank #8 baked correctly');
  const s6 = Store.score(kids[5].id);
  assert(s6 === Store.settings().contestPointsBeyond, 'beyond-points count toward student score');
  assert(Store.globalRank(kids[5].id).rank >= 1, 'beyond-rank student ranked on leaderboard');
  Store.updateSettings({ contestPointsBeyond: 40 });
  assert(Store.settings().contestPointsBeyond === 40, 'beyond points editable in settings');
  const c3 = Store.addContest({ name: 'Second Cup', date: '2026-07-30', entries: kids.slice(0, 7).map((k, i) => ({ studentId: k.id, solved: 2 })) });
  assert(c3.results[5].points === 40 && c3.results[6].points === 40, 'editing beyond value applies to new contests');
  Store.updateSettings({ contestPointsBeyond: 20 });
  kids.forEach((k) => Store.deleteStudent(k.id));
  Store.deleteContest(c2.id); Store.deleteContest(c3.id);
})();

// --- old backups without joinRequests still import fine (sync, runs first) ---
(function () {
  const dump = JSON.parse(Store.exportJSON());
  delete dump.data.joinRequests;
  const r = Store.importJSON(dump);
  assert(r.ok && Array.isArray(Store.raw().joinRequests), 'old backup without joinRequests imports & heals');
})();

// (join-request async tests run at the very end — see below)
...
// --- join requests (local / offline mode) — must be the LAST block (async, nothing swaps db after it) ---
const joinTestsDone = (async function () {
  /* ---------- password security (PBKDF2, legacy auto-migration) ---------- */
  assert(await Store.verifyPassword('admin123'), 'legacy default password verifies');
  assert(Store.usesStrongPasswordStorage() === true, 'legacy cyrb53 hash auto-upgraded to PBKDF2 on first login');
  assert((await Store.isDefaultPassword()) === true, 'default password detected as default');
  assert((await Store.changePassword('admin123', 'supersecret9')).ok === true, 'password changed to PBKDF2');
  assert(!(await Store.verifyPassword('admin123')) && (await Store.verifyPassword('supersecret9')), 'new password verifies, old one dead');
  assert((await Store.changePassword('wrong', 'nope12345')).ok === false, 'wrong old password rejected');
  assert((await Store.changePassword('supersecret9', 'admin123')).ok === false, 'changing back to the default is rejected');
  assert((await Store.changePassword('supersecret9', 'short1')).ok === false, 'min length of 8 enforced');
  assert((await Store.isDefaultPassword()) === false, 'no longer default after change');
  const pwDump = JSON.parse(Store.exportJSON());
  Store.factoryReset();
  assert(await Store.verifyPassword('admin123'), 'factory reset restores default password');
  assert(Store.importJSON(pwDump).ok === true, 'import accepted');
  assert(await Store.verifyPassword('supersecret9'), 'import restored PBKDF2 password intact');
  Store.factoryReset();

  /* ---------- join requests (public application form) ---------- */
  const j1 = await Store.addJoinRequest({ name: 'Wannabe Coder', age: '15', email: 'kid@mail.tn', level: 'beginner', description: 'i <script>alert(1)</script> love cp' });
  assert(j1.ok && j1.where === 'local', 'join request stored locally when cloud not configured');
  assert(Store.joinRequests()[0].name === 'Wannabe Coder', 'join request persisted');
  assert(!Store.joinRequests()[0].description.includes('<'), 'join description sanitized');
  assert(Store.newJoinCount() === 1, 'new count = 1');
  const j2 = await Store.addJoinRequest({ name: 'No Email', age: '', email: '', level: '', description: '' });
  assert(j2.ok === false, 'join request without email rejected');
  await Store.setJoinStatus(Store.joinRequests()[0], 'read');
  assert(Store.newJoinCount() === 0, 'mark read works');
  const bad = await Store.fetchJoinRequests();
  assert(bad.ok === false, 'fetch fails gracefully without cloud');
  await Store.deleteJoinRequest(Store.joinRequests()[0]);
  assert(Store.joinRequests().length === 0, 'join request deleted');

  /* ---------- questions (ask-the-coach form) ---------- */
  assert(Array.isArray(Store.questions()), 'questions collection exists');
  const q1 = await Store.addQuestion({ name: 'Curious Parent', age: '40', email: 'dad@mail.tn', level: '', question: 'when do trainings happen? <b>x</b>' });
  assert(q1.ok && q1.where === 'local', 'question stored locally when cloud not configured');
  assert(Store.questions()[0].question.indexOf('<') === -1, 'question text sanitized');
  assert(Store.questions()[0].age === 40, 'question age clamped & parsed');
  assert(Store.newQuestionCount() === 1, 'new question count = 1');
  const q2 = await Store.addQuestion({ name: 'Silent One', age: '', email: 's@mail.tn', level: '', question: '' });
  assert(q2.ok === false, 'question without question text rejected');
  const q3 = await Store.addQuestion({ name: 'No Mail', age: '', email: '', level: '', question: 'hello?' });
  assert(q3.ok === false, 'question without email rejected');
  await Store.setQuestionStatus(Store.questions()[0], 'read');
  assert(Store.newQuestionCount() === 0, 'question mark-read works');
  const badQ = await Store.fetchQuestions();
  assert(badQ.ok === false, 'question fetch fails gracefully without cloud');
  await Store.deleteQuestion(Store.questions()[0]);
  assert(Store.questions().length === 0, 'question deleted');
  assert(Store.questionsSQL().indexOf('create table public.questions') === 0, 'questions SQL snippet available for admin');
})();
