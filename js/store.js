...
(function () {
  ...
  const Cloud = { client: null, authed: false, empty: false, synced: false, lastError: null };
  let cloudTimer = null;
  ...
  Store.cloudStatus = () => ({ enabled: !!cloudClient(), authed: Cloud.authed, synced: Cloud.synced, error: Cloud.lastError });
  ...
  Store._cloudPush = async function () {
    ...
    try {
      const res = await client.from(CLOUD_TABLE).upsert({ id: CLOUD_ROW_ID, data: db, updated_at: new Date().toISOString() });
      if (res.error) throw res.error;
      Cloud.synced = true; Cloud.lastError = null;
    } catch (e) {
      Cloud.lastError = String(e && e.message || e);
  ...
  Store.solvesOf = function (sid) {
    ...
    if (!s) return [];
    return s.solves
      .map((sv) => ({ problem: Store.problem(sv.problemId), date: sv.date }))
      .filter((x) => x.problem)
  ...
  Store.recentSolves = function (n) {
    ...
    db.students.forEach((s) => s.solves.forEach((sv) => {
      const p = Store.problem(sv.problemId);
      if (p) all.push({ student: s, problem: p, date: sv.date });
    }));
  ...
  Store.verifyPassword = (pw) => hashPW(pw) === db.settings.passwordHash;
  Store.changePassword = function (oldPw, newPw) {
    if (!Store.verifyPassword(oldPw)) return { ok: false, error: 'Current password is incorrect.' };
    if (!newPw || newPw.length < 6) return { ok: false, error: 'New password must be at least 6 characters.' };
    db.settings.passwordHash = hashPW(newPw);
