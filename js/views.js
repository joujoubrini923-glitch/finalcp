...
(function () {
  ...
  PV.join = (root, focus) => {
    ...
    const timeOptions = Array.from({ length: 29 }, (_, i) => { const mins = 8 * 60 + i * 30; const h = String(Math.floor(mins / 60)).padStart(2, '0'); const m = String(mins % 60).padStart(2, '0'); return `<option value="${h}:${m}">${h}:${m}</option>`; }).join('');
    const endTimeOptions = timeOptions.replace('value="22:00"', 'value="22:00" selected');
    const availabilityRows = Store.weekdays().map((day) => `<div class="availability-day" data-av-day="${day.key}"><label class="availability-day-name"><input type="checkbox" data-av-enabled><span>${U.esc(day.label)}</span></label><div class="availability-range"><select class="select" data-av-start disabled>${timeOptions}</select><span>to</span><select class="select" data-av-end disabled>${endTimeOptions}</select></div></div>`).join('');
    root.innerHTML = `
        ...
        <div class="card join-option join-apply-card reveal" id="apply">
          ...
          <form id="join-form" style="margin-top:16px">
            ...
            <div class="availability-picker">
              <div class="availability-picker-head"><b>Weekly availability</b><span>Choose your free time, not specific dates.</span></div>
              <div class="availability-grid">${availabilityRows}</div>
            </div>
    ...
    U.$$('[data-av-day]', root).forEach((row) => {
      const check = U.$('[data-av-enabled]', row);
      const inputs = U.$$('[data-av-start], [data-av-end]', row);
    ...
    const readAvailability = () => {
      ...
      U.$$('[data-av-day]', root).forEach((row) => {
        ...
        out[row.dataset.avDay] = [{ start: U.$('[data-av-start]', row).value, end: U.$('[data-av-end]', row).value }];
      });
      return out;
    };
  ...
  function wireSpace(root, p) {
    ...
    const renderCoach = () => {
      ...
      if (interest) interest.onclick = async () => {
        interest.disabled = true;
        const r = await Store.addNextCourseRequest(s.id);
