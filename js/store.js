...
`create table if not exists public.next_course_requests (
  ...
  course_level text, availability jsonb not null default '{}'::jsonb, status text not null default 'new'
);
...
create policy "coach deletes next course requests" on public.next_course_requests for delete using (auth.role() = 'authenticated');`;
  ...
  Store.addNextCourseRequest = async function (sid, availability) {
    const s = Store.student(sid), course = Store.nextCourseFor(sid);
    if (!s || !course) return { ok: false, error: 'no-course' };
    const existing = Store.nextCourseRequests().find((r) => r.studentId === sid && r.courseLevel === course.level && r.status === 'new');
    if (existing) return { ok: true, duplicate: true };
    const req = { studentId: s.id, studentName: s.name, groupId: s.groupId, courseLevel: course.level, availability: cleanAvailability(availability || {}), status: 'new', createdAt: new Date().toISOString() };
    const client = cloudClient();
    if (client) {
      try {
        const res = await client.from('next_course_requests').insert({ student_id: req.studentId, student_name: req.studentName, group_id: req.groupId, course_level: req.courseLevel, availability: req.availability, status: req.status });
        if (res.error) throw res.error;
        return { ok: true, where: 'cloud' };
      } catch (e) { /* local fallback */ }
    }
    req.id = uid('ncr');
    db.nextCourseRequests.unshift(req);
    Store.save();
    return { ok: true, where: 'local' };
  };
  Store.fetchNextCourseRequests = async function () {
    ...
    try {
      ...
      db.nextCourseRequests = (res.data || []).map((r) => ({ cloudId: r.id, studentId: r.student_id, studentName: r.student_name, groupId: r.group_id, courseLevel: r.course_level, availability: cleanAvailability(r.availability || {}), status: r.status || 'new', createdAt: r.created_at })).concat(localOnly);
      Store.save();
      return { ok: true };
