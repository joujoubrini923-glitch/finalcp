...
(function () {
  ...
  Admin.render = function (root, tab) {
    ...
    ({
      ...
      achievements: tabAchievements, requests: tabRequests, nextcourse: tabNextCourseRequests, scheduling: tabScheduling, clinic: tabClinic, seasons: tabSeasons, verify: tabVerify, settings: tabSettings, academy: (el) => tabSettingsSection(el, 'academy-settings'), course: (el) => tabSettingsSection(el, 'course-settings'), nextcourses: (el) => tabSettingsSection(el, 'next-course-settings'), scoring: (el) => tabSettingsSection(el, 'scoring-settings'), gamification: (el) => tabSettingsSection(el, 'gamification-settings'), levels: (el) => tabSettingsSection(el, 'levels-settings'), topics: (el) => tabSettingsSection(el, 'topics-settings'), security: (el) => tabSettingsSection(el, 'security-settings'), data: tabData,
    })[tab](content);
  ...
  function tabSettingsSection(el, id) {
    ...
    if (!target) return;
    const labels = { 'academy-settings': 'Academy Settings', 'course-settings': 'Course Advertisement', 'next-course-settings': 'Next Course Ads', 'scoring-settings': 'Scoring', 'gamification-settings': 'Bonuses & Clinic', 'levels-settings': 'Levels', 'topics-settings': 'Topics', 'security-settings': 'Security' };
  ...
  function tabScheduling(el) {
    let failed = false;
    const render = () => {
      const apps = Store.joinRequests().filter((r) => r.availability && Object.values(r.availability).some((ranges) => Array.isArray(ranges) && ranges.length));
      const cell = (r, key) => ((r.availability && r.availability[key]) || []).map((x) => `${U.esc(x.start)}–${U.esc(x.end)}`).join('<br>') || '<span class="muted">—</span>';
      el.innerHTML = `<div class="admin-subpage-head"><span class="section-kicker"><i class="dot"></i>OPERATIONS</span><h2>${ic('calendar')}Weekly Scheduling</h2><p>See everyone’s free weekly ranges before you create the next group timetable.</p></div>
        <div class="card availability-board scheduling-board">
          <div class="card-title">${ic('calendar')}Student availability <span class="chip" style="margin-left:auto">${apps.length} students</span></div>
          ${failed ? `<div class="vf-offline" style="margin-top:12px">${ic('warn')}<span>The cloud availability column could not be loaded. Run the updated Join Requests SQL from the Requests tab.</span></div>` : ''}
          ${apps.length ? `<div class="table-wrap" style="margin-top:12px"><table class="table availability-table"><thead><tr><th>Student</th>${Store.weekdays().map((d) => `<th>${d.label.slice(0, 3)}</th>`).join('')}</tr></thead><tbody>${apps.map((r) => `<tr><td><b>${U.esc(r.name)}</b><div class="muted tiny">${U.esc(r.level || '')}</div></td>${Store.weekdays().map((d) => `<td>${cell(r, d.key)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>` : '<div class="availability-empty">No student availability has been submitted yet.</div>'}
        </div>`;
    };
    render();
    if (Store.cloudEnabled && Store.cloudEnabled() && Store.cloudStatus().authed) Store.fetchJoinRequests().then((r) => { failed = !r.ok; render(); });
  }
