...
(function () {
  ...
  function wireSpace(root, p) {
    ...
    const renderRules = () => {
      const s = me(); if (!s) return; const S = Store.settings(); const desc = S.scoringRuleDescriptions || {}; const challenges = Store.visibleReferralChallengesFor ? Store.visibleReferralChallengesFor(s.id) : [];
      const challengeHTML = challenges.length ? challenges.map((c) => { const standings = Store.referralStandings(c); const mine = standings.find((x) => x.s.id === s.id); const third = standings[2]; const gap = third && mine && mine.rank > 3 ? Math.max(0, third.score - mine.score + 1) : 0; const win = (c.winners || []).find((w) => w.studentId === s.id); return `<div class="card" style="margin-bottom:14px"><div class="card-title">${ic('crown')}${U.esc(c.title || 'Leaderboard Discount Challenge')}</div><p class="muted tiny">${U.esc(desc.referral || '')}</p><div class="table-wrap"><table class="table"><tbody><tr><th>Leaderboard</th><td>${c.groupId ? U.esc((Store.group(c.groupId) || {}).name || 'Selected group') : 'Global academy leaderboard'}</td></tr><tr><th>Period</th><td>${U.fmtDate(c.start)} → ${c.end ? U.fmtDate(c.end) : 'open'}</td></tr><tr><th>Metric</th><td>Total score at the end of the period, including coach manual adjustments</td></tr><tr><th>Your rank</th><td>${mine ? '#' + mine.rank + ' · ' + U.fmtNum(mine.score) + ' pts' : 'Not ranked yet'}${gap ? ` · ${U.fmtNum(gap)} pts from Rank #3` : ''}</td></tr><tr><th>Top 3 discounts</th><td>${(c.discounts || [30,20,10]).map((d, i) => `#${i + 1}: ${d}%`).join(' · ')} for one invited friend in Level 1</td></tr>${win ? `<tr><th>Your winner code</th><td><b class="mono">${U.esc(win.code)}</b> · ${win.discount}% discount · ${win.used ? 'used' : 'valid until used'}</td></tr>` : ''}</tbody></table></div></div>`; }).join('') : `<div class="card" style="margin-bottom:14px"><div class="card-title">${ic('crown')}Leaderboard Discount Challenge</div><p class="muted tiny">No referral discount challenge is visible for you right now.</p></div>`;
      const details = {
        problems: `${['easy','medium','hard'].map((d) => `${d}: ${S.difficultyScores[d] || 0} pts`).join(' · ')}`,
        daily: `${Store.dailyBonusTiers().map((t) => `${t.count}+ solves: +${t.points}`).join(' · ') || 'No daily bonus active'}`,
        streak: `+${S.bonusWeeklyStreak || 0} pts`,
        mastery: `+${S.bonusTopicMastery || 0} pts`,
        contests: `Podium: ${(S.contestPoints || []).join(' / ')} pts · beyond podium: +${S.contestPointsBeyond || 0}`,
        hints: `Hint costs: ${(S.hintCuts || []).join(' / ')} pts · minimum earnable score: ${S.hintFloor || 0}`,
        levels: (S.levels || []).map((l) => `${l.name}: ${l.minScore} pts, ${l.minTopics} topics, ${l.minAchievements || 0} achievements`).join(' · '),
      };
      const rules = (S.scoringRules || []).slice().sort((a, b) => (a.order || 0) - (b.order || 0));
      body.innerHTML = `${challengeHTML}<div class="card"><div class="card-title">${ic('shield')}Scoring Rules</div><div class="table-wrap"><table class="table"><thead><tr><th>Rule</th><th>Description</th></tr></thead><tbody>${rules.map((r) => `<tr><td><b>${ic(r.icon || 'shield')}${U.esc(r.name || r.key)}</b></td><td>${U.esc((details[r.key] ? details[r.key] + '. ' : '') + (r.description || (desc && desc[r.key]) || ''))}</td></tr>`).join('')}</tbody></table></div></div>`;
    };
    ...
    U.$('#sp-tabs', root).addEventListener('click', (e) => {
      ...
      ({ coach: renderCoach, unsolved: renderUnsolved, rules: renderRules, certificates: renderCertificates, score: renderScore, submit: renderSubmit, clinic: renderClinic }[b.dataset.t] || renderCoach)();
    });
    renderCoach();
  }
