...
(function () {
  ...
  Store.bonusPoints = function (sid) {
    ...
    let bonus = Math.max(0, Number(snap[sid]) || 0);
  ...
  Store.scoreEventsOf = function (sid) {
    ...
    const legacyBonus = S.legacyBonusSnapshot && S.legacyBonusSnapshot.values ? (Number(S.legacyBonusSnapshot.values[sid]) || 0) : 0;
    if (legacyBonus) add(start, legacyBonus, 'legacy-bonus', 'Previous bonus score', 'Bonuses earned before the new scoring rules started', 'star');
